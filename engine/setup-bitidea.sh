...
# 5. Symlinks the 'bitidea' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Bitidea Agent Setup${NC}"
echo ""
...
# ============================================================================
# PATH setup — symlink bitidea into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up bitidea command..."

BITIDEA_BIN="$SCRIPT_DIR/venv/bin/bitidea"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$BITIDEA_BIN" "$COMMAND_LINK_DIR/bitidea"
echo -e "${GREEN}✓${NC} Symlinked bitidea → $COMMAND_LINK_DISPLAY_DIR/bitidea"
...
# ============================================================================
# Seed bundled skills into ~/.bitidea/skills/
# ============================================================================

BITIDEA_SKILLS_DIR="${BITIDEA_HOME:-$HOME/.bitidea}/skills"
mkdir -p "$BITIDEA_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.bitidea/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$BITIDEA_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  bitidea status        # Check configuration"
if is_termux; then
    echo "  bitidea gateway       # Run gateway in foreground"
else
    echo "  bitidea gateway install # Install gateway service (messaging + cron)"
fi
echo "  bitidea cron list     # View scheduled jobs"
echo "  bitidea doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m bitidea_cli.main setup
fi
