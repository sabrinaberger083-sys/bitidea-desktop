# ============================================================================
# Bitidea Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$SkipSetup,
    [string]$Branch = "main",
    [string]$BitideaHome = "$env:LOCALAPPDATA\bitidea",
    [string]$InstallDir = "$env:LOCALAPPDATA\bitidea\bitidea-agent"
)
...
$RepoUrlSsh = "git@github.com:sabrinaberger083-sys/bitidea-agent.git"
$RepoUrlHttps = "https://github.com/sabrinaberger083-sys/bitidea-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "│             ⚕ Bitidea Agent Installer                    │" -ForegroundColor Magenta
    Write-Host "├─────────────────────────────────────────────────────────┤" -ForegroundColor Magenta
...
function Test-Node {
    ...
    $managedNode = "$BitideaHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$BitideaHome\node;$env:Path"
        Write-Success "Node.js $version found (Bitidea-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # Fallback: download binary zip to ~/.bitidea/node/
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\bitidea-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$BitideaHome\node") { Remove-Item -Recurse -Force "$BitideaHome\node" }
                Move-Item $extractedDir.FullName "$BitideaHome\node"
                $env:Path = "$BitideaHome\node;$env:Path"

                $version = & "$BitideaHome\node\node.exe" --version
                Write-Success "Node.js $version installed to ~/.bitidea/node/"
                $script:HasNode = $true
...
function Install-Repository {
    ...
    if (Test-Path $InstallDir) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                $zipUrl = "https://github.com/sabrinaberger083-sys/bitidea-agent/archive/refs/heads/$Branch.zip"
                $zipPath = "$env:TEMP\bitidea-agent-$Branch.zip"
                $extractPath = "$env:TEMP\bitidea-agent-extract"
...
function Set-PathVariable {
    Write-Info "Setting up bitidea command..."
    
    if ($NoVenv) {
        $bitideaBin = "$InstallDir"
    } else {
        $bitideaBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so bitidea is globally available
    # On Windows, the bitidea.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$bitideaBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$bitideaBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $bitideaBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set BITIDEA_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\bitidea instead
    # of the Unix default ~/.bitidea
    $currentBitideaHome = [Environment]::GetEnvironmentVariable("BITIDEA_HOME", "User")
    if (-not $currentBitideaHome -or $currentBitideaHome -ne $BitideaHome) {
        [Environment]::SetEnvironmentVariable("BITIDEA_HOME", $BitideaHome, "User")
        Write-Success "Set BITIDEA_HOME=$BitideaHome"
    }
    $env:BITIDEA_HOME = $BitideaHome
    
    # Update current session
    $env:Path = "$bitideaBin;$env:Path"
    
    Write-Success "bitidea command ready"
}

function Copy-ConfigTemplates {
    Write-Info "Setting up configuration files..."
    
    # Create ~/.bitidea directory structure
    New-Item -ItemType Directory -Force -Path "$BitideaHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$BitideaHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$BitideaHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$BitideaHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$BitideaHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$BitideaHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$BitideaHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$BitideaHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$BitideaHome\skills" | Out-Null
    New-Item -ItemType Directory -Force -Path "$BitideaHome\whatsapp\session" | Out-Null
    
    # Create .env
    $envPath = "$BitideaHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.bitidea/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.bitidea/.env"
        }
    } else {
        Write-Info "~/.bitidea/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$BitideaHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.bitidea/config.yaml from template"
        }
    } else {
        Write-Info "~/.bitidea/config.yaml already exists, keeping it"
    }
    
    # Create SOUL.md if it doesn't exist (global persona file)
    $soulPath = "$BitideaHome\SOUL.md"
...
"@ | Set-Content -Path $soulPath -Encoding UTF8
        Write-Success "Created ~/.bitidea/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.bitidea/"
    
    # Seed bundled skills into ~/.bitidea/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.bitidea/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.bitidea/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$BitideaHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.bitidea/skills/"
            }
        }
    }
}
...
function Invoke-SetupWizard {
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m bitidea_cli.main setup
    } else {
        python -m bitidea_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $bitideaCmd = "$InstallDir\venv\Scripts\bitidea.exe"
    if (-not (Test-Path $bitideaCmd)) {
        $bitideaCmd = "bitidea"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$BitideaHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'bitidea whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$BitideaHome\logs\gateway.log"
            Start-Process -FilePath $bitideaCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$BitideaHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: bitidea gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: bitidea gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$BitideaHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$BitideaHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$BitideaHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$BitideaHome\bitidea-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   bitidea              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   bitidea setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   bitidea config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   bitidea config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   bitidea gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   bitidea update       " -NoNewline -ForegroundColor Green
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/sabrinaberger083-sys/bitidea-agent/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
