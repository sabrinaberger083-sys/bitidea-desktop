...
# Configuration
REPO_URL_SSH="git@github.com:sabrinaberger083-sys/bitidea-agent.git"
REPO_URL_HTTPS="https://github.com/sabrinaberger083-sys/bitidea-agent.git"
BITIDEA_HOME="${BITIDEA_HOME:-$HOME/.bitidea}"
INSTALL_DIR="${BITIDEA_INSTALL_DIR:-$BITIDEA_HOME/bitidea-agent}"
PYTHON_VERSION="3.11"
NODE_VERSION="22"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --dir PATH     Installation directory (default: ~/.bitidea/bitidea-agent)"
            echo "  --bitidea-home PATH  Data directory (default: ~/.bitidea, or \$BITIDEA_HOME)"
...
get_bitidea_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/bitidea" ]; then
        echo "$link_dir/bitidea"
    else
        echo "bitidea"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/sabrinaberger083-sys/bitidea-agent/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
check_node() {
    ...
    if [ -x "$BITIDEA_HOME/node/bin/node" ]; then
        export PATH="$BITIDEA_HOME/node/bin:$PATH"
        local found_ver=$("$BITIDEA_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (Bitidea-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.bitidea/node/..."
    ...
    # Place into ~/.bitidea/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$BITIDEA_HOME/node"
    mkdir -p "$BITIDEA_HOME"
    mv "$extracted_dir" "$BITIDEA_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$BITIDEA_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$BITIDEA_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$BITIDEA_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$BITIDEA_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$BITIDEA_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.bitidea/node/"
    HAS_NODE=true
}
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Bitidea Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="bitidea-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Bitidea behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up bitidea command..."

    if [ "$USE_VENV" = true ]; then
        BITIDEA_BIN="$INSTALL_DIR/venv/bin/bitidea"
    else
        BITIDEA_BIN="$(which hermes 2>/dev/null || echo "")"
        if [ -z "$BITIDEA_BIN" ]; then
            log_warn "bitidea not found on PATH after install"
            return 0
        fi
    fi

    # Verify the entry point script was actually generated
    if [ ! -x "$BITIDEA_BIN" ]; then
        log_warn "bitidea entry point not found at $BITIDEA_BIN"
    ...
    # Create a user-facing shim for the bitidea command.
    mkdir -p "$command_link_dir"
    ln -sf "$BITIDEA_BIN" "$command_link_dir/bitidea"
    log_success "Symlinked bitidea → $command_link_display_dir/bitidea"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "bitidea command ready"
        return 0
    fi
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Bitidea Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Bitidea Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so bitidea works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.bitidea directory structure (config at top level, code in subdir)
    mkdir -p "$BITIDEA_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills,whatsapp/session}

    # Create .env at ~/.bitidea/.env (top level, easy to find)
    if [ ! -f "$BITIDEA_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$BITIDEA_HOME/.env"
            log_success "Created ~/.bitidea/.env from template"
        else
            touch "$BITIDEA_HOME/.env"
            log_success "Created ~/.bitidea/.env"
        fi
    else
        log_info "~/.bitidea/.env already exists, keeping it"
    fi

    # Create config.yaml at ~/.bitidea/config.yaml (top level, easy to find)
    if [ ! -f "$BITIDEA_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$BITIDEA_HOME/config.yaml"
            log_success "Created ~/.bitidea/config.yaml from template"
        fi
    else
        log_info "~/.bitidea/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$BITIDEA_HOME/SOUL.md" ]; then
        cat > "$BITIDEA_HOME/SOUL.md" << 'SOUL_EOF'
# Bitidea Agent Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how Bitidea communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.bitidea/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.bitidea/"

    # Seed bundled skills into ~/.bitidea/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.bitidea/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.bitidea/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$BITIDEA_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$BITIDEA_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.bitidea/skills/"
        fi
    fi
}
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        case "$DISTRO" in
            ubuntu|debian|raspbian|pop|linuxmint|elementary|zorin|kali|parrot)
                ...
                log_info "This is standard Playwright setup — Bitidea itself does not require root access."
...
run_setup_wizard() {
    ...
    if ! [ -e /dev/tty ]; then
        log_info "Setup wizard skipped (no terminal available). Run 'bitidea setup' after install."
        return 0
    fi
    ...
    # Run bitidea setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m bitidea_cli.main setup < /dev/tty
    else
        python -m bitidea_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$BITIDEA_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Bitidea to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$BITIDEA_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'bitidea whatsapp' to pair via QR code..."
            echo ""
            read -p "Pair WhatsApp now? [Y/n] " -n 1 -r
            echo
            if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
                BITIDEA_CMD="$(get_bitidea_command_path)"
                $BITIDEA_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'bitidea whatsapp' to pair."
        fi
    fi

    if ! [ -e /dev/tty ]; then
        log_info "Gateway setup skipped (no terminal available). Run 'bitidea gateway install' later."
        return 0
    fi
    ...
    if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $BITIDEA_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $BITIDEA_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: bitidea gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: bitidea gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $BITIDEA_CMD gateway > "$BITIDEA_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.bitidea/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: bitidea gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: bitidea gateway"
    fi
}
...
print_success() {
    ...
    # Show file locations
    echo -e "${CYAN}${BOLD}📁 Your files (all in ~/.bitidea/):${NC}"
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    ~/.bitidea/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  ~/.bitidea/.env"
    echo -e "   ${YELLOW}Data:${NC}      ~/.bitidea/cron/, sessions/, logs/"
    echo -e "   ${YELLOW}Code:${NC}      ~/.bitidea/bitidea-agent/"
    echo ""
    ...
    echo ""
    echo -e "   ${GREEN}bitidea${NC}              Start chatting"
    echo -e "   ${GREEN}bitidea setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}bitidea config${NC}       View/edit configuration"
    echo -e "   ${GREEN}bitidea config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}bitidea gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}bitidea update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'bitidea' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'bitidea' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
