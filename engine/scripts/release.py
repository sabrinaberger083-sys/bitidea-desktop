...
VERSION_FILE = REPO_ROOT / "bitidea_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "bitidea@nousresearch.com": "NousResearch",
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "bitidea@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/sabrinaberger083-sys/bitidea-agent",
    ...
    lines.append(f"# Bitidea Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Bitidea Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Bitidea Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Bitidea Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Bitidea Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Bitidea Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Bitidea Agent v{new_version} ({calver_date})' "
