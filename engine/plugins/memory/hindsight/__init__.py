...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID   — memory bank identifier (default: bitidea)
  HINDSIGHT_BUDGET    — recall budget: low/mid/high (default: mid)
  HINDSIGHT_API_URL   — API endpoint
  HINDSIGHT_MODE      — cloud or local (default: cloud)

Or via $BITIDEA_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $BITIDEA_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_bitidea_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "bitidea": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "bitidea"),
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Bitidea Agent and the User"
    ...
    def save_config(self, values, bitidea_home):
        """Write config to $BITIDEA_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(bitidea_home) / "hindsight"
    ...
    def post_setup(self, bitidea_home: str, config: dict) -> None:
        ...
        from bitidea_cli.memory_setup import _curses_select
        ...
        provider_config["bank_id"] = "bitidea"
        provider_config["recall_budget"] = "mid"
        bank_id = "bitidea"
        config["memory"]["provider"] = "hindsight"
        save_config(config)

        self.save_config(provider_config, bitidea_home)

        if env_writes:
            env_path = Path(bitidea_home) / ".env"
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name", "default": "bitidea"},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Bitidea Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "bitidea"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "bitidea"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = self._config.get("banks", {}).get("bitidea", {})
        self._bank_id = self._config.get("bank_id") or banks.get("bankId", "bitidea")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Bitidea Agent and the User")
