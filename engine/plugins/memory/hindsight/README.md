...
Bitidea spins up a local Hindsight daemon with built-in PostgreSQL. Requires an LLM API key for memory extraction and synthesis. The daemon starts automatically in the background on first use and stops after 5 minutes of inactivity.
...
Daemon startup logs: `~/.bitidea/logs/hindsight-embed.log`
Daemon runtime logs: `~/.hindsight/profiles/<profile>.log`

To open the Hindsight web UI (local embedded mode only):
```bash
hindsight-embed -p bitidea ui start
```
...
Config file: `~/.bitidea/hindsight/config.json`
...
| Key | Default | Description |
|-----|---------|-------------|
| `bank_id` | `bitidea` | Memory bank name |
...
| `retain_context` | `conversation between Bitidea Agent and the User` | Context label for retained memories |
...
The LLM API key is stored in `~/.bitidea/.env` as `HINDSIGHT_LLM_API_KEY`.
