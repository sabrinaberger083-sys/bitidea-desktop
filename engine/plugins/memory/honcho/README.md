...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/bitidea>
...
```bash
bitidea honcho setup    # full interactive wizard (cloud or local)
bitidea memory setup    # generic picker, also works
```

Or manually:
```bash
bitidea config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.bitidea/.env
```
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$BITIDEA_HOME/honcho.json` | Profile-local (isolated Bitidea instances) |
| 2 | `~/.bitidea/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Bitidea profile: `bitidea` (default) or `bitidea.<profile>`.
...
| 4 | `per-session` strategy | Bitidea session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`bitidea-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`bitidea`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-bitidea-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening bitidea in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Bitidea session ID (timestamp + hex). Every `bitidea` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Bitidea profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`bitidea`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Bitidea profile: `bitidea` (default) or `bitidea.<profile>` (e.g. `bitidea -p coder` → host key `bitidea.coder`).
...
| `BITIDEA_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `bitidea honcho setup` | Full interactive setup wizard |
| `bitidea honcho status` | Show resolved config for active profile |
| `bitidea honcho enable` / `disable` | Toggle Honcho for active profile |
| `bitidea honcho mode <mode>` | Change recall or observation mode |
| `bitidea honcho peer --user <name>` | Update user peer name |
| `bitidea honcho peer --ai <name>` | Update AI peer name |
| `bitidea honcho tokens --context <N>` | Set context token budget |
| `bitidea honcho tokens --dialectic <N>` | Set dialectic max chars |
| `bitidea honcho map <name>` | Map current directory to a session name |
| `bitidea honcho sync` | Create host blocks for all Bitidea profiles |
