...
Config file: `$BITIDEA_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `bitidea` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `bitidea-{identity}` → `bitidea-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Bitidea profile:

```json
{
  "container_tag": "bitidea-{identity}"
}
```

For a profile named `coder`, this resolves to `bitidea-coder`. The default profile resolves to `bitidea-default`. Without `{identity}`, all profiles share the same container.
