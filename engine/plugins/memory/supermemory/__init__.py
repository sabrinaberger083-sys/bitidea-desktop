...
_DEFAULT_CONTAINER_TAG = "bitidea"
...
def _load_supermemory_config(bitidea_home: str) -> dict:
    config = _default_config()
    config_path = Path(bitidea_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, bitidea_home: str) -> None:
    config_path = Path(bitidea_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `bitidea memory setup`.
        # All other options are documented for $BITIDEA_HOME/supermemory.json
    ...
    def save_config(self, values, bitidea_home):
        ...
        _save_supermemory_config(sanitized, bitidea_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from bitidea_constants import get_bitidea_home
        self._bitidea_home = kwargs.get("bitidea_home") or str(get_bitidea_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._bitidea_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "bitidea", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "bitidea_memory", "target": target, "type": "explicit_memory"},
