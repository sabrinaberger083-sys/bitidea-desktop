"""
Bitidea MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Bitidea-specific extra)

Usage:
    bitidea mcp serve
    bitidea mcp serve --verbose
...
logger = logging.getLogger("bitidea.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using BITIDEA_HOME."""
    try:
        from bitidea_constants import get_bitidea_home
        return get_bitidea_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("BITIDEA_HOME", Path.home() / ".bitidea")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from bitidea_constants import get_bitidea_home
        directory_file = get_bitidea_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("BITIDEA_HOME", Path.home() / ".bitidea")
...
class EventBridge:
    ...
    This is the Bitidea equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from bitidea_constants import get_bitidea_home
            db_file = get_bitidea_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("BITIDEA_HOME", Path.home() / ".bitidea")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Bitidea MCP server with all tools registered."""
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(
            "MCP server requires the 'mcp' package. "
            "Install with: pip install 'bitidea-agent[mcp]'"
        )

    mcp = FastMCP(
        "bitidea",
        instructions=(
            "Bitidea Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Bitidea MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
