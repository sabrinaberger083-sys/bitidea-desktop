"""ACP agent server — exposes Bitidea Agent via the Agent Client Protocol."""
...
try:
    from bitidea_cli import __version__ as BITIDEA_VERSION
except Exception:
    BITIDEA_VERSION = "0.0.0"
...
class BitideaACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Bitidea AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Bitidea version",
        },
    )
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = getattr(state.agent, "enabled_toolsets", None) or ["bitidea-acp"]
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        if provider:
            auth_methods = [
                AuthMethodAgent(
                    ...
                    description=f"Authenticate Bitidea using the currently configured {provider} runtime credentials.",
                )
            ]
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="bitidea-agent", version=BITIDEA_VERSION),
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Bitidea on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
    ...
    def _cmd_model(self, args: str, state: SessionState) -> str:
        ...
        try:
            from bitidea_cli.models import parse_model_input, detect_provider_for_model
    ...
    def _cmd_tools(self, args: str, state: SessionState) -> str:
        try:
            from model_tools import get_tool_definitions
            toolsets = getattr(state.agent, "enabled_toolsets", None) or ["bitidea-acp"]
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Bitidea Agent v{BITIDEA_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Bitidea has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
