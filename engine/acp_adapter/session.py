"""ACP session manager — maps ACP sessions to Bitidea AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.bitidea/state.db``) so they
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Bitidea agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Bitidea AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Bitidea runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.bitidea/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``BITIDEA_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_bitidea_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=bitidea_home / "state.db")
            return self._db_instance
        except Exception:
