...
if tinker_atropos_dir.exists():
    ...
    os.environ['BITIDEA_QUIET'] = '1'  # Disable temp subdirectory creation
    print(f"📂 Terminal working directory: {tinker_atropos_dir}")
else:
    # Fall back to bitidea-agent directory if submodule not found
...
from bitidea_constants import get_bitidea_home, OPENROUTER_BASE_URL

DEFAULT_MODEL = "anthropic/claude-opus-4.5"
DEFAULT_BASE_URL = OPENROUTER_BASE_URL


def load_bitidea_config() -> dict:
    """
...
def main(
    ...
    Args:
        ...
        model: Model to use for the agent (reads from ~/.bitidea/config.yaml if not provided)
    ...
    """
    # Load config from ~/.bitidea/config.yaml
    config = load_bitidea_config()
