...
description: Migrate a user's OpenClaw customization footprint into Bitidea Agent. Imports Bitidea-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
version: 1.0.0
author: Bitidea Agent (Nous Research)
license: MIT
metadata:
  bitidea:
    tags: [Migration, OpenClaw, Bitidea, Memory, Persona, Import]
    related_skills: [bitidea-agent]
---

# OpenClaw -> Bitidea Migration

Use this skill when a user wants to move their OpenClaw setup into Bitidea Agent with minimal manual cleanup.
...
```bash
bitidea claw migrate              # Full interactive migration
...
bitidea claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `bitidea setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_bitidea.py` to:

- import `SOUL.md` into the Bitidea home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Bitidea memory entries
- merge OpenClaw command approval patterns into Bitidea `command_allowlist`
- migrate Bitidea-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.bitidea/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Bitidea workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.bitidea/tts/`
- archive non-secret docs that do not have a direct Bitidea destination
...
- `scripts/openclaw_to_bitidea.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.bitidea/skills/migration/openclaw-migration/scripts/openclaw_to_bitidea.py`

Do not guess a shorter path like `~/.bitidea/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.bitidea/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Bitidea-compatible secrets, currently:
...
Bitidea CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Bitidea. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Bitidea workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Bitidea skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.bitidea/skills/migration/openclaw-migration/scripts/openclaw_to_bitidea.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.bitidea/skills/migration/openclaw-migration/scripts/openclaw_to_bitidea.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.bitidea/skills/migration/openclaw-migration/scripts/openclaw_to_bitidea.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.bitidea/skills/migration/openclaw-migration/scripts/openclaw_to_bitidea.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.bitidea/skills/migration/openclaw-migration/scripts/openclaw_to_bitidea.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.bitidea/skills/migration/openclaw-migration/scripts/openclaw_to_bitidea.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Bitidea unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Bitidea targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Bitidea destination. Unsupported auth blobs must still be reported as skipped.
...
- Bitidea persona state imported
- Bitidea memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.bitidea/skills/openclaw-imports/`
