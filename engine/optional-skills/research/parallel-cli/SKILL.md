...
This is an optional third-party workflow, not a Bitidea core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with Bitidea native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer Bitidea native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer Bitidea native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
## Recommended Bitidea usage patterns
...
- Do not silently choose Parallel when Bitidea native tools are already sufficient.
