...
- smoke-test a server with the FastMCP CLI before wiring it into Bitidea or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Bitidea. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.bitidea/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.bitidea/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Bitidea integration, either:

- configure the server in `~/.bitidea/config.yaml` using the `native-mcp` skill, or
...
### Bitidea cannot see the deployed server

The server-building part may be correct while the Bitidea config is not. Load the `native-mcp` skill and configure the server in `~/.bitidea/config.yaml`, then restart Bitidea.
