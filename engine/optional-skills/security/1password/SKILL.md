...
version: 1.0.0
author: arceus77-7, enhanced by Bitidea Agent
license: MIT
metadata:
  bitidea:
...
- `tmux` available for stable authenticated sessions during Bitidea terminal calls (desktop app flow only)
...
### Service Account (recommended for Bitidea)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.bitidea/.env` (the skill will prompt for this on first load).
...
## Bitidea Execution Pattern (desktop app flow)

Bitidea terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/bitidea-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/bitidea-op.sock"
