---
name: honcho
description: Configure and use Honcho memory with Bitidea -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.
version: 2.0.0
author: Bitidea Agent
license: MIT
metadata:
  bitidea:
...
# Honcho Memory for Bitidea

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Bitidea profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/bitidea#running-honcho-locally-with-bitidea

### Verify

```bash
bitidea honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Bitidea profile's AI peer
...
Honcho models conversations as interactions between **peers**. Bitidea creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Bitidea instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Bitidea run |
| `global` | Single session across all directories |

Manual override: `bitidea honcho map my-project-name`
...
Each Bitidea profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
```bash
bitidea profile create coder --clone
# creates host block bitidea.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `bitidea.coder` host block in `honcho.json`
...
```bash
bitidea honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Bitidea when Honcho memory is active.
...
Config file: `$BITIDEA_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `bitidea honcho setup`. Ensure `memory.provider: honcho` is in `~/.bitidea/config.yaml`.

### Memory not persisting across sessions
Check `bitidea honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `bitidea profile create <name> --clone`. For existing profiles: `bitidea honcho sync`.
...
| Command | Description |
|---------|-------------|
| `bitidea honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `bitidea honcho status` | Show resolved config, connection test, peer info for active profile |
| `bitidea honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `bitidea honcho disable` | Disable Honcho for the active profile |
| `bitidea honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `bitidea honcho peers` | Show peer identities across all profiles |
| `bitidea honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `bitidea honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `bitidea honcho sessions` | List known directory-to-session-name mappings |
| `bitidea honcho map <name>` | Map current working directory to a Honcho session name |
| `bitidea honcho identity` | Seed AI peer identity or show both peer representations |
| `bitidea honcho sync` | Create host blocks for all Bitidea profiles that don't have one yet |
| `bitidea honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Bitidea + Honcho |
| `bitidea memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `bitidea memory status` | Show active memory provider and config |
| `bitidea memory off` | Disable external memory provider |
