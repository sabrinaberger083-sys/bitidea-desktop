...
Training tasks (3):
    1. Create ~/greeting.txt with "Hello from Bitidea Agent"
...
TRAIN_TASKS = [
    {
        "prompt": "Create a file at ~/greeting.txt containing exactly the text: Hello from Bitidea Agent",
        "verify_path": "~/greeting.txt",
        "expected_content": "Hello from Bitidea Agent",
    },
    {
...
class TerminalTestEnv(BitideaAgentBaseEnv):
    ...
    def config_init(cls) -> Tuple[TerminalTestEnvConfig, List[APIServerConfig]]:
        ...
        Claude for inference. API keys loaded from ~/bitidea-agent/.env.
        """
