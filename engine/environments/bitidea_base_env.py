"""
BitideaAgentBaseEnv -- Abstract Base Environment for Bitidea-Agent + Atropos

Provides the Atropos integration plumbing that all bitidea-agent environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via BitideaAgentLoop
...
# Ensure the bitidea-agent repo root is on sys.path so that imports like
...
# Load API keys from bitidea-agent/.env so all environments can access them
...
# Import bitidea-agent toolset infrastructure
...
class BitideaAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for bitidea-agent Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of bitidea toolsets to enable (e.g., ['terminal', 'file', 'web']). "
    ...
    tool_call_parser: str = Field(
        ...
        "Options: bitidea, mistral, llama3_json, qwen, deepseek_v3, etc.",
    )
    ...
    default_result_size_chars: int = Field(
        ...
        "to sandbox. Results exceeding this are written to /tmp/bitidea-results/ "
...
class BitideaAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for bitidea-agent Atropos integration.
    ...
    name: Optional[str] = "bitidea-agent"
    env_config_cls = BitideaAgentEnvConfig

    def __init__(
        self,
        config: BitideaAgentEnvConfig,
        ...
        # Set terminal environment variables so bitidea tools pick them up.
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY bitidea-agent tool (terminal, file, web,
