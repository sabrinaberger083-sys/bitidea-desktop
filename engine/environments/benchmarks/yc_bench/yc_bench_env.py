...
The evaluate flow:
    ...
    2. evaluate()  -- Iterates over all runs sequentially through:
        a. rollout_and_score_eval()  -- Per-run agent loop
            ...
            - Runs BitideaAgentLoop with terminal tool only
...
Requires: pip install bitidea-agent[yc-bench]
"""
...
class YCBenchEvalConfig(BitideaAgentEnvConfig):
    """
    Configuration for the YC-Bench evaluation environment.

    Extends BitideaAgentEnvConfig with YC-Bench-specific settings for
...
class YCBenchEvalEnv(BitideaAgentBaseEnv):
    """
    ...
    a competing built-in agent loop). The BitideaAgentLoop then drives the
    ...
    async def rollout_and_score_eval(self, eval_item: Dict[str, Any]) -> Dict:
        ...
        3. Runs BitideaAgentLoop with terminal tool
        ...
        try:
            ...
            # LiteLLM), which would compete with our BitideaAgentLoop.
            ...
            # ----------------------------------------------------------
            # Step 2: Run the BitideaAgentLoop
            # ----------------------------------------------------------
