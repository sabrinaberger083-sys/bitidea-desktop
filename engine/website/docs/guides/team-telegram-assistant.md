...
This tutorial walks you through setting up a Telegram bot powered by Bitidea Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Bitidea Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.bitidea/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Bitidea itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Bitidea Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_bitidea_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Bitidea Agent. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.bitidea/.env`:
...
```
[Gateway] Starting Bitidea Gateway...
...
bitidea gateway stop
bitidea gateway status

# View live logs
journalctl --user -u bitidea-gateway -f
...
bitidea gateway start
bitidea gateway stop
tail -f ~/.bitidea/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `bitidea gateway install` to update the plist.
:::
...
```bash
bitidea gateway stop && bitidea gateway start
```
...
bitidea pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.bitidea/.env`:
...
Control how much detail the bot shows when using tools. In `~/.bitidea/config.yaml`:
...
Customize how the bot communicates by editing `~/.bitidea/SOUL.md`:

For a full guide, see [Use SOUL.md with Bitidea](/docs/guides/use-soul-with-bitidea).
...
```bash
# From the CLI
bitidea cron list          # View all scheduled jobs
bitidea cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u bitidea-gateway` (Linux) or `~/.bitidea/logs/gateway.log` (macOS) |
| Cron job output | `~/.bitidea/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.bitidea/cron/jobs.json` |
| Pairing data | `~/.bitidea/pairing/` |
| Session history | `~/.bitidea/sessions/` |
