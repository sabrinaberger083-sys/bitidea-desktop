---
sidebar_position: 8
title: "Use Voice Mode with Bitidea"
description: "A practical guide to setting up and using Bitidea voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Bitidea
...
- you want Bitidea sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Bitidea.
...
## Step 1: make sure normal Bitidea works first
...
Bitidea supports both local and cloud speech stacks.
...
### If you use `bitidea setup`

If you choose NeuTTS in the setup wizard, Bitidea checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Bitidea transcribes and responds
...
- asking Bitidea to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Bitidea loop.

## Tuning CLI behavior

### Silence threshold

If Bitidea starts/stops too aggressively, tune:
...
Bitidea stays a normal chat bot, but can speak replies.
...
- you want Bitidea to function like a portable research or ops assistant
...
Bitidea joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Bitidea detects speech boundaries
- transcripts are posted in the associated text channel
- Bitidea responds in text and audio
...
1. get text Bitidea working
2. install `bitidea-agent[voice]`
