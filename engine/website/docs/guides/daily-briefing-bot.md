...
2. **Bitidea spins up** a fresh agent session with your prompt
...
- **Bitidea Agent installed** — see the [Installation guide](/docs/getting-started/installation)
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.bitidea/cron/output/` and you can read them anytime.
:::
...
Bitidea will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Bitidea Agent
```
...
Just tell Bitidea what you want:
...
Bitidea will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Bitidea to delegate each topic to a sub-agent:
...
Bitidea will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
