...
Copy-paste recipes for common automation patterns. Each template uses Bitidea's built-in [cron scheduler](/docs/user-guide/features/cron) for time-based triggers and [webhook platform](/docs/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`bitidea webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `bitidea webhook subscribe`) |
...
```bash
bitidea cron create "0 2 * * *" \
  "You are a project manager triaging the sabrinaberger083-sys/bitidea-agent GitHub repo.

1. Run: gh issue list --repo sabrinaberger083-sys/bitidea-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
bitidea webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
bitidea cron create "0 9 * * 1" \
  "Scan the sabrinaberger083-sys/bitidea-agent repo for documentation drift.

1. Run: gh pr list --repo sabrinaberger083-sys/bitidea-agent --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (bitidea_cli/commands.py, bitidea_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (bitidea_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
bitidea cron create "0 6 * * *" \
  "Run a dependency security audit on the bitidea-agent project.

1. cd ~/.bitidea/bitidea-agent && source .venv/bin/activate
...
```bash
bitidea webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
bitidea webhook subscribe alert-triage \
...
```python title="~/.bitidea/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Bitidea-Monitor/1.0"})
...
bitidea cron create "every 30m" \
  ...
  --script ~/.bitidea/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
bitidea cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Bitidea Agent development." \
...
```bash
bitidea webhook subscribe github-issues \
  --events "issues" \
...
```bash
bitidea webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
bitidea webhook subscribe stripe-payments \
...
```bash
bitidea cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the bitidea-agent codebase.
