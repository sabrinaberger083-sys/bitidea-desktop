...
This guide covers adding a new messaging platform to the Bitidea gateway. A platform adapter connects Bitidea to an external messaging service (Telegram, Discord, WeCom, etc.) so users can interact with the agent through that service.
...
1. **`bitidea_cli/config.py`** — Add all `NEWPLAT_*` vars to `_EXTRA_ENV_KEYS`
2. **`bitidea_cli/gateway.py`** — Add entry to `_PLATFORMS` list with key, label, emoji, token_var, setup_instructions, and vars
3. **`bitidea_cli/platforms.py`** — Add `PlatformInfo` entry with label and default_toolset (used by `skills_config` and `tools_config` TUIs)
4. **`bitidea_cli/setup.py`** — Add `_setup_newplat()` function (can delegate to `gateway.py`) and add tuple to the messaging platforms list
5. **`bitidea_cli/status.py`** — Add platform detection entry: `"NewPlat": ("NEWPLAT_TOKEN", "NEWPLAT_HOME_CHANNEL")`
6. **`bitidea_cli/dump.py`** — Add `"newplat": "NEWPLAT_TOKEN"` to platform detection dict
...
1. **`toolsets.py`** — Add `"bitidea-newplat"` toolset definition with `_BITIDEA_CORE_TOOLS`
2. **`toolsets.py`** — Add `"bitidea-newplat"` to the `"bitidea-gateway"` includes list
...
| `website/docs/reference/toolsets-reference.md` | bitidea-newplat toolset |
