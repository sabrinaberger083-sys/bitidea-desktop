...
description: "How Bitidea builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

Bitidea deliberately separates:
...
1. agent identity — `SOUL.md` from `BITIDEA_HOME` when available, otherwise falls back to `DEFAULT_AGENT_IDENTITY` in `prompt_builder.py`
...
```
# Layer 1: Agent Identity (from ~/.bitidea/SOUL.md)
You are Bitidea, an AI assistant created by Nous Research.
...
`SOUL.md` lives at `~/.bitidea/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_bitidea_md(cwd_path)       # 1. .bitidea.md / HERMES.md (walks to git root)
    ...
    # SOUL.md from BITIDEA_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.bitidea.md`, `HERMES.md` | CWD up to git root | Bitidea-native project config |
...
- **YAML frontmatter stripped** — `.bitidea.md` frontmatter is removed (reserved for future config overrides)
...
1. `.bitidea.md` / `HERMES.md` (walks to git root)
