...
The messaging gateway is the long-running process that connects Bitidea to 14+ external messaging platforms through a unified architecture.
...
1. `resolve_command()` from `bitidea_cli/commands.py` maps input to canonical name (handles aliases, prefix matching)
...
|--------|-----------------|
| `~/.bitidea/.env` | API keys, bot tokens, platform credentials |
| `~/.bitidea/config.yaml` | Model settings, tool configuration, display options |
...
Hooks are discovered from `gateway/builtin_hooks/` (always active) and `~/.bitidea/hooks/` (user-installed). Each hook is a directory with a `HOOK.yaml` manifest and `handler.py`.
...
- `bitidea gateway start` / `bitidea gateway stop` — manual control
- `systemctl` (Linux) or `launchctl` (macOS) — service management
- PID file at `~/.bitidea/gateway.pid` — profile-scoped process tracking

**Profile-scoped vs global**: `start_gateway()` uses profile-scoped PID files. `bitidea gateway stop` stops only the current profile's gateway. `bitidea gateway stop --all` uses global `ps aux` scanning to kill all gateway processes (used during updates).
