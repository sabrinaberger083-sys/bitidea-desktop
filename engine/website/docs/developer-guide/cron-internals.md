...
description: "How Bitidea stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `bitidea_cli/cron.py` | CLI `bitidea cron` subcommands |
...
Jobs are stored in `~/.bitidea/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `bitidea cron` commands are run or during active CLI sessions.
...
```python
# ~/.bitidea/scripts/check_competitors.py
...
2. **Environment variable** — `BITIDEA_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.bitidea/cron/output/` |
...
The `bitidea cron` CLI provides direct job management:

```bash
bitidea cron list                    # Show all jobs
bitidea cron create                  # Interactive job creation (alias: add)
bitidea cron edit <job_id>           # Edit job configuration
bitidea cron pause <job_id>          # Pause a running job
bitidea cron resume <job_id>         # Resume a paused job
bitidea cron run <job_id>            # Trigger immediate execution
bitidea cron remove <job_id>         # Delete a job
```
