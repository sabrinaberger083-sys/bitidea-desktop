...
description: "Building RL training environments, running evaluation benchmarks, and generating SFT data with the Bitidea-Agent Atropos integration"
---

# Environments, Benchmarks & Data Generation

Bitidea Agent includes a full environment framework that connects its tool-calling capabilities to the [Atropos](https://github.com/NousResearch/atropos) RL training framework. This enables three workflows:
...
The Python environment framework documented here lives under the repo's `environments/` directory and is the implementation-level API for Bitidea/Atropos integration. This is separate from the user-facing `rl_*` tools, which operate as an orchestration surface for remote RL training workflows.
:::
...
classDiagram
    ...
    BaseEnv <|-- BitideaAgentBaseEnv
    BitideaAgentBaseEnv <|-- TerminalTestEnv
    BitideaAgentBaseEnv <|-- BitideaSweEnv
    BitideaAgentBaseEnv <|-- TerminalBench2EvalEnv
...
The bitidea-agent layer (`environments/bitidea_base_env.py`). Adds:
- **Terminal backend configuration** — sets `TERMINAL_ENV` for sandboxed execution (local, Docker, Modal, Daytona, SSH, Singularity)
- **Tool resolution** — `_resolve_tools_for_group()` calls bitidea-agent's `get_tool_definitions()` to get the right tool schemas based on enabled/disabled toolsets
- **Agent loop integration** — `collect_trajectory()` runs `BitideaAgentLoop` and scores the result
...
Your environment inherits from `BitideaAgentBaseEnv` and implements five methods:
...
`BitideaAgentLoop` (`environments/agent_loop.py`) is the reusable multi-turn agent engine. It runs the same tool-calling pattern as bitidea-agent's main loop:
...
| **Generic** | `call_tool(name, args)` — escape hatch for any bitidea-agent tool |
...
Available parsers: `bitidea`, `mistral`, `llama3_json`, `qwen`, `qwen3_coder`, `deepseek_v3`, `deepseek_v3_1`, `kimi_k2`, `longcat`, `glm45`, `glm47`.
...
pip install "bitidea-agent[yc-bench]"
...
```bash
python environments/bitidea_swe_env/bitidea_swe_env.py serve \
...
- Set `tool_call_parser` in config to match your model's format (e.g., `"bitidea"`, `"qwen"`, `"mistral"`)
...
```python
from environments.bitidea_base_env import BitideaAgentBaseEnv, BitideaAgentEnvConfig
from atroposlib.envs.server_handling.server_manager import APIServerConfig

class MyEnvConfig(BitideaAgentEnvConfig):
    my_custom_field: str = "default_value"

class MyEnv(BitideaAgentBaseEnv):
    name = "my-env"
...
### BitideaAgentEnvConfig Fields

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled_toolsets` | `List[str]` | `None` (all) | Which bitidea toolsets to enable |
...
| `tool_call_parser` | `str` | `"bitidea"` | Parser for Phase 2 raw output |
...
- [Modal](https://modal.com) account and CLI: `pip install "bitidea-agent[modal]"`
- `MODAL_TOKEN_ID` and `MODAL_TOKEN_SECRET` environment variables

### For YC-Bench

- `pip install "bitidea-agent[yc-bench]"` (installs the yc-bench CLI + SQLAlchemy)
...
```
environments/
├── bitidea_base_env.py          # Abstract base class (BitideaAgentBaseEnv)
├── agent_loop.py               # Multi-turn agent engine (BitideaAgentLoop)
├── tool_context.py             # Per-rollout tool access for reward functions
├── patches.py                  # Async-safety patches for Modal backend
│
├── tool_call_parsers/          # Phase 2 client-side parsers
│   ├── hermes_parser.py        # Bitidea/ChatML <tool_call> format
...
├── bitidea_swe_env/             # SWE-bench training environment
│
