...
Bitidea Agent uses a SQLite database (`~/.bitidea/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `bitidea_state.py`


## Architecture Overview

```
~/.bitidea/state.db (SQLite, WAL mode)
...
Multiple bitidea processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.bitidea/state.db
...
Default path: `~/.bitidea/state.db`

This is derived from `bitidea_constants.get_bitidea_home()` which resolves to
`~/.bitidea/` by default, or the value of `BITIDEA_HOME` environment variable.
