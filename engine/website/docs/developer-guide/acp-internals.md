...
The ACP adapter wraps Bitidea' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
bitidea acp / bitidea-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> load ~/.bitidea/.env
  -> configure stderr logging
  -> construct BitideaACPAgent
  -> acp.run_agent(agent)
```
...
- `allow_once` -> Bitidea `once`
- `allow_always` -> Bitidea `always`
- reject options -> Bitidea `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Bitidea tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["bitidea-acp"])
...
Instead it reuses Bitidea' runtime resolver:

- `acp_adapter/auth.py`
- `bitidea_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Bitidea provider/credentials.
...
- `toolsets.py` — `bitidea-acp` toolset definition
- `bitidea_cli/main.py` — `bitidea acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `bitidea-acp` script
