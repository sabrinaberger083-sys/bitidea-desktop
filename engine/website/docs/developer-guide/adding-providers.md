...
description: "How to add a new inference provider to Bitidea Agent — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

Bitidea can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `bitidea model` menu entries
...
1. `bitidea_cli/auth.py` decides how credentials are found.
2. `bitidea_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `bitidea_cli/models.py` and `bitidea_cli/main.py` make the provider show up in the CLI. (`bitidea_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
1. `bitidea_cli/auth.py`
2. `bitidea_cli/models.py`
3. `bitidea_cli/runtime_provider.py`
4. `bitidea_cli/main.py`
...
:::tip
`bitidea_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `bitidea setup`.
:::
...
- `PROVIDER_REGISTRY` in `bitidea_cli/auth.py`
- `_PROVIDER_LABELS` in `bitidea_cli/models.py`
- `_PROVIDER_ALIASES` in both `bitidea_cli/auth.py` and `bitidea_cli/models.py`
- CLI `--provider` choices in `bitidea_cli/main.py`
...
## Step 2: Add auth metadata in `bitidea_cli/auth.py`
...
- What env vars should Bitidea check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `bitidea_cli/models.py`
...
## Step 4: Resolve runtime data in `bitidea_cli/runtime_provider.py`
...
Be careful with API-key precedence. Bitidea already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `bitidea_cli/main.py`

A provider is not discoverable until it shows up in the interactive `bitidea model` flow.

Update these in `bitidea_cli/main.py`:
...
:::tip
`bitidea_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `bitidea model` and `bitidea setup` automatically.
:::
...
When you add a native provider, double-check that Bitidea is only sending fields that provider actually understands.
...
```bash
source venv/bin/activate
python -m bitidea_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
```bash
source venv/bin/activate
python -m bitidea_cli.main model
...
- [ ] `ProviderConfig` added in `bitidea_cli/auth.py`
- [ ] aliases added in `bitidea_cli/auth.py` and `bitidea_cli/models.py`
- [ ] model catalog added in `bitidea_cli/models.py`
- [ ] runtime branch added in `bitidea_cli/runtime_provider.py`
- [ ] CLI wiring added in `bitidea_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `bitidea model` but not `bitidea setup`
