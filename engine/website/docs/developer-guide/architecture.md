...
sidebar_position: 1
title: "Architecture"
description: "Bitidea Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Bitidea Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ bitidea_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # BitideaCLI — interactive terminal UI (~10,000 lines)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── bitidea_state.py           # SQLite session/state database with FTS5
├── bitidea_constants.py       # BITIDEA_HOME, profile-aware paths
...
│   ├── main.py               # Entry point — all `bitidea` subcommands (~6,000 lines)
...
│   ├── skills_config.py      # bitidea skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # bitidea tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # bitidea gateway start/stop
│
...
```text
User input → BitideaCLI.process_input()
...
- **`prompt_builder.py`** — Assembles the system prompt from: personality (SOUL.md), memory (MEMORY.md, USER.md), skills, context files (AGENTS.md, .bitidea.md), tool-use guidance, and model-specific instructions
...
Three discovery sources: `~/.bitidea/plugins/` (user), `.bitidea/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `bitidea plugins` or `config.yaml`.

→ [Plugin Guide](/docs/guides/build-a-bitidea-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Bitidea as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`bitidea -p <name>`) gets its own BITIDEA_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
