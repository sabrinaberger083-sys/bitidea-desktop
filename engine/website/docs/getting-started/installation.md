...
sidebar_position: 2
title: "Installation"
description: "Install Bitidea Agent on Linux, macOS, WSL2, or Android via Termux"
---

# Installation

Get Bitidea Agent up and running in under two minutes with the one-line installer, or follow the manual steps for full control.
...
Bitidea now ships a Termux-aware installer path too:
...
:::warning Windows
Native Windows is **not supported**. Please install [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) and run Bitidea Agent from there. The install command above works inside WSL2.
:::

### What the Installer Does

The installer handles everything automatically — all dependencies (Python, Node.js, ripgrep, ffmpeg), the repo clone, virtual environment, global `bitidea` command setup, and LLM provider configuration. By the end, you're ready to chat.
...
bitidea gateway setup  # Set up messaging platforms
bitidea config set     # Set individual config values
bitidea setup          # Or run the full setup wizard to configure everything at once
```
...
:::tip
You do **not** need to activate the venv to use `bitidea`. The entry point has a hardcoded shebang pointing to the venv Python, so it works globally once symlinked.
:::
...
mkdir -p ~/.bitidea/{cron,sessions,logs,memories,skills,pairing,hooks,image_cache,audio_cache,whatsapp/session}
...
Open `~/.bitidea/.env` and add at minimum an LLM provider key:
...
```bash
bitidea config set OPENROUTER_API_KEY sk-or-v1-your-key-here
```

### Step 8: Add `bitidea` to Your PATH
...
```bash
bitidea model       # Select your LLM provider and model
```

### Step 10: Verify the Installation

```bash
bitidea version    # Check that the command is available
bitidea doctor     # Run diagnostics to verify everything is working
bitidea status     # Check your configuration
bitidea chat -q "Hello! What tools do you have available?"
```
...
# Make bitidea available globally
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/bitidea" ~/.local/bin/bitidea
...
| Problem | Solution |
|---------|----------|
| `bitidea: command not found` | Reload your shell (`source ~/.bashrc`) or check PATH |
| `API key not set` | Run `bitidea model` to configure your provider, or `bitidea config set OPENROUTER_API_KEY your_key` |
| Missing config after update | Run `bitidea config check` then `bitidea config migrate` |

For more diagnostics, run `bitidea doctor` — it will tell you exactly what's missing and how to fix it.
