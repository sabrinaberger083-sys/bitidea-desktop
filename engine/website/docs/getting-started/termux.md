...
description: "Run Bitidea Agent directly on an Android phone with Termux"
---

# Bitidea on Android with Termux

This is the tested path for running Bitidea Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Bitidea from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Bitidea now ships a Termux-aware installer path:
...
- links `bitidea` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `bitidea` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/bitidea" "$PREFIX/bin/bitidea"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `bitidea` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.bitidea/.env`.
...
### `bitidea doctor` says ripgrep or Node is missing
