...
This page covers setting up inference providers for Bitidea Agent — from cloud APIs like OpenRouter and Anthropic, to self-hosted endpoints like Ollama and vLLM, to advanced routing and fallback configurations. You need at least one provider configured to use Bitidea.

## Inference Providers

You need at least one way to connect to an LLM. Use `bitidea model` to switch providers and models interactively, or configure directly:

| Provider | Setup |
|----------|-------|
| **Nous Portal** | `bitidea model` (OAuth, subscription-based) |
| **OpenAI Codex** | `bitidea model` (ChatGPT OAuth, uses Codex models) |
| **GitHub Copilot** | `bitidea model` (OAuth device code flow, `COPILOT_GITHUB_TOKEN`, `GH_TOKEN`, or `gh auth token`) |
| **GitHub Copilot ACP** | `bitidea model` (spawns local `copilot --acp --stdio`) |
| **Anthropic** | `bitidea model` (Claude Pro/Max via Claude Code auth, Anthropic API key, or manual setup-token) |
| **OpenRouter** | `OPENROUTER_API_KEY` in `~/.bitidea/.env` |
| **AI Gateway** | `AI_GATEWAY_API_KEY` in `~/.bitidea/.env` (provider: `ai-gateway`) |
| **z.ai / GLM** | `GLM_API_KEY` in `~/.bitidea/.env` (provider: `zai`) |
| **Kimi / Moonshot** | `KIMI_API_KEY` in `~/.bitidea/.env` (provider: `kimi-coding`) |
| **Kimi / Moonshot (China)** | `KIMI_CN_API_KEY` in `~/.bitidea/.env` (provider: `kimi-coding-cn`; aliases: `kimi-cn`, `moonshot-cn`) |
| **Arcee AI** | `ARCEEAI_API_KEY` in `~/.bitidea/.env` (provider: `arcee`; aliases: `arcee-ai`, `arceeai`) |
| **MiniMax** | `MINIMAX_API_KEY` in `~/.bitidea/.env` (provider: `minimax`) |
| **MiniMax China** | `MINIMAX_CN_API_KEY` in `~/.bitidea/.env` (provider: `minimax-cn`) |
| **Alibaba Cloud** | `DASHSCOPE_API_KEY` in `~/.bitidea/.env` (provider: `alibaba`, aliases: `dashscope`, `qwen`) |
| **Kilo Code** | `KILOCODE_API_KEY` in `~/.bitidea/.env` (provider: `kilocode`) |
| **Xiaomi MiMo** | `XIAOMI_API_KEY` in `~/.bitidea/.env` (provider: `xiaomi`, aliases: `mimo`, `xiaomi-mimo`) |
| **OpenCode Zen** | `OPENCODE_ZEN_API_KEY` in `~/.bitidea/.env` (provider: `opencode-zen`) |
| **OpenCode Go** | `OPENCODE_GO_API_KEY` in `~/.bitidea/.env` (provider: `opencode-go`) |
| **DeepSeek** | `DEEPSEEK_API_KEY` in `~/.bitidea/.env` (provider: `deepseek`) |
| **Hugging Face** | `HF_TOKEN` in `~/.bitidea/.env` (provider: `huggingface`, aliases: `hf`) |
| **Google / Gemini** | `GOOGLE_API_KEY` (or `GEMINI_API_KEY`) in `~/.bitidea/.env` (provider: `gemini`) |
| **Custom Endpoint** | `bitidea model` → choose "Custom endpoint" (saved in `config.yaml`) |
...
:::info Codex Note
The OpenAI Codex provider authenticates via device code (open a URL, enter a code). Bitidea stores the resulting credentials in its own auth store under `~/.bitidea/auth.json` and can import existing Codex CLI credentials from `~/.codex/auth.json` when present. No Codex CLI installation is required.
:::
...
Paid Nous Portal subscribers also get access to the **[Tool Gateway](/docs/user-guide/features/tool-gateway)** — web search, image generation, TTS, and browser automation routed through your subscription. No extra API keys needed. It's offered automatically during `bitidea model` setup, or enable it later with `bitidea tools`.
:::

### Two Commands for Model Management

Bitidea has **two** model commands that serve different purposes:

| Command | Where to run | What it does |
|---------|-------------|--------------|
| **`bitidea model`** | Your terminal (outside any session) | Full setup wizard — add providers, run OAuth, enter API keys, configure endpoints |
| **`/model`** | Inside a Bitidea chat session | Quick switch between **already-configured** providers and models |

If you're trying to switch to a provider you haven't set up yet (e.g. you only have OpenRouter configured and want to use Anthropic), you need `bitidea model`, not `/model`. Exit your session first (`Ctrl+C` or `/quit`), run `bitidea model`, complete the provider setup, then start a new session.
...
bitidea chat --provider anthropic --model claude-sonnet-4-6

# Preferred: authenticate through `bitidea model`
# Bitidea will use Claude Code's credential store directly when available
bitidea model

# Manual override with a setup-token (fallback / legacy)
export ANTHROPIC_TOKEN=***  # setup-token or manual OAuth token
bitidea chat --provider anthropic

# Auto-detect Claude Code credentials (if you already use Claude Code)
bitidea chat --provider anthropic  # reads Claude Code credential files automatically
```

When you choose Anthropic OAuth through `bitidea model`, Bitidea prefers Claude Code's own credential store over copying the token into `~/.bitidea/.env`. That keeps refreshable Claude credentials refreshable.
...
Bitidea supports GitHub Copilot as a first-class provider with two modes:
...
```bash
bitidea chat --provider copilot --model gpt-5.4
```
...
If no token is found, `bitidea model` offers an **OAuth device code login** — the same flow used by the Copilot CLI and opencode.
...
|------|--------|------------|
| OAuth token | `gho_` | `bitidea model` → GitHub Copilot → Login with GitHub |
...
If your `gh auth token` returns a `ghp_*` token, use `bitidea model` to authenticate via OAuth instead.
:::
...
```bash
bitidea chat --provider copilot-acp --model copilot-acp
...
| `BITIDEA_COPILOT_ACP_COMMAND` | Override the Copilot CLI binary path (default: `copilot`) |
| `BITIDEA_COPILOT_ACP_ARGS` | Override ACP args (default: `--acp --stdio`) |
...
```bash
# z.ai / ZhipuAI GLM
bitidea chat --provider zai --model glm-5
# Requires: GLM_API_KEY in ~/.bitidea/.env

# Kimi / Moonshot AI (international: api.moonshot.ai)
bitidea chat --provider kimi-coding --model kimi-for-coding
# Requires: KIMI_API_KEY in ~/.bitidea/.env

# Kimi / Moonshot AI (China: api.moonshot.cn)
bitidea chat --provider kimi-coding-cn --model kimi-k2.5
# Requires: KIMI_CN_API_KEY in ~/.bitidea/.env

# MiniMax (global endpoint)
bitidea chat --provider minimax --model MiniMax-M2.7
# Requires: MINIMAX_API_KEY in ~/.bitidea/.env

# MiniMax (China endpoint)
bitidea chat --provider minimax-cn --model MiniMax-M2.7
# Requires: MINIMAX_CN_API_KEY in ~/.bitidea/.env

# Alibaba Cloud / DashScope (Qwen models)
bitidea chat --provider alibaba --model qwen3.5-plus
# Requires: DASHSCOPE_API_KEY in ~/.bitidea/.env

# Xiaomi MiMo
bitidea chat --provider xiaomi --model mimo-v2-pro
# Requires: XIAOMI_API_KEY in ~/.bitidea/.env

# Arcee AI (Trinity models)
bitidea chat --provider arcee --model trinity-large-thinking
# Requires: ARCEEAI_API_KEY in ~/.bitidea/.env
```
...
When using the Z.AI / GLM provider, Bitidea automatically probes multiple endpoints (global, China, coding variants) to find one that accepts your API key. You don't need to set `GLM_BASE_URL` manually — the working endpoint is detected and cached automatically.
:::

### xAI (Grok) Prompt Caching

When using xAI as a provider (any base URL containing `x.ai`), Bitidea automatically enables prompt caching by sending the `x-grok-conv-id` header with every API request. This routes requests to the same server within a conversation session, allowing xAI's infrastructure to reuse cached system prompts and conversation history.
...
bitidea chat --provider huggingface --model Qwen/Qwen3-235B-A22B-Thinking-2507
# Requires: HF_TOKEN in ~/.bitidea/.env

# Short alias
bitidea chat --provider hf --model deepseek-ai/DeepSeek-V3.2
```
...
Bitidea Agent works with **any OpenAI-compatible API endpoint**. If a server implements `/v1/chat/completions`, you can point Bitidea at it. This means you can use local models, GPU inference servers, multi-provider routers, or any third-party API.
...
`OPENAI_BASE_URL` and `LLM_MODEL` in `.env` are **removed**. Neither is read by any part of Bitidea — `config.yaml` is the single source of truth for model and endpoint configuration. If you have stale entries in your `.env`, they are automatically cleared on the next `bitidea setup` or config migration. Use `bitidea model` or edit `config.yaml` directly.
:::
...
:::warning bitidea model vs /model
**`bitidea model`** (run from your terminal, outside any chat session) is the **full provider setup wizard**. Use it to add new providers, run OAuth flows, enter API keys, and configure custom endpoints.

**`/model`** (typed inside an active Bitidea chat session) can only **switch between providers and models you've already set up**. It cannot add new providers, run OAuth, or prompt for API keys. If you've only configured one provider (e.g. OpenRouter), `/model` will only show models for that provider.

**To add a new provider:** Exit your session (`Ctrl+C` or `/quit`), run `bitidea model`, set up the new provider, then start a new session.
:::
...
When switching providers, Bitidea persists the base URL and provider to config so the change survives restarts. When switching away from a custom endpoint to a built-in provider, the stale base URL is automatically cleared.
...
**You cannot set context length through the OpenAI-compatible API** (`/v1/chat/completions`). It must be configured server-side or via a Modelfile. This is the #1 source of confusion when integrating Ollama with tools like Bitidea.
:::
...
| Flag | Purpose |
|------|---------|
| `--enable-auto-tool-choice` | Required for `tool_choice: "auto"` (the default in Bitidea) |
| `--tool-call-parser <name>` | Parser for the model's tool call format |

Supported parsers: `bitidea` (Qwen 2.5, Hermes 2/3), `llama3_json` (Llama 3.x), `mistral`, `deepseek_v3`, `deepseek_v31`, `xlam`, `pythonic`. Without these flags, tool calls won't work — the model will output tool calls as text.
...
Then configure Bitidea to point at it:
...
Without `--jinja`, llama-server ignores the `tools` parameter entirely. The model will try to call tools by writing JSON in its response text, but Bitidea won't recognize it as a tool call — you'll see raw JSON like `{"name": "web_search", ...}` printed as a message instead of an actual search.
...
**Tool calling:** Supported since LM Studio 0.3.6. Models with native tool-calling training (Qwen 2.5, Llama 3.x, Mistral, Bitidea) are auto-detected and shown with a tool badge. Other models use a generic fallback that may be less reliable.

---

### WSL2 Networking (Windows Users)

Since Bitidea Agent requires a Unix environment, Windows users run it inside WSL2. If your model server (Ollama, LM Studio, etc.) runs on the **Windows host**, you need to bridge the network gap — WSL2 uses a virtual network adapter with its own subnet, so `localhost` inside WSL2 refers to the Linux VM, **not** the Windows host.
...
Use that IP in your Bitidea config:
...
If you get a JSON response listing your models, you're good. Use that same URL as the `base_url` in your Bitidea config.

---

### Troubleshooting Local Models

These issues affect **all** local inference servers when used with Bitidea.

#### "Connection refused" from WSL2 to a Windows-hosted model server

If you're running Bitidea inside WSL2 and your model server on the Windows host, `http://localhost:<port>` won't work in WSL2's default NAT networking mode. See [WSL2 Networking](#wsl2-networking-windows-users) above for the fix.
...
| **vLLM** | Add `--enable-auto-tool-choice --tool-call-parser bitidea` |
...
**Cause:** Context window is too small. When the conversation exceeds the context limit, most servers silently drop older messages. Bitidea's system prompt + tool schemas alone can use 4k–8k tokens.

**Diagnosis:**

```bash
# Check what Bitidea thinks the context is
...
Bitidea auto-detects context length from your server's `/v1/models` endpoint. If the server reports a low value (or doesn't report one at all), Bitidea uses the model's declared limit which may be wrong.
...
**Possible causes:**
1. **Low output cap (`max_tokens`) on the server** — SGLang defaults to 128 tokens per response. Set `--default-max-tokens` on the server or configure Bitidea with `model.max_tokens` in config.yaml. Note: `max_tokens` controls response length only — it is unrelated to how long your conversation history can be (that is `context_length`).
2. **Context exhaustion** — The model filled its context window. Increase `model.context_length` or enable [context compression](/docs/user-guide/configuration#context-compression) in Bitidea.
...
Then configure Bitidea with `bitidea model` → Custom endpoint → `http://localhost:4000/v1`.
...
Then configure Bitidea with `bitidea model` → Custom endpoint → `http://localhost:8402/v1` → model name `blockrun/auto`.
...
Configure any of these with `bitidea model` → Custom endpoint, or in `config.yaml`:
...
**`context_length`** is the **total context window** — the combined budget for input *and* output tokens (e.g. 200,000 for Claude Opus 4.6). Bitidea uses this to decide when to compress history and to validate API requests.
...
Bitidea uses a multi-source resolution chain to detect the correct context window for your model and provider:
...
`bitidea model` will prompt for context length when configuring a custom endpoint. Leave it blank for auto-detection.
...
custom_providers:
  - name: local
    base_url: http://localhost:8080/v1
    # api_key omitted — Bitidea uses "no-key-required" for keyless local servers
  - name: work
...
You can also select named custom providers from the interactive `bitidea model` menu.
...
:::tip
You can switch between providers at any time with `bitidea model` — no restart required. Your conversation history, memory, and skills carry over regardless of which provider you use.
:::
...
2. Point Bitidea at your instance (no API key needed):
   ```bash
   bitidea config set FIRECRAWL_API_URL http://localhost:3002
   ```
...
When using OpenRouter, you can control how requests are routed across providers. Add a `provider_routing` section to `~/.bitidea/config.yaml`:
...
Configure a backup provider:model that Bitidea switches to automatically when your primary model fails (rate limits, server errors, auth failures):
...
Optional cheap-vs-strong routing lets Bitidea keep your main model for complex work while sending very short/simple turns to a cheaper model.
...
How it works:
- If a turn is short, single-line, and does not look code/tool/debug heavy, Bitidea may route it to `cheap_model`
- If the turn looks complex, Bitidea stays on your primary model/provider
- If the cheap route cannot be resolved cleanly, Bitidea falls back to the primary model automatically
