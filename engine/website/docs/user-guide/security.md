...
Bitidea Agent is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, Bitidea checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.bitidea/config.yaml`:
...
1. **CLI flag**: Start a session with `bitidea --yolo` or `bitidea chat --yolo`
2. **Slash command**: Type `/yolo` during a session to toggle it on/off
3. **Environment variable**: Set `BITIDEA_YOLO_MODE=1`
...
YOLO mode is available in both CLI and gateway sessions. Internally, it sets the `BITIDEA_YOLO_MODE` environment variable which is checked before every command execution.
...
Configure the timeout in `~/.bitidea/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.bitidea/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.bitidea/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` bitidea/gateway | Self-termination prevention |
...
The `BITIDEA_EXEC_ASK=1` environment variable is automatically set when running the gateway.

### Permanent Allowlist

Commands approved with "always" are saved to `~/.bitidea/config.yaml`:
...
:::tip
Use `bitidea config edit` to review or remove patterns from your permanent allowlist.
:::

## User Authorization (Gateway)

When running the messaging gateway, Bitidea controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.bitidea/.env`:
...
For more flexible authorization, Bitidea includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `bitidea pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.bitidea/config.yaml`:
...
bitidea pairing list

# Approve a pairing code
bitidea pairing approve telegram ABC12DEF

# Revoke a user's access
bitidea pairing revoke telegram 123456789

# Clear all pending codes
bitidea pairing clear-pending
```

**Storage:** Pairing data is stored in `~/.bitidea/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, Bitidea applies strict security hardening to every container.
...
Container resources are configurable in `~/.bitidea/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.bitidea/sandboxes/docker/<task_id>/`
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` under the active profile's `BITIDEA_HOME`. Skills declare these in frontmatter:
...
When loaded, Bitidea checks if these files exist in the active profile's `BITIDEA_HOME` and registers them for mounting:
...
Paths are relative to `~/.bitidea/`. Files are mounted to `/root/.bitidea/` inside the container.
...
| **terminal** (local) | Blocks explicit Bitidea infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- Bitidea infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
Bitidea integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
4. **Store secrets securely** — keep API keys in `~/.bitidea/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.bitidea/logs/` for unauthorized access attempts
10. **Keep updated** — run `bitidea update` regularly for security patches

### Securing API Keys

```bash
# Set proper permissions on the .env file
chmod 600 ~/.bitidea/.env
