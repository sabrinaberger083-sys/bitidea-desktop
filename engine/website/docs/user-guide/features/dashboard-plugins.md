...
description: "Build custom tabs and extensions for the Bitidea web dashboard"
---

# Dashboard Plugins

Dashboard plugins let you add custom tabs to the web dashboard. A plugin can display its own UI, call the Bitidea API, and optionally register backend endpoints — all without touching the dashboard source code.
...
```bash
mkdir -p ~/.bitidea/plugins/my-plugin/dashboard/dist
```
...
```javascript
(function () {
  var SDK = window.__BITIDEA_PLUGIN_SDK__;
  var React = SDK.React;
  ...
  window.__BITIDEA_PLUGINS__.register("my-plugin", MyPage);
})();
```
...
Plugins live inside the standard `~/.bitidea/plugins/` directory. The dashboard extension is a `dashboard/` subfolder:
...
Plugins don't bundle React or UI components — they use the SDK exposed on `window.__BITIDEA_PLUGIN_SDK__`. This avoids version conflicts and keeps plugin bundles tiny.
...
// API
SDK.api                // Bitidea API client (getStatus, getSessions, etc.)
...
The `SDK.api` object has methods for all built-in Bitidea endpoints:
...
### Accessing Bitidea Internals

Backend routes can import from the bitidea-agent codebase:
...
1. Dashboard loads — `main.tsx` exposes the SDK on `window.__BITIDEA_PLUGIN_SDK__`
...
4. Plugin JS calls `window.__BITIDEA_PLUGINS__.register(name, Component)`
...
3. **Project plugins:** `./.bitidea/plugins/<name>/dashboard/manifest.json` (only when `BITIDEA_ENABLE_PROJECT_PLUGINS` is set)
...
- Registering via `window.__BITIDEA_PLUGINS__.register()`

To try it, run `bitidea dashboard` — the "Example" tab appears after Skills.
...
- **Test locally** — run `bitidea dashboard --no-open` and use browser dev tools to verify your plugin loads and registers correctly.
