...
description: "Customize Bitidea Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Bitidea Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `BITIDEA_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Bitidea is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Bitidea now seeds a default `SOUL.md` automatically in:

```text
~/.bitidea/SOUL.md
```

More precisely, it uses the current instance's `BITIDEA_HOME`, so if you run Bitidea with a custom home directory, it will use:

```text
$BITIDEA_HOME/SOUL.md
```

### Important behavior

- **SOUL.md is the agent's primary identity.** It occupies slot #1 in the system prompt, replacing the hardcoded default identity.
- Bitidea creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Bitidea loads `SOUL.md` only from `BITIDEA_HOME`
- Bitidea does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Bitidea falls back to a built-in default identity
...
If Bitidea loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `BITIDEA_HOME`, the personality belongs to the Bitidea instance itself.

That also makes it easier to teach users:
- "Edit `~/.bitidea/SOUL.md` to change Bitidea' default personality."
...
- how Bitidea should handle uncertainty, disagreement, or ambiguity
...
## What Bitidea injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Bitidea falls back to a built-in default identity ("You are Bitidea Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Bitidea ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Bitidea, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Bitidea its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.bitidea/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.bitidea/SOUL.md`
...
- [SOUL.md Guide](/docs/guides/use-soul-with-bitidea)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Bitidea speaks
- `display.skin` and `/skin` affect how Bitidea looks in the terminal
