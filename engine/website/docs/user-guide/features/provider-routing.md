...
When using [OpenRouter](https://openrouter.ai) as your LLM provider, Bitidea Agent supports **provider routing** — fine-grained control over which underlying AI providers handle your requests and how they're prioritized.
...
Add a `provider_routing` section to your `~/.bitidea/config.yaml`:
...
- **CLI mode** — configured in `~/.bitidea/config.yaml`, loaded at startup
