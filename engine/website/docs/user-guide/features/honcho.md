...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Bitidea's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Bitidea instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
bitidea memory setup    # select "honcho" from the provider list
```
...
Honcho is configured in `~/.honcho/config.json` (global) or `$BITIDEA_HOME/honcho.json` (profile-local). The setup wizard handles this for you.
...
- `per-session` — each `bitidea` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
```bash
bitidea honcho status          # Connection status, config, and key settings
bitidea honcho setup           # Interactive setup wizard
bitidea honcho strategy        # Show or set session strategy
bitidea honcho peer            # Update peer names for multi-agent setups
bitidea honcho mode            # Show or set recall mode
bitidea honcho tokens          # Show or set context token budget
bitidea honcho identity        # Show Honcho peer identity
bitidea honcho sync            # Sync host blocks for all profiles
bitidea honcho enable          # Enable Honcho
bitidea honcho disable         # Disable Honcho
```

## Migrating from `bitidea honcho`

If you previously used the standalone `bitidea honcho setup`:
...
No re-login or re-setup needed. Run `bitidea memory setup` and select "honcho" — the wizard detects your existing config.
