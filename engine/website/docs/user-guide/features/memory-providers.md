...
Bitidea Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
bitidea memory setup      # interactive picker + configuration
bitidea memory status     # check what's active
bitidea memory off        # disable external provider
```

You can also select the active memory provider via `bitidea plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.bitidea/config.yaml`:
...
When a memory provider is active, Bitidea automatically:
...
**Setup Wizard:**
```bash
bitidea honcho setup        # (legacy command) 
# or
bitidea memory setup        # select "honcho"
```

**Config:** `$BITIDEA_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$BITIDEA_HOME/honcho.json` > `~/.bitidea/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/bitidea-ai/bitidea-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/bitidea).
...
:::tip Migrating from `bitidea honcho`
If you previously used `bitidea honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-agent / Profiles:**

Each Bitidea profile gets its own Honcho AI peer while sharing the same workspace -- all profiles see the same user representation, but each agent builds its own identity and observations.

```bash
bitidea profile create coder --clone   # creates honcho peer "coder", inherits config from default
```

What `--clone` does: creates a `bitidea.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The peer is eagerly created in Honcho so it exists before first message.

For profiles created before Honcho was set up:

```bash
bitidea honcho sync   # scans all profiles, creates host blocks for any missing ones
```

This inherits settings from the default `bitidea` host block and creates new AI peers for each profile. Idempotent -- skips profiles that already have a host block.
...
See the [config reference](https://github.com/bitidea-ai/bitidea-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/bitidea).
...
bitidea memory setup    # select "openviking"
# Or manually:
bitidea config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.bitidea/.env
```
...
**Setup:**
```bash
bitidea memory setup    # select "mem0"
# Or manually:
bitidea config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.bitidea/.env
```

**Config:** `$BITIDEA_HOME/mem0.json`

| Key | Default | Description |
|-----|---------|-------------|
| `user_id` | `bitidea-user` | User identifier |
| `agent_id` | `bitidea` | Agent identifier |
...
**Setup:**
```bash
bitidea memory setup    # select "hindsight"
# Or manually:
bitidea config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.bitidea/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p bitidea ui start`

**Config:** `$BITIDEA_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `bitidea` | Memory bank identifier |
...
See [plugin README](https://github.com/sabrinaberger083-sys/bitidea-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
bitidea memory setup    # select "holographic"
# Or manually:
bitidea config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.bitidea-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$BITIDEA_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
bitidea memory setup    # select "retaindb"
# Or manually:
bitidea config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.bitidea/.env
```
...
# Then configure Bitidea
bitidea memory setup    # select "byterover"
# Or manually:
bitidea config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$BITIDEA_HOME/byterover/` (profile-scoped)
...
**Setup:**
```bash
bitidea memory setup    # select "supermemory"
# Or manually:
bitidea config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.bitidea/.env
```

**Config:** `$BITIDEA_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `bitidea` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `bitidea-{identity}` → `bitidea-coder`) to isolate memories per Bitidea profile
...
- **Local storage providers** (Holographic, ByteRover) use `$BITIDEA_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$BITIDEA_HOME/` so each profile has its own credentials
