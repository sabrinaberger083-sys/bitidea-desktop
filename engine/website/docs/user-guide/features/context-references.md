...
Type `@` followed by a reference to inject content directly into your message. Bitidea expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Bitidea env: `$BITIDEA_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$BITIDEA_HOME/skills/.hub/`
