...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Bitidea Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Bitidea Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Bitidea](/docs/guides/use-voice-mode-with-bitidea).

## Prerequisites

Before using voice features, make sure you have:

1. **Bitidea Agent installed** — `pip install bitidea-agent` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `bitidea model` or set your preferred provider credentials in `~/.bitidea/.env`
3. **A working base setup** — run `bitidea` to verify the agent responds to text before enabling voice

:::tip
The `~/.bitidea/` directory and default `config.yaml` are created automatically the first time you run `bitidea`. You only need to create `~/.bitidea/.env` manually for API keys.
:::
...
pip install "bitidea-agent[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "bitidea-agent[all]"
```
...
```bash
bitidea                # Start the interactive CLI
```
...
1. Start the CLI with `bitidea` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.bitidea/config.yaml` (default: `ctrl+b`).
:::
...
```bash
bitidea gateway        # Start the gateway (connects to configured platforms)
bitidea gateway setup  # Interactive setup wizard for first-time configuration
```
...
:::tip
To disable the mention requirement in server channels, add to `~/.bitidea/.env`:
```bash
...
```bash
bitidea gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.bitidea/.env`
...
- Review gateway logs: `tail -f ~/.bitidea/logs/gateway.log`
