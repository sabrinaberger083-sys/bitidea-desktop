...
description: "Overview of Bitidea Agent's tools — what's available, how toolsets work, and terminal backends"
---
...
Bitidea ships with a broad built-in tool registry covering web search, browser automation, terminal execution, file editing, memory, delegation, RL training, messaging delivery, Home Assistant, and more.
...
Paid [Nous Portal](https://portal.nousresearch.com) subscribers can use web search, image generation, TTS, and browser automation through the **[Tool Gateway](tool-gateway.md)** — no separate API keys needed. Run `bitidea model` to enable it, or configure individual tools with `bitidea tools`.
:::
...
bitidea chat --toolsets "web,terminal"
...
See [Toolsets Reference](/docs/reference/toolsets-reference) for the full set, including platform presets such as `bitidea-cli`, `bitidea-telegram`, and dynamic MCP toolsets like `mcp-<server>`.
...
```
```bash
# Set credentials in ~/.bitidea/.env
...
# Configure
bitidea config set terminal.backend singularity
bitidea config set terminal.singularity_image ~/python.sif
```
...
modal setup
bitidea config set terminal.backend modal
```
...
If a command needs sudo, you'll be prompted for your password (cached for the session). Or set `SUDO_PASSWORD` in `~/.bitidea/.env`.

:::warning
On messaging platforms, if sudo fails, the output includes a tip to add `SUDO_PASSWORD` to `~/.bitidea/.env`.
:::
