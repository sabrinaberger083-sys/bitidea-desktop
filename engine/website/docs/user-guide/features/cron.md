...
Schedule tasks to run automatically with natural language or cron expressions. Bitidea exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
:::warning
Cron-run sessions cannot recursively create more cron jobs. Bitidea disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
bitidea cron create "every 2h" "Check server status"
bitidea cron create "every 1h" "Summarize new feed items" --skill blogwatcher
bitidea cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill find-nearby \
  --name "Skill combo"
```
...
Ask Bitidea normally:

```text
Every morning at 9am, check Hacker News for AI news and send me a summary on Telegram.
```

Bitidea will use the unified `cronjob` tool internally.
...
```bash
bitidea cron edit <job_id> --schedule "every 4h"
bitidea cron edit <job_id> --prompt "Use the revised task"
bitidea cron edit <job_id> --skill blogwatcher --skill find-nearby
bitidea cron edit <job_id> --add-skill find-nearby
bitidea cron edit <job_id> --remove-skill blogwatcher
bitidea cron edit <job_id> --clear-skills
```
...
```bash
bitidea cron list
bitidea cron pause <job_id>
bitidea cron resume <job_id>
bitidea cron run <job_id>
bitidea cron remove <job_id>
bitidea cron status
bitidea cron tick
```
...
sudo bitidea gateway install --system   # Linux: boot-time system service for servers
bitidea gateway             # Or run in foreground
...
On each tick Bitidea:

1. loads jobs from `~/.bitidea/cron/jobs.json`
...
A file lock at `~/.bitidea/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
| `"local"` | Save to local files only (`~/.bitidea/cron/output/`) | Default on CLI |
...
If the agent's final response starts with `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.bitidea/cron/output/`), but no message is sent to the delivery target.
...
Or set the `BITIDEA_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 120s default.
...
The agent's final response is automatically delivered — you do **not** need to include `send_message` in the cron prompt for that same destination. If a cron run calls `send_message` to the exact target the scheduler will already deliver to, Bitidea skips that duplicate send and tells the model to put the user-facing content in the final response instead. Use `send_message` only for additional or different targets.
...
Jobs are stored in `~/.bitidea/cron/jobs.json`. Output from job runs is saved to `~/.bitidea/cron/output/{job_id}/{timestamp}.md`.
