...
title: "Plugins"
description: "Extend Bitidea with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

Bitidea has a plugin system for adding custom tools, hooks, and integrations without modifying core code.

**→ [Build a Bitidea Plugin](/docs/guides/build-a-bitidea-plugin)** — step-by-step guide with a complete working example.

## Quick overview

Drop a directory into `~/.bitidea/plugins/` with a `plugin.yaml` and Python code:
...
Start Bitidea — your tools appear alongside built-in tools. The model can call them immediately.

### Minimal working example

Here is a complete plugin that adds a `hello_world` tool and logs every tool call via a hook.

**`~/.bitidea/plugins/hello-world/plugin.yaml`**
...
**`~/.bitidea/plugins/hello-world/__init__.py`**

```python
"""Minimal Bitidea plugin — registers a tool and a hook."""
...
Drop both files into `~/.bitidea/plugins/hello-world/`, restart Bitidea, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

Project-local plugins under `./.bitidea/plugins/` are disabled by default. Enable them only for trusted repositories by setting `BITIDEA_ENABLE_PROJECT_PLUGINS=true` before starting Bitidea.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `bitidea <plugin> <subcommand>` |
| Inject messages | `ctx.inject_message(content, role="user")` — see [Injecting Messages](#injecting-messages) |
| Ship data files | `Path(__file__).parent / "data" / "file.yaml"` |
| Bundle skills | `ctx.register_skill(name, path)` — namespaced as `plugin:skill`, loaded via `skill_view("plugin:skill")` |
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `bitidea plugins install` |
| Distribute via pip | `[project.entry-points."bitidea_agent.plugins"]` |

## Plugin discovery

| Source | Path | Use case |
|--------|------|----------|
| User | `~/.bitidea/plugins/` | Personal plugins |
| Project | `.bitidea/plugins/` | Project-specific plugins (requires `BITIDEA_ENABLE_PROJECT_PLUGINS=true`) |
| pip | `bitidea_agent.plugins` entry_points | Distributed packages |
...
Bitidea has three kinds of plugins:

| Type | What it does | Selection | Location |
|------|-------------|-----------|----------|
| **General plugins** | Add tools, hooks, slash commands, CLI commands | Multi-select (enable/disable) | `~/.bitidea/plugins/` |
...
```bash
bitidea plugins                  # unified interactive UI
bitidea plugins list             # table view with enabled/disabled status
bitidea plugins install user/repo  # install from Git
bitidea plugins update my-plugin   # pull latest
bitidea plugins remove my-plugin   # uninstall
bitidea plugins enable my-plugin   # re-enable a disabled plugin
bitidea plugins disable my-plugin  # disable without removing
```

### Interactive UI

Running `bitidea plugins` with no arguments opens a composite interactive screen:
...
See the **[full guide](/docs/guides/build-a-bitidea-plugin)** for handler contracts, schema format, hook behavior, error handling, and common mistakes.
