...
Bitidea Agent can generate images from text prompts using FAL.ai's **FLUX 2 Pro** model with automatic 2x upscaling via the **Clarity Upscaler** for enhanced quality.

## Setup

:::tip Nous Subscribers
If you have a paid [Nous Portal](https://portal.nousresearch.com) subscription, you can use image generation through the **[Tool Gateway](tool-gateway.md)** without a FAL API key. Run `bitidea model` or `bitidea tools` to enable it.
:::
...
When you ask Bitidea to generate an image:
...
Simply ask Bitidea to create an image:
