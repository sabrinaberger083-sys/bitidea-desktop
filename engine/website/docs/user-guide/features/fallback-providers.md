...
Bitidea Agent has three layers of resilience that keep your sessions running when providers hit issues:
...
When your main LLM provider encounters errors — rate limits, server overload, auth failures, connection drops — Bitidea can automatically switch to a backup provider:model pair mid-session without losing your conversation.

### Configuration

Add a `fallback_model` section to `~/.bitidea/config.yaml`:
...
| Nous Portal | `nous` | `bitidea auth` (OAuth) |
| OpenAI Codex | `openai-codex` | `bitidea model` (ChatGPT OAuth) |
...
Bitidea uses separate lightweight models for side tasks. Each task has its own provider resolution chain that acts as a built-in fallback system.
...
When a task's provider is set to `"auto"` (the default), Bitidea tries providers in order until one works:
...
If the resolved provider fails at call time, Bitidea also has an internal retry: if the provider is not OpenRouter and no explicit `base_url` is set, it tries OpenRouter as a last-resort fallback.
...
| `"codex"` | Force Codex OAuth | `bitidea model` → Codex |
...
`base_url` takes precedence over `provider`. Bitidea uses the configured `api_key` for authentication, falling back to `OPENAI_API_KEY` if not set. It does **not** reuse `OPENROUTER_API_KEY` for custom endpoints.
...
If no provider is available for compression, Bitidea drops middle conversation turns without generating a summary rather than failing the session.
