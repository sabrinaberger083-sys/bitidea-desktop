...
sidebar_position: 2
title: "Configuration"
description: "Configure Bitidea Agent — config.yaml, providers, models, API keys, and more"
---

# Configuration

All settings are stored in the `~/.bitidea/` directory for easy access.
...
```bash
bitidea config              # View current configuration
bitidea config edit         # Open config.yaml in your editor
bitidea config set KEY VAL  # Set a specific value
bitidea config check        # Check for missing options (after updates)
bitidea config migrate      # Interactively add missing options

# Examples:
bitidea config set model anthropic/claude-opus-4
bitidea config set terminal.backend docker
bitidea config set OPENROUTER_API_KEY sk-or-...  # Saves to .env
```

:::tip
The `bitidea config set` command automatically routes values to the right file — API keys are saved to `.env`, everything else to `config.yaml`.
:::
...
1. **CLI arguments** — e.g., `bitidea chat --model anthropic/claude-sonnet-4` (per-invocation override)
2. **`~/.bitidea/config.yaml`** — the primary config file for all non-secret settings
3. **`~/.bitidea/.env`** — fallback for env vars; **required** for secrets (API keys, tokens, passwords)
...
Bitidea supports six terminal backends. Each determines where the agent's shell commands actually execute — your local machine, a Docker container, a remote server via SSH, a Modal cloud sandbox, a Daytona workspace, or a Singularity/Apptainer container.
...
For cloud sandboxes such as Modal and Daytona, `container_persistent: true` means Bitidea will try to preserve filesystem state across sandbox recreation. It does not promise that the same live sandbox, PID space, or background processes will still be running later.
...
:::warning
The agent has the same filesystem access as your user account. Use `bitidea tools` to disable tools you don't want, or switch to Docker for sandboxing.
:::
...
**Requirements:** Docker Desktop or Docker Engine installed and running. Bitidea probes `$PATH` plus common macOS install locations (`/usr/local/bin/docker`, `/opt/homebrew/bin/docker`, Docker Desktop app bundle).
...
**Credential forwarding:** Env vars listed in `docker_forward_env` are resolved from your shell environment first, then `~/.bitidea/.env`. Skills can also declare `required_environment_variables` which are merged automatically.
...
**Persistence:** When enabled, the sandbox filesystem is snapshotted on cleanup and restored on next session. Snapshots are tracked in `~/.bitidea/modal_snapshots.json`. This preserves filesystem state, not live processes, PID space, or background jobs.

**Credential files:** Automatically mounted from `~/.bitidea/` (OAuth tokens, etc.) and synced before each command.
...
**Persistence:** When enabled, sandboxes are stopped (not deleted) on cleanup and resumed on next session. Sandbox names follow the pattern `bitidea-{task_id}`.
...
**Scratch directory:** Resolved in order: `TERMINAL_SCRATCH_DIR` → `TERMINAL_SANDBOX_DIR/singularity` → `/scratch/$USER/bitidea-agent` (HPC convention) → `~/.bitidea/sandboxes/singularity`.
...
- **Docker** — Run `docker version` to verify Docker is working. If it fails, fix Docker or `bitidea config set terminal.backend local`.
- **SSH** — Both `TERMINAL_SSH_HOST` and `TERMINAL_SSH_USER` must be set. Bitidea logs a clear error if either is missing.
- **Modal** — Needs `MODAL_TOKEN_ID` env var or `~/.modal.toml`. Run `bitidea doctor` to check.
...
Bitidea resolves each listed variable from your current shell first, then falls back to `~/.bitidea/.env` if it was saved with `bitidea config set`.
...
Docker sandboxes stay isolated by default. Bitidea does **not** pass your current host working directory into the container unless you explicitly opt in.
...
When enabled:
- if you launch Bitidea from `~/projects/my-app`, that host directory is bind-mounted to `/workspace`
...
- `true` gives the sandbox direct access to the directory you launched Bitidea from
...
```bash
bitidea config set terminal.persistent_shell false
```
...
- `bitidea config migrate` scans all enabled skills, finds unconfigured settings, and offers to prompt you
- `bitidea config show` displays all skill settings under "Skill Settings" with the skill they belong to
- When a skill loads, its resolved config values are injected into the skill context automatically

**Setting values manually:**

```bash
bitidea config set skills.config.wiki.path ~/my-research-wiki
```
...
```yaml
worktree: true    # Always create a worktree (same as bitidea -w)
...
Bitidea automatically compresses long conversations to stay within your model's context window. The compression summarizer is a separate LLM call — you can point it at any provider or endpoint.
...
Plugin engines are **never auto-activated** — you must explicitly set `context.engine` to the plugin name. Available engines can be browsed and selected via `bitidea plugins` → Provider Plugins → Context Engine.
...
|---------|---------|----------------|---------|
| Socket read timeout | 120s | Auto-raised to 1800s | `BITIDEA_STREAM_READ_TIMEOUT` |
| Stale stream detection | 180s | Auto-disabled | `BITIDEA_STREAM_STALE_TIMEOUT` |
| API call (non-streaming) | 1800s | Unchanged | `BITIDEA_API_TIMEOUT` |

The **socket read timeout** controls how long httpx waits for the next chunk of data from the provider. Local LLMs can take minutes for prefill on large contexts before producing the first token, so Bitidea raises this to 30 minutes when it detects a local endpoint. If you explicitly set `BITIDEA_STREAM_READ_TIMEOUT`, that value is always used regardless of endpoint detection.
...
Bitidea uses lightweight "auxiliary" models for side tasks like image analysis, web page summarization, and browser screenshot analysis. By default, these use **Gemini Flash** via auto-detection — you don't need to configure anything.

### The universal config pattern

Every model slot in Bitidea — auxiliary tasks, compression, fallback — uses the same three knobs:
...
When `base_url` is set, Bitidea ignores the provider and calls that endpoint directly (using `api_key` or `OPENAI_API_KEY` for auth). When only `provider` is set, Bitidea uses that provider's built-in auth and base URL.
...
Or via environment variable (in `~/.bitidea/.env`):
...
| `"nous"` | Force Nous Portal | `bitidea auth` |
| `"codex"` | Force Codex OAuth (ChatGPT account). Supports vision (gpt-5.3-codex). | `bitidea model` → Codex |
| `"main"` | Use your active custom/main endpoint. This can come from `OPENAI_BASE_URL` + `OPENAI_API_KEY` or from a custom endpoint saved via `bitidea model` / `config.yaml`. Works with OpenAI, local models, or any OpenAI-compatible API. **Auxiliary tasks only — not valid for `model.provider`.** | Custom endpoint credentials + base URL |
...
`base_url` takes precedence over `provider`, so this is the most explicit way to route an auxiliary task to a specific endpoint. For direct endpoint overrides, Bitidea uses the configured `api_key` or falls back to `OPENAI_API_KEY`; it does not reuse `OPENROUTER_API_KEY` for that custom endpoint.
...
`provider: "main"` uses whatever provider Bitidea uses for normal chat — whether that's a named custom provider (e.g. `beans`), a built-in provider like `openrouter`, or a legacy `OPENAI_BASE_URL` endpoint.
...
:::tip
Run `bitidea config` to see your current auxiliary model settings. Overrides only show up when they differ from the defaults.
:::
...
`interim_assistant_messages` is gateway-only. When enabled, Bitidea sends completed mid-turn assistant updates as separate chat messages. This is independent from `tool_progress` and does not require gateway streaming.
...
If the requested provider is unavailable, Bitidea falls back automatically in this order: `local` → `groq` → `openai`.
...
:::note
Streaming is disabled by default. Enable it in `~/.bitidea/config.yaml` to try the streaming UX.
:::
...
- `false` reverts to the old shared-room behavior. That can be useful if you explicitly want Bitidea to treat a channel like one collaborative conversation, but it also means users share context, token costs, and interrupt state.
- Direct messages are unaffected. Bitidea still keys DMs by chat/DM ID as usual.
...
Control what Bitidea does when an unknown user sends a direct message:
...
- `pair` is the default. Bitidea denies access, but replies with a one-time pairing code in DMs.
...
quick_commands:
  status:
    type: exec
    command: systemctl status bitidea-agent
  disk:
    type: exec
    command: df -h /
  update:
    type: exec
    command: cd ~/.bitidea/bitidea-agent && git pull && pip install -e .
  gpu:
    type: exec
...
The `web_search`, `web_extract`, and `web_crawl` tools support four backend providers. Configure the backend in `config.yaml` or via `bitidea tools`:
...
browser:
  ...
  record_sessions: false         # Auto-record browser sessions as WebM videos to ~/.bitidea/browser_recordings/
  camofox:
...
Control how Bitidea handles potentially dangerous commands:
...
| `off` | Skip all approval checks. Equivalent to `BITIDEA_YOLO_MODE=true`. **Use with caution.** |
...
```yaml
checkpoints:
  enabled: true                  # Enable automatic checkpoints (also: bitidea --checkpoints)
...
**Direct endpoint override:** If you want the obvious custom-endpoint path, set `delegation.base_url`, `delegation.api_key`, and `delegation.model`. That sends subagents directly to that OpenAI-compatible endpoint and takes precedence over `delegation.provider`. If `delegation.api_key` is omitted, Bitidea falls back to `OPENAI_API_KEY` only.
...
Bitidea uses two different context scopes:

| File | Purpose | Scope |
|------|---------|-------|
| `SOUL.md` | **Primary agent identity** — defines who the agent is (slot #1 in the system prompt) | `~/.bitidea/SOUL.md` or `$BITIDEA_HOME/SOUL.md` |
| `.bitidea.md` / `HERMES.md` | Project-specific instructions (highest priority) | Walks to git root |
...
- If SOUL.md is missing, empty, or cannot be loaded, Bitidea falls back to a built-in default identity.
- **Project context files use a priority system** — only ONE type is loaded (first match wins): `.bitidea.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`. SOUL.md is always loaded independently.
- **AGENTS.md** is hierarchical: if subdirectories also have AGENTS.md, all are combined.
- Bitidea automatically seeds a default `SOUL.md` if one does not already exist.
...
| Context | Default |
|---------|---------|
| **CLI (`bitidea`)** | Current directory where you run the command |
| **Messaging gateway** | Home directory `~` (override with `MESSAGING_CWD`) |
| **Docker / Singularity / Modal / SSH** | User's home directory inside the container or remote machine |

Override the working directory:
```bash
# In ~/.bitidea/.env or ~/.bitidea/config.yaml:
