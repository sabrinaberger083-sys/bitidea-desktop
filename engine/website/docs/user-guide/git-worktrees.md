...
title: "Git Worktrees"
description: "Run multiple Bitidea agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Bitidea Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Bitidea so each session has a clean, isolated working directory.

## Why Use Worktrees with Bitidea?

Bitidea treats the **current working directory** as the project root:

- CLI: the directory where you run `bitidea` or `bitidea chat`
...
git worktree add ../repo-feature feature/bitidea-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/bitidea-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Bitidea there:

```bash
cd ../repo-feature

# Start Bitidea in the worktree
bitidea
```
...
git worktree add ../repo-experiment-a feature/bitidea-a
git worktree add ../repo-experiment-b feature/bitidea-b
```
...
Each Bitidea process:

- Works on its own branch (`feature/bitidea-a` vs `feature/bitidea-b`).
...
- Bitidea checkpoint data under `~/.bitidea/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Bitidea experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/bitidea-checkpoints-docs`, `feature/bitidea-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Bitidea from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `bitidea -w` (Automatic Worktree Mode)

Bitidea has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `bitidea/bitidea-<hash>`).
...
```bash
bitidea -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `bitidea -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Bitidea session its own clean checkout.
