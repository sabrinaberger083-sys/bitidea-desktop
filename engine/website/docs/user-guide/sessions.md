...
Bitidea Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.bitidea/state.db`) — structured session metadata with FTS5 full-text search
2. **JSONL transcripts** (`~/.bitidea/sessions/`) — raw conversation transcripts including tool calls (gateway)
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`bitidea` or `bitidea chat`) |
...
bitidea -c "my project"

# If there are lineage variants (my project, my project #2, my project #3),
# this automatically resumes the most recent one
bitidea -c "my project"   # → resumes "my project #3"
```
...
bitidea --resume 20250305_091523_a1b2c3d4
bitidea -r 20250305_091523_a1b2c3d4

# Resume by title
bitidea --resume "refactoring auth"

# Or with the chat subcommand
bitidea chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `bitidea sessions list`.

### Conversation Recap on Resume

When you resume a session, Bitidea displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Bitidea session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.bitidea/config.yaml`:
...
Bitidea automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `bitidea sessions list` or `bitidea sessions browse`.
...
```bash
bitidea sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Bitidea creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`bitidea -c "my project"`), it automatically picks the most recent session in the lineage.
...
Bitidea provides a full set of session management commands via `bitidea sessions`:
...
# Filter by platform
bitidea sessions list --source telegram

# Show more sessions
bitidea sessions list --limit 50
```
...
bitidea sessions export backup.jsonl

# Export sessions from a specific platform
bitidea sessions export telegram-history.jsonl --source telegram

# Export a single session
bitidea sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
bitidea sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
bitidea sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
bitidea sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
bitidea sessions prune --older-than 30

# Only prune sessions from a specific platform
bitidea sessions prune --source telegram --older-than 60

# Skip confirmation
bitidea sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`bitidea insights`](/docs/reference/cli-commands#bitidea-insights).
...
When Bitidea cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Bitidea uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Bitidea in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.bitidea/state.db` | All session metadata + messages with FTS5 |
| Gateway transcripts | `~/.bitidea/sessions/` | JSONL transcripts per session + sessions.json index |
| Gateway index | `~/.bitidea/sessions/sessions.json` | Maps session keys to active session IDs |
...
bitidea sessions delete <session_id>
