...
exec(open(os.path.expanduser(
    "~/.bitidea/skills/red-teaming/godmode/scripts/load_godmode.py"
)).read())
...
The godmode skill integrates with two Bitidea Agent config mechanisms:

### Ephemeral System Prompt (`config.yaml`)

Set the jailbreak system prompt in `~/.bitidea/config.yaml`:
...
The `agent.system_prompt` is appended **after** Bitidea's own system prompt — it augments, not replaces.
...
Create `~/.bitidea/prefill.json` and reference it in config:
...
```bash
# Load the skill in a Bitidea session
/godmode

# Or via CLI one-shot
bitidea chat -q "jailbreak my current model"
...
bitidea chat -q "race models on: how does SQL injection work?"

# Apply Parseltongue encoding to a query
bitidea chat -q "parseltongue encode: how to hack into WiFi"
```
...
5. **For hard refusals, switch models** — When all techniques fail, ULTRAPLINIAN (racing multiple models) is the practical fallback. Bitidea models and Grok are typically least filtered.
...
4. **Bitidea models don't need jailbreaking** — `nousresearch/hermes-3-*` and `hermes-4-*` are already uncensored. Use them directly.
...
6. **Restart Bitidea after auto-jailbreak** — The CLI reads config once at startup. Gateway sessions pick up changes immediately.

7. **execute_code sandbox lacks env vars** — Load dotenv explicitly: `from dotenv import load_dotenv; load_dotenv(os.path.expanduser("~/.bitidea/.env"))`

8. **`boundary_inversion` is model-version specific** — Works on Claude 3.5 Sonnet but NOT Claude Sonnet 4 or Claude 4.6.

9. **Gray-area vs hard queries** — Jailbreak techniques work much better on dual-use queries (lock picking, security tools) than overtly harmful ones (phishing, malware). For hard queries, skip to ULTRAPLINIAN or use Bitidea/Grok.
