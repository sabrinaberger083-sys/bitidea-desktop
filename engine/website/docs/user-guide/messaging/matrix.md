...
sidebar_position: 9
title: "Matrix"
description: "Set up Bitidea Agent as a Matrix bot"
---

# Matrix Setup

Bitidea Agent integrates with Matrix, the open, federated messaging protocol. Matrix lets you run your own homeserver or use a public one like matrix.org — either way, you keep control of your communications. The bot connects via the `mautrix` Python SDK, processes messages through the Bitidea Agent pipeline (including tool use, memory, and reasoning), and responds in real time. It supports text, file attachments, images, audio, video, and optional end-to-end encryption (E2EE).

Bitidea works with any Matrix homeserver — Synapse, Conduit, Dendrite, or matrix.org.

Before setup, here's the part most people want to know: how Bitidea behaves once it's connected.

## How Bitidea Behaves

| Context | Behavior |
|---------|----------|
| **DMs** | Bitidea responds to every message. No `@mention` needed. Each DM has its own session. Set `MATRIX_DM_MENTION_THREADS=true` to start a thread when the bot is `@mentioned` in a DM. |
| **Rooms** | By default, Bitidea requires an `@mention` to respond. Set `MATRIX_REQUIRE_MENTION=false` or add room IDs to `MATRIX_FREE_RESPONSE_ROOMS` for free-response rooms. Room invites are auto-accepted. |
| **Threads** | Bitidea supports Matrix threads (MSC3440). If you reply in a thread, Bitidea keeps the thread context isolated from the main room timeline. Threads where the bot has already participated do not require a mention. |
| **Auto-threading** | By default, Bitidea auto-creates a thread for each message it responds to in a room. This keeps conversations isolated. Set `MATRIX_AUTO_THREAD=false` to disable. |
| **Shared rooms with multiple users** | By default, Bitidea isolates session history per user inside the room. Two people talking in the same room do not share one transcript unless you explicitly disable that. |
...
2. Choose a username like `bitidea` — the full user ID will be `@bitidea:your-server.org`.

### Option B: Use matrix.org or Another Public Homeserver

1. Go to [Element Web](https://app.element.io) and create a new account.
2. Pick a username for your bot (e.g., `bitidea-bot`).

### Option C: Use Your Own Account

You can also run Bitidea as your own user. This means the bot posts as you — useful for personal assistants.

## Step 2: Get an Access Token

Bitidea needs an access token to authenticate with the homeserver. You have two options:
...
Instead of providing an access token, you can give Bitidea the bot's user ID and password. Bitidea will log in automatically on startup. This is simpler but means the password is stored in your `.env` file.

```bash
MATRIX_USER_ID=@bitidea:your-server.org
MATRIX_PASSWORD=your-password
```

## Step 3: Find Your Matrix User ID

Bitidea Agent uses your Matrix User ID to control who can interact with the bot. Matrix User IDs follow the format `@username:server`.
...
## Step 4: Configure Bitidea Agent
...
Add the following to your `~/.bitidea/.env` file:
...
# MATRIX_USER_ID=@bitidea:matrix.example.org
...
Optional behavior settings in `~/.bitidea/config.yaml`:
...
:::tip
You can run `bitidea gateway` in the background or as a systemd service for persistent operation. See the deployment docs for details.
:::

## End-to-End Encryption (E2EE)

Bitidea supports Matrix end-to-end encryption, so you can chat with your bot in encrypted rooms.
...
# Or install with bitidea extras
pip install 'bitidea-agent[matrix]'
```
...
Add to your `~/.bitidea/.env`:

```bash
MATRIX_ENCRYPTION=true
```

When E2EE is enabled, Bitidea:

- Stores encryption keys in `~/.bitidea/platforms/matrix/store/` (legacy installs: `~/.bitidea/matrix/store/`)
...
On each startup, if `MATRIX_RECOVERY_KEY` is set, Bitidea imports cross-signing keys from the homeserver's secure secret storage and signs the current device. This is idempotent and safe to leave enabled permanently.

:::warning
If you delete the `~/.bitidea/platforms/matrix/store/` directory, the bot loses its encryption keys. You'll need to verify the device again in your Matrix client. Back up this directory if you want to preserve encrypted sessions.
:::
...
If you previously used Bitidea with `MATRIX_ENCRYPTION=true` and are upgrading to
...
1. **Generate a new access token** to get a fresh device ID. The simplest way:
   ...
   curl -X POST https://your-server/_matrix/client/v3/login \
     ...
     -d '{
       "type": "m.login.password",
       "identifier": {"type": "m.id.user", "user": "@bitidea:your-server.org"},
       "password": "***",
       "initial_device_display_name": "Bitidea Agent"
     }'
   ```

   Copy the new `access_token` and update `MATRIX_ACCESS_TOKEN` in `~/.bitidea/.env`.

2. **Delete old encryption state**:

   ```bash
   rm -f ~/.bitidea/platforms/matrix/store/crypto.db
   rm -f ~/.bitidea/platforms/matrix/store/crypto_store.*
   ```

3. **Set your recovery key** (if you use cross-signing — most Element users do). Add to `~/.bitidea/.env`:
...
a working E2EE setup with a previous version of Bitidea and are upgrading.
...
services:
  bitidea-matrix:
    ...
    environment:
      ...
      MATRIX_ENCRYPTION: "true"
      MATRIX_DEVICE_ID: "BITIDEA_BOT"

      # Proxy mode — forward to host agent
      GATEWAY_PROXY_URL: "http://192.168.1.100:8642"
      GATEWAY_PROXY_KEY: "your-secret-key-here"
    volumes:
      - ./matrix-store:/root/.bitidea/platforms/matrix/store
```
...
RUN pip install 'bitidea-agent[matrix]'
...
| Setting | Description |
|---------|-------------|
| `GATEWAY_PROXY_URL` | URL of the remote Bitidea API server (e.g., `http://192.168.1.100:8642`) |
...
:::tip
Session continuity is maintained via the `X-Bitidea-Session-Id` header. The host's API server tracks sessions by this ID, so conversations persist across messages just like they would with a local agent.
:::
...
**Fix**: The sync loop automatically retries every 5 seconds on error. Check the Bitidea logs for sync-related warnings. If the bot consistently falls behind, ensure your homeserver has adequate resources.

### Bot is offline

**Cause**: The Bitidea gateway isn't running, or it failed to connect.

**Fix**: Check that `bitidea gateway` is running. Look at the terminal output for error messages. Common issues: wrong homeserver URL, expired access token, homeserver unreachable.

### "User not allowed" / Bot ignores you

**Cause**: Your User ID isn't in `MATRIX_ALLOWED_USERS`.

**Fix**: Add your User ID to `MATRIX_ALLOWED_USERS` in `~/.bitidea/.env` and restart the gateway. Use the full `@user:server` format.
...
For more information on securing your Bitidea Agent deployment, see the [Security Guide](../security.md).
...
- **Media support**: Bitidea can send and receive images, audio, video, and file attachments. Media is uploaded to your homeserver using the Matrix content repository API.
