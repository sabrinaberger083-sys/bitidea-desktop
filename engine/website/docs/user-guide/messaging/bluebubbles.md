...
Connect Bitidea to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between Bitidea and the BlueBubbles server
...
Or set environment variables directly in `~/.bitidea/.env`:
...
When someone messages your iMessage, Bitidea automatically sends them a pairing code. Approve it with:
```bash
bitidea pairing approve bluebubbles <CODE>
```
Use `bitidea pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in `~/.bitidea/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.bitidea/.env`):
```bash
...
Bitidea will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → Bitidea
Bitidea → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** Bitidea sends messages via the BlueBubbles REST API.
...
### Chat Addressing
You can address chats by email or phone number — Bitidea resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `bitidea logs gateway` for webhook errors (or `bitidea logs -f` to follow in real-time)
