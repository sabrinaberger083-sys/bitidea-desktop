...
sidebar_position: 10
title: "DingTalk"
description: "Set up Bitidea Agent as a DingTalk chatbot"
---

# DingTalk Setup

Bitidea Agent integrates with DingTalk (钉钉) as a chatbot, letting you chat with your AI assistant through direct messages or group chats. The bot connects via DingTalk's Stream Mode — a long-lived WebSocket connection that requires no public URL or webhook server — and replies using markdown-formatted messages through DingTalk's session webhook API.

Before setup, here's the part most people want to know: how Bitidea behaves once it's in your DingTalk workspace.

## How Bitidea Behaves

| Context | Behavior |
|---------|----------|
| **DMs (1:1 chat)** | Bitidea responds to every message. No `@mention` needed. Each DM has its own session. |
| **Group chats** | Bitidea responds when you `@mention` it. Without a mention, Bitidea ignores the message. |
| **Shared groups with multiple users** | By default, Bitidea isolates session history per user inside the group. Two people talking in the same group do not share one transcript unless you explicitly disable that. |
...
Bitidea Agent uses your DingTalk User ID to control who can interact with the bot. DingTalk User IDs are alphanumeric strings set by your organization's admin.
...
**Fix**: Verify `DINGTALK_CLIENT_ID` and `DINGTALK_CLIENT_SECRET` are set correctly in `~/.bitidea/.env`. The Client ID is your AppKey, and the Client Secret is your AppSecret from the DingTalk Developer Console.
...
**Fix**: Check that `bitidea gateway` is running. Look at the terminal output for error messages. Common issues: wrong credentials, app deactivated, `dingtalk-stream` or `httpx` not installed.
