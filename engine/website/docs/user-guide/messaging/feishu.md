...
sidebar_position: 11
title: "Feishu / Lark"
description: "Set up Bitidea Agent as a Feishu or Lark bot"
---

# Feishu / Lark Setup

Bitidea Agent integrates with Feishu and Lark as a full-featured bot. Once connected, you can chat with the agent in direct messages or group chats, receive cron job results in a home chat, and send text, images, audio, and file attachments through the normal gateway flow.

The integration supports both connection modes:

- `websocket` — recommended; Bitidea opens the outbound connection and you do not need a public webhook endpoint
...
| Context | Behavior |
|---------|----------|
| Direct messages | Bitidea responds to every message. |
| Group chats | Bitidea responds only when the bot is @mentioned in the chat. |
...
Select **Feishu / Lark** and scan the QR code with your Feishu or Lark mobile app. Bitidea will automatically create a bot application with the correct permissions and save the credentials.
...
5. Run `bitidea gateway setup`, select **Feishu / Lark**, and enter the credentials when prompted.
...
Use WebSocket mode when Bitidea runs on your laptop, workstation, or a private server. No public URL is required. The official Lark SDK opens and maintains a persistent outbound WebSocket connection with automatic reconnection.
...
Use webhook mode only when you already run Bitidea behind a reachable HTTP endpoint.

```bash
FEISHU_CONNECTION_MODE=webhook
```

In webhook mode, Bitidea starts an HTTP server (via `aiohttp`) and serves a Feishu endpoint at:
...
The `FEISHU_GROUP_POLICY` environment variable controls whether and how Bitidea responds in group chats:
...
| Value | Behavior |
|-------|----------|
| `open` | Bitidea responds to @mentions from any user in any group. |
| `allowlist` | Bitidea only responds to @mentions from users listed in `FEISHU_ALLOWED_USERS`. |
| `disabled` | Bitidea ignores all group messages entirely. |
...
|---------|---------|---------|
| Quiet period | `BITIDEA_FEISHU_TEXT_BATCH_DELAY_SECONDS` | 0.6s |
| Max messages per batch | `BITIDEA_FEISHU_TEXT_BATCH_MAX_MESSAGES` | 8 |
| Max characters per batch | `BITIDEA_FEISHU_TEXT_BATCH_MAX_CHARS` | 4000 |
...
| Setting | Env Var | Default |
|---------|---------|---------|
| Quiet period | `BITIDEA_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | 0.8s |
...
Inbound messages are deduplicated using message IDs with a 24-hour TTL. The dedup state is persisted across restarts to `~/.bitidea/feishu_seen_message_ids.json`.

| Setting | Env Var | Default |
|---------|---------|---------|
| Cache size | `BITIDEA_FEISHU_DEDUP_CACHE_SIZE` | 2048 entries |
...
| `BITIDEA_FEISHU_DEDUP_CACHE_SIZE` | — | `2048` | Max deduplicated message IDs to track |
| `BITIDEA_FEISHU_TEXT_BATCH_DELAY_SECONDS` | — | `0.6` | Text burst debounce quiet period |
| `BITIDEA_FEISHU_TEXT_BATCH_MAX_MESSAGES` | — | `8` | Max messages merged per text batch |
| `BITIDEA_FEISHU_TEXT_BATCH_MAX_CHARS` | — | `4000` | Max characters merged per text batch |
| `BITIDEA_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | — | `0.8` | Media burst debounce quiet period |
...
| `FEISHU_APP_ID or FEISHU_APP_SECRET not set` | Set both env vars or configure via `bitidea gateway setup` |
| `Another local Bitidea gateway is already using this Feishu app_id` | Only one Bitidea instance can use the same app_id at a time. Stop the other gateway first. |
...
Feishu / Lark uses the `bitidea-feishu` platform preset, which includes the same core tools as Telegram and other gateway-based messaging platforms.
