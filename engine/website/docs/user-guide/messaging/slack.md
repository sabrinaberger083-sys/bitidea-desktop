...
sidebar_position: 4
title: "Slack"
description: "Set up Bitidea Agent as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Bitidea Agent to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your Bitidea instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Bitidea uses the modern
...
4. Enter an app name (e.g., "Bitidea Agent") and select your workspace
5. Click **Create App**
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `bitidea-socket` (the name doesn't matter)
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a Bitidea configuration issue.
:::
...
Bitidea uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
bitidea gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
bitidea gateway              # Foreground
bitidea gateway install      # Install as a user service
sudo bitidea gateway install --system   # Linux only: boot-time system service
```
...
Understanding how Bitidea behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Bitidea Agent what time is it?`). In channels, Bitidea replies in a thread attached to that message. |
| **Threads** | If you @mention Bitidea inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.bitidea/config.yaml`.
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to Bitidea in `#general` will have separate histories and contexts.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Bitidea will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @Bitidea Agent`).

---

## Multi-Workspace Support

Bitidea can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, Bitidea also loads tokens from an **OAuth token file** at:

```
~/.bitidea/slack_tokens.json
```
...
- When a message arrives, Bitidea uses the correct workspace-specific client to respond.
...
Bitidea supports voice on Slack:
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Bitidea Agent` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Bitidea Agent` |
...
7. ✅ Bot was **invited** to the channel (`/invite @Bitidea Agent`)
...
- Tokens should be stored in `~/.bitidea/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your Bitidea config directory
