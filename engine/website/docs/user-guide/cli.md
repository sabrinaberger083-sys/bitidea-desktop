...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Bitidea Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Bitidea Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.
...
bitidea chat --model "anthropic/claude-sonnet-4"

# With a specific provider
bitidea chat --provider nous        # Use Nous Portal
bitidea chat --provider openrouter  # Force OpenRouter
...
bitidea chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
bitidea --continue             # Resume the most recent CLI session (-c)
bitidea --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
bitidea chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
bitidea -w                         # Interactive mode in worktree
bitidea -w -q "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/img/docs/cli-layout.svg" alt="Stylized preview of the Bitidea CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Bitidea CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Bitidea to background (Unix only). Run `fg` in the shell to resume. |
| `Tab` | Accept auto-suggestion (ghost text) or autocomplete slash commands |

## Slash Commands

Type `/` to see the autocomplete dropdown. Bitidea supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Bitidea replies |
...
```bash
bitidea -s bitidea-agent-dev,github-auth
bitidea chat -s github-pr-workflow -s github-auth
```

Bitidea loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.bitidea/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.bitidea/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Bitidea..."
  # Add your own!
```
...
On Unix systems, press **`Ctrl+Z`** to suspend Bitidea to the background — just like any terminal process. The shell prints a confirmation:

```
Bitidea Agent has been suspended. Run `fg` to bring Bitidea Agent back.
```
...
Resume this session with:
  bitidea --resume 20260225_143052_a1b2c3
...
```bash
bitidea --continue                          # Resume the most recent CLI session
bitidea -c                                  # Short form
bitidea -c "my project"                     # Resume a named session (latest in lineage)
bitidea --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
bitidea --resume "refactoring auth"         # Resume by title
bitidea -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `bitidea sessions rename <id> <title>` from the command line. Use `bitidea sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Bitidea's SQLite state database under `~/.bitidea/state.db`. The database keeps:
...
Bitidea immediately confirms the task and gives you back the prompt:
