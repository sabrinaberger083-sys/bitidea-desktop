...
sidebar_position: 7
title: "Docker"
description: "Running Bitidea Agent in Docker and using Docker as a terminal backend"
---

# Bitidea Agent — Docker

There are two distinct ways Docker intersects with Bitidea Agent:

1. **Running Bitidea IN Docker** — the agent itself runs inside a container (this page's primary focus)
...
If this is your first time running Bitidea Agent, create a data directory on the host and start the container interactively to run the setup wizard:
...
This drops you into the setup wizard, which will prompt you for your API keys and write them to `~/.bitidea/.env`. You only need to do this once. It is highly recommended to set up a chat system for the gateway to work with at this point.
...
The `/opt/data` volume is the single source of truth for all Bitidea state. It maps to your host's `~/.bitidea/` directory and contains:

| Path | Contents |
|------|----------|
| `.env` | API keys and secrets |
| `config.yaml` | All Bitidea configuration |
...
:::warning
Never run two Bitidea **gateway** containers against the same data directory simultaneously — session files and memory stores are not designed for concurrent write access. Running a dashboard container alongside the gateway is safe since the dashboard only reads data.
:::
...
services:
  ...
  dashboard:
    image: nousresearch/hermes-agent:latest
    container_name: bitidea-dashboard
    restart: unless-stopped
    ...
    volumes:
      - ~/.bitidea:/opt/data
    environment:
      - GATEWAY_HEALTH_URL=http://bitidea:8642
    networks:
      - bitidea-net
    depends_on:
      - bitidea
...
The Bitidea container needs moderate resources. Recommended minimums:
...
- Python 3 with all Bitidea dependencies (`pip install -e ".[all]"`)
...
- Then runs `bitidea` with whatever arguments you pass
...
Check logs: `docker logs bitidea`. Common causes:
- Missing or invalid `.env` file — run interactively first to complete setup
- Port conflicts if running with exposed ports

### "Permission denied" errors

The container runs as root by default. If your host `~/.bitidea/` was created by a non-root user, permissions should work. If you get errors, ensure the data directory is writable:
...
```sh
docker restart bitidea
```

### Checking container health

```sh
docker logs --tail 50 bitidea          # Recent logs
docker run -it --rm nousresearch/hermes-agent:latest version     # Verify version
docker stats bitidea                    # Resource usage
```
