...
description: "Reference for Bitidea Agent MCP configuration keys, filtering semantics, and utility-tool policy"
---
...
Bitidea may register these utility wrappers per MCP server:
...
Even when `resources: true` or `prompts: true`, Bitidea only registers those utility tools if the MCP session actually exposes the corresponding capability.
...
If filtering removes all server-native tools and no utility tools are registered, Bitidea does not create an empty MCP runtime toolset for that server.
...
Behavior:
- Bitidea uses the MCP SDK's OAuth 2.1 PKCE flow (metadata discovery, dynamic client registration, token exchange, and refresh)
- On first connect, a browser window opens for authorization
- Tokens are persisted to `~/.bitidea/mcp-tokens/<server>.json` and reused across sessions
