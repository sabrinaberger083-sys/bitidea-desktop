...
description: "Frequently asked questions and solutions to common issues with Bitidea Agent"
---
...
### What LLM providers work with Bitidea?

Bitidea Agent works with any OpenAI-compatible API. Supported providers include:
...
Set your provider with `bitidea model` or by editing `~/.bitidea/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.

### Does it work on Windows?

**Not natively.** Bitidea Agent requires a Unix-like environment. On Windows, install [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) and run Bitidea from inside it. The standard install command works perfectly in WSL2:
...
Yes — Bitidea now has a tested Termux install path for Android phones.
...
API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). Bitidea Agent does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.bitidea/`.

### Can I use it offline / with local models?

Yes. Run `bitidea model`, select **Custom endpoint**, and enter your server's URL:
...
Bitidea persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 16384`), make sure to set the matching context length in Bitidea — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::

:::tip Timeouts with local models
Bitidea auto-detects local endpoints and relaxes streaming timeouts (read timeout raised from 120s to 1800s, stale stream detection disabled). If you still hit timeouts on very large contexts, set `BITIDEA_STREAM_READ_TIMEOUT=1800` in your `.env`. See the [Local LLM guide](../guides/local-llm-on-mac.md#timeouts) for details.
:::

### How much does it cost?

Bitidea Agent itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same Bitidea Agent instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use Bitidea programmatically:
...
#### `bitidea: command not found` after installation
...
**Cause:** Bitidea requires Python 3.11 or newer.
...
**Solution:** Exit your session and use `bitidea model` from your terminal to add new providers:

```bash
# Exit the Bitidea chat session first (Ctrl+C or /quit)
...
After adding a new provider via `bitidea model`, start a new chat session — `/model` will now show all your configured providers.

:::tip Quick reference
| Want to... | Use |
|-----------|-----|
| Add a new provider | `bitidea model` (from terminal) |
| Enter/change API keys | `bitidea model` (from terminal) |
...
# Or set directly
bitidea config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```

:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.bitidea/.env` for conflicting entries.
:::
...
# Set a valid model
bitidea config set BITIDEA_MODEL openrouter/nous/hermes-3-llama-3.1-70b

# Or specify per-session
bitidea chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `bitidea chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or Bitidea detected the wrong context length for your model.
...
bitidea chat --model openrouter/google/gemini-3-flash-preview
```

If this happens on the first long conversation, Bitidea may have the wrong context length for your model. Check what it detected:
...
**Cause:** Bitidea detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — Bitidea never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `bitidea chat`
...
# Start the gateway
bitidea gateway start

# Check logs for errors
cat ~/.bitidea/logs/gateway.log | tail -50
```
...
**Solution:**
- Verify your bot token is valid with `bitidea gateway setup`
- Check gateway logs: `cat ~/.bitidea/logs/gateway.log | tail -50`
...
Configure in `~/.bitidea/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
pip install "bitidea-agent[telegram]"   # or [discord], [slack], [whatsapp]
...
#### WSL: Gateway keeps disconnecting or `bitidea gateway start` fails
...
tmux new -s bitidea 'bitidea gateway run'
# Reattach later: tmux attach -t bitidea

# Option 3: Background via nohup
nohup bitidea gateway run > ~/.bitidea/logs/gateway.log 2>&1 &
```
...
1. Create a task that runs `wsl -d Ubuntu -- bash -lc 'bitidea gateway run'`
...
**Solution:** The gateway captures your shell PATH when you run `bitidea gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
bitidea gateway install    # Re-snapshots your current PATH
bitidea gateway start      # Detects the updated plist and reloads
```

You can verify the plist has the correct PATH:
```bash
/usr/libexec/PlistBuddy -c "Print :EnvironmentVariables:PATH" \
  ~/Library/LaunchAgents/ai.bitidea.gateway.plist
```
...
**Solution:**
- Try a faster/smaller model: `bitidea chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `bitidea chat -t "terminal"`
...
cd ~/.bitidea/bitidea-agent && uv pip install -e ".[mcp]"
...
Verify your `~/.bitidea/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
bitidea config show | grep -A 12 mcp_servers

# Restart Bitidea or reload MCP after config changes
bitidea chat
```

See also:
- [MCP (Model Context Protocol)](/docs/user-guide/features/mcp)
- [Use MCP with Bitidea](/docs/guides/use-mcp-with-bitidea)
...
:::warning
If an MCP server crashes mid-request, Bitidea will report a timeout. Check the server's own logs (not just Bitidea logs) to diagnose the root cause.
:::
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `bitidea profile create newname --clone-all` to copy everything from the current profile.

### What happens when I run `bitidea update`?

`bitidea update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `bitidea update` once — it covers every profile on the machine.
...
bitidea profile export work ./work-backup.tar.gz

# Copy the file to the target machine, then:
bitidea profile import ./work-backup.tar.gz work
```
...
There is no hard limit. Each profile is just a directory under `~/.bitidea/profiles/`. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Solution: Delegation config.** Bitidea can route subagents to a different model automatically. Set this in `~/.bitidea/config.yaml`:
...
Now when you tell Bitidea "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can Bitidea do this?

**Current limitation:** Bitidea profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, Bitidea reasoning, and tool call details in Telegram instead of just the final output.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `bitidea skills config` settings don't seem to take effect.

**Solution:** Use `bitidea skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`bitidea gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
**Current behavior:** Bitidea creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting Bitidea to another machine
...
1. Install Bitidea Agent on the new machine:
   ```bash
   curl -fsSL https://raw.githubusercontent.com/sabrinaberger083-sys/bitidea-agent/main/scripts/install.sh | bash
   ```

2. Copy your entire `~/.bitidea/` directory **except** the `bitidea-agent` subdirectory (that's the code repo — the new install has its own):
   ```bash
   # On the source machine
   rsync -av --exclude='bitidea-agent' ~/.bitidea/ newmachine:~/.bitidea/
   ```

   Or use profile export/import:
   ```bash
   # On source machine
   bitidea profile export default ./bitidea-backup.tar.gz

   # On target machine
   bitidea profile import ./bitidea-backup.tar.gz default
   ```

3. On the new machine, run `bitidea setup` to verify API keys and provider config are working. Re-authenticate any messaging platforms (especially WhatsApp, which uses QR pairing).

The `~/.bitidea/` directory contains everything: `config.yaml`, `.env`, `SOUL.md`, `memories/`, `skills/`, `state.db` (sessions), `cron/`, and any custom plugins. The code itself lives in `~/.bitidea/bitidea-agent/` and is installed fresh.

### Permission denied when reloading shell after install

**Scenario:** After running the Bitidea installer, `source ~/.zshrc` gives a permission denied error.

**Cause:** This usually happens when `~/.zshrc` (or `~/.bashrc`) has incorrect file permissions, or when the installer couldn't write to it cleanly. It's not a Bitidea-specific issue — it's a shell config permissions problem.
...
bitidea config show | head -20

# Re-run model selection
bitidea model

# Or test with a known-good model
bitidea chat -q "hello" --model anthropic/claude-sonnet-4.6
```
...
1. **Search existing issues:** [GitHub Issues](https://github.com/sabrinaberger083-sys/bitidea-agent/issues)
2. **Ask the community:** [Nous Research Discord](https://discord.gg/nousresearch)
3. **File a bug report:** Include your OS, Python version (`python3 --version`), Bitidea version (`bitidea --version`), and the full error message
