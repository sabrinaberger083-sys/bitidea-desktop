...
description: "Complete reference of all environment variables used by Bitidea Agent"
---

# Environment Variables Reference

All variables go in `~/.bitidea/.env`. You can also set them with `bitidea config set VAR value`.
...
| `BITIDEA_COPILOT_ACP_COMMAND` | Override Copilot ACP CLI binary path (default: `copilot`) |
| `COPILOT_CLI_PATH` | Alias for `BITIDEA_COPILOT_ACP_COMMAND` |
| `BITIDEA_COPILOT_ACP_ARGS` | Override Copilot ACP arguments (default: `--acp --stdio`) |
...
| `BITIDEA_MODEL` | Override model name at process level (used by cron scheduler; prefer `config.yaml` for normal use) |
| `VOICE_TOOLS_OPENAI_KEY` | Preferred OpenAI key for OpenAI speech-to-text and text-to-speech providers |
| `BITIDEA_LOCAL_STT_COMMAND` | Optional local speech-to-text command template. Supports `{input_path}`, `{output_dir}`, `{language}`, and `{model}` placeholders |
| `BITIDEA_LOCAL_STT_LANGUAGE` | Default language passed to `BITIDEA_LOCAL_STT_COMMAND` or auto-detected local `whisper` CLI fallback (default: `en`) |
| `BITIDEA_HOME` | Override Bitidea config directory (default: `~/.bitidea`). Also scopes the gateway PID file and systemd service name, so multiple installations can run concurrently |

## Provider Auth (OAuth)

For native Anthropic auth, Bitidea prefers Claude Code's own credential files when they exist because those credentials can refresh automatically. Environment variables such as `ANTHROPIC_TOKEN` remain useful as manual overrides, but they are no longer the preferred path for Claude Pro/Max login.

| Variable | Description |
|----------|-------------|
| `BITIDEA_INFERENCE_PROVIDER` | Override provider selection: `auto`, `openrouter`, `nous`, `openai-codex`, `copilot`, `copilot-acp`, `anthropic`, `huggingface`, `zai`, `kimi-coding`, `kimi-coding-cn`, `minimax`, `minimax-cn`, `kilocode`, `xiaomi`, `arcee`, `alibaba`, `deepseek`, `opencode-zen`, `opencode-go`, `ai-gateway` (default: `auto`) |
| `BITIDEA_PORTAL_BASE_URL` | Override Nous Portal URL (for development/testing) |
| `NOUS_INFERENCE_BASE_URL` | Override Nous inference API URL |
| `BITIDEA_NOUS_MIN_KEY_TTL_SECONDS` | Min agent key TTL before re-mint (default: 1800 = 30min) |
| `BITIDEA_NOUS_TIMEOUT_SECONDS` | HTTP timeout for Nous credential / token flows |
| `BITIDEA_DUMP_REQUESTS` | Dump API request payloads to log files (`true`/`false`) |
| `BITIDEA_PREFILL_MESSAGES_FILE` | Path to a JSON file of ephemeral prefill messages injected at API-call time |
| `BITIDEA_TIMEZONE` | IANA timezone override (for example `America/New_York`) |
...
These variables configure the [Tool Gateway](/docs/user-guide/features/tool-gateway) for paid Nous subscribers or self-hosted gateway deployments. Most users don't need to set these — the gateway is configured automatically via `bitidea model` or `bitidea tools`.
...
For cloud sandbox backends, persistence is filesystem-oriented. `TERMINAL_LIFETIME_SECONDS` controls when Bitidea cleans up an idle terminal session, and later resumes may recreate the sandbox rather than keep the same live processes running.
...
| `TERMINAL_SANDBOX_DIR` | Host directory for workspaces and overlays (default: `~/.bitidea/sandboxes/`) |
...
| `MATRIX_USER_ID` | Matrix user ID (e.g. `@bitidea:matrix.org`) — required for password login, optional with access token |
...
| `API_SERVER_MODEL_NAME` | Model name advertised on `/v1/models`. Defaults to the profile name (or `bitidea-agent` for the default profile). Useful for multi-user setups where frontends like Open WebUI need distinct model names per connection. |
| `GATEWAY_PROXY_URL` | URL of a remote Bitidea API server to forward messages to ([proxy mode](/docs/user-guide/messaging/matrix#proxy-mode-e2ee-on-macos)). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Also configurable via `gateway.proxy_url` in `config.yaml`. |
...
| Variable | Description |
|----------|-------------|
| `BITIDEA_MAX_ITERATIONS` | Max tool-calling iterations per conversation (default: 90) |
| `BITIDEA_TOOL_PROGRESS` | Deprecated compatibility variable for tool progress display. Prefer `display.tool_progress` in `config.yaml`. |
| `BITIDEA_TOOL_PROGRESS_MODE` | Deprecated compatibility variable for tool progress mode. Prefer `display.tool_progress` in `config.yaml`. |
| `BITIDEA_HUMAN_DELAY_MODE` | Response pacing: `off`/`natural`/`custom` |
| `BITIDEA_HUMAN_DELAY_MIN_MS` | Custom delay range minimum (ms) |
| `BITIDEA_HUMAN_DELAY_MAX_MS` | Custom delay range maximum (ms) |
| `BITIDEA_QUIET` | Suppress non-essential output (`true`/`false`) |
| `BITIDEA_API_TIMEOUT` | LLM API call timeout in seconds (default: `1800`) |
| `BITIDEA_STREAM_READ_TIMEOUT` | Streaming socket read timeout in seconds (default: `120`). Auto-increased to `BITIDEA_API_TIMEOUT` for local providers. Increase if local LLMs time out during long code generation. |
| `BITIDEA_STREAM_STALE_TIMEOUT` | Stale stream detection timeout in seconds (default: `180`). Auto-disabled for local providers. Triggers connection kill if no chunks arrive within this window. |
| `BITIDEA_EXEC_ASK` | Enable execution approval prompts in gateway mode (`true`/`false`) |
| `BITIDEA_ENABLE_PROJECT_PLUGINS` | Enable auto-discovery of repo-local plugins from `./.bitidea/plugins/` (`true`/`false`, default: `false`) |
| `BITIDEA_BACKGROUND_NOTIFICATIONS` | Background process notification mode in gateway: `all` (default), `result`, `error`, `off` |
| `BITIDEA_EPHEMERAL_SYSTEM_PROMPT` | Ephemeral system prompt injected at API-call time (never persisted to sessions) |

## Cron Scheduler

| Variable | Description |
|----------|-------------|
| `BITIDEA_CRON_TIMEOUT` | Inactivity timeout for cron job agent runs in seconds (default: `600`). The agent can run indefinitely while actively calling tools or receiving stream tokens — this only triggers when idle. Set to `0` for unlimited. |
| `BITIDEA_CRON_SCRIPT_TIMEOUT` | Timeout for pre-run scripts attached to cron jobs in seconds (default: `120`). Override for scripts that need longer execution (e.g., randomized delays for anti-bot timing). Also configurable via `cron.script_timeout_seconds` in `config.yaml`. |
...
For task-specific direct endpoints, Bitidea uses the task's configured API key or `OPENAI_API_KEY`. It does not reuse `OPENROUTER_API_KEY` for those custom endpoints.
...
These go in `~/.bitidea/config.yaml` under the `provider_routing` section:
...
:::tip
Use `bitidea config set` to set environment variables — it automatically saves them to the right file (`.env` for secrets, `config.yaml` for everything else).
:::
