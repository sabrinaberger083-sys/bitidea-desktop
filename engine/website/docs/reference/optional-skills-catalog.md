...
description: "Official optional skills shipped with bitidea-agent — install via bitidea skills install official/<category>/<skill>"
---

# Optional Skills Catalog

Official optional skills ship with the bitidea-agent repository under `optional-skills/` but are **not active by default**. Install them explicitly:
...
```bash
bitidea skills install official/blockchain/solana
bitidea skills install official/mlops/flash-attention
```
...
```bash
bitidea skills uninstall <skill-name>
```
...
| **honcho** | Configure and use Honcho memory with Bitidea — cross-session user modeling, multi-profile peer isolation, observation config, and dialectic reasoning. |
...
| Skill | Description |
|-------|-------------|
| **blender-mcp** | Control Blender directly from Bitidea via socket connection to the blender-mcp addon. Create 3D objects, materials, animations, and run arbitrary Blender Python (bpy) code. |
...
| Skill | Description |
|-------|-------------|
| **openclaw-migration** | Migrate a user's OpenClaw customization footprint into Bitidea Agent. Imports memories, SOUL.md, command allowlists, user skills, and selected workspace assets. |
...
| **bitidea-atropos-environments** | Build, test, and debug Bitidea Agent RL environments for Atropos training. Covers the BitideaAgentBaseEnv interface, reward functions, agent loop integration, and evaluation. |
...
| **telephony** | Give Bitidea phone capabilities — provision a Twilio number, send/receive SMS/MMS, make calls, and place AI-driven outbound calls through Bland.ai or Vapi. |
