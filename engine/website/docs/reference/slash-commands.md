...
Bitidea has two slash-command surfaces, both driven by a central `COMMAND_REGISTRY` in `bitidea_cli/commands.py`:
...
Installed skills are also exposed as dynamic slash commands on both surfaces. That includes bundled skills like `/plan`, which opens plan mode and saves markdown plans under `.bitidea/plans/` relative to the active workspace/backend working directory.
...
| `/snapshot [create\|restore <id>\|prune]` (alias: `/snap`) | Create or restore state snapshots of Bitidea config/state. `create [label]` saves a snapshot, `restore <id>` reverts to it, `prune [N]` removes old snapshots, or list all with no args. |
...
| `/snapshot` (alias: `/snap`) | Create or restore state snapshots of Bitidea config/state (usage: /snapshot [create\|restore \<id\>\|prune]) |
...
| `/plan [request]` | Load the bundled `plan` skill to write a markdown plan instead of executing the work. Plans are saved under `.bitidea/plans/` relative to the active workspace/backend working directory. |
...
User-defined quick commands map a short alias to a longer prompt. Configure them in `~/.bitidea/config.yaml`:
...
| `/snapshot [create\|restore <id>\|prune]` (alias: `/snap`) | Create or restore state snapshots of Bitidea config/state. |
