...
description: "Reference for Bitidea core, composite, platform, and dynamic toolsets"
---
...
- **Platform** — A complete tool configuration for a specific deployment context (e.g., `bitidea-cli` is the default for interactive CLI sessions)
...
```bash
bitidea chat --toolsets web,file,terminal
bitidea chat --toolsets debugging        # composite — expands to file + terminal + web
bitidea chat --toolsets all              # everything
```
...
```yaml
toolsets:
  - bitidea-cli          # default for CLI
  # - bitidea-telegram   # override for Telegram gateway
```

### Interactive management

```bash
bitidea tools                            # curses UI to enable/disable per platform
```
...
| `code_execution` | `execute_code` | Run Python scripts that call Bitidea tools programmatically. |
...
Platform toolsets define the complete tool configuration for a deployment target. Most messaging platforms use the same set as `bitidea-cli`:

| Toolset | Differences from `bitidea-cli` |
|---------|-------------------------------|
| `bitidea-cli` | Full toolset — all 36 tools including `clarify`. The default for interactive CLI sessions. |
| `bitidea-acp` | Drops `clarify`, `cronjob`, `image_generate`, `send_message`, `text_to_speech`, homeassistant tools. Focused on coding tasks in IDE context. |
| `bitidea-api-server` | Drops `clarify`, `send_message`, and `text_to_speech`. Adds everything else — suitable for programmatic access where user interaction isn't possible. |
| `bitidea-telegram` | Same as `bitidea-cli`. |
| `bitidea-discord` | Same as `bitidea-cli`. |
| `bitidea-slack` | Same as `bitidea-cli`. |
| `bitidea-whatsapp` | Same as `bitidea-cli`. |
| `bitidea-signal` | Same as `bitidea-cli`. |
| `bitidea-matrix` | Same as `bitidea-cli`. |
| `bitidea-mattermost` | Same as `bitidea-cli`. |
| `bitidea-email` | Same as `bitidea-cli`. |
| `bitidea-sms` | Same as `bitidea-cli`. |
| `bitidea-dingtalk` | Same as `bitidea-cli`. |
| `bitidea-feishu` | Same as `bitidea-cli`. |
| `bitidea-wecom` | Same as `bitidea-cli`. |
| `bitidea-wecom-callback` | WeCom callback toolset — enterprise self-built app messaging (full access). |
| `bitidea-weixin` | Same as `bitidea-cli`. |
| `bitidea-bluebubbles` | Same as `bitidea-cli`. |
| `bitidea-qqbot` | Same as `bitidea-cli`. |
| `bitidea-homeassistant` | Same as `bitidea-cli`. |
| `bitidea-webhook` | Same as `bitidea-cli`. |
| `bitidea-gateway` | Union of all messaging platform toolsets. Used internally when the gateway needs the broadest possible tool set. |
...
## Relationship to `bitidea tools`

The `bitidea tools` command provides a curses-based UI for toggling individual tools on or off per platform. This operates at the tool level (finer than toolsets) and persists to `config.yaml`. Disabled tools are filtered out even if their toolset is enabled.
