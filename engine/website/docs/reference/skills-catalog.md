...
description: "Catalog of bundled skills that ship with Bitidea Agent"
---

# Bundled Skills Catalog

Bitidea ships with a large built-in skill library copied into `~/.bitidea/skills/` on install. This page catalogs the bundled skills that live in the repository under `skills/`.
...
| `bitidea-agent-spawning` | Spawn additional Bitidea Agent instances as autonomous subprocesses for independent long-running tasks. Supports non-interactive one-shot mode (-q) and interactive PTY mode for multi-turn collaboration. Different from delegate_task — this runs a full separate bitidea process. | `autonomous-ai-agents/bitidea-agent` |
...
| `bitidea-agent-setup` | Help users configure Bitidea Agent — CLI usage, setup wizard, model/provider selection, tools, skills, voice/STT/TTS, gateway, and troubleshooting. | `dogfood/bitidea-agent-setup` |
...
| `native-mcp` | Built-in MCP (Model Context Protocol) client that connects to external MCP servers, discovers their tools, and registers them as native Bitidea Agent tools. Supports stdio and HTTP transports with automatic reconnection, security filtering, and zero-config tool injection. | `mcp/native-mcp` |
...
| `bitidea-atropos-environments` | Build, test, and debug Bitidea Agent RL environments for Atropos training. Covers the BitideaAgentBaseEnv interface, reward functions, agent loop integration, evaluation with tools, wandb logging, and the three CLI modes (serve/process/evaluate). Use when creating, reviewing, or f… | `mlops/training/bitidea-atropos-environments` |
...
| Skill | Description | Path |
|-------|-------------|------|
| `google-workspace` | Gmail, Calendar, Drive, Contacts, Sheets, and Docs integration via Python. Uses OAuth2 with automatic token refresh. No external binaries needed — runs entirely with Google's Python client libraries in the Bitidea venv. | `productivity/google-workspace` |
...
| `plan` | Plan mode for Bitidea — inspect context, write a markdown plan into `.bitidea/plans/` in the active workspace/backend working directory, and do not execute the work. | `software-development/plan` |
...
```bash
bitidea skills install official/<category>/<skill>
```
...
| Skill | Description | Path |
|-------|-------------|------|
| `blender-mcp` | Control Blender directly from Bitidea via socket connection to the blender-mcp addon. Create 3D objects, materials, animations, and run arbitrary Blender Python (bpy) code. | `creative/blender-mcp` |
...
| Skill | Description | Path |
|-------|-------------|------|
| `agentmail` | Give the agent its own dedicated email inbox via AgentMail. Send, receive, and manage email autonomously using agent-owned email addresses (e.g. bitidea-agent@agentmail.to). | `email/agentmail` |
...
| Skill | Description | Path |
|-------|-------------|------|
| `openclaw-migration` | Migrate a user's OpenClaw customization footprint into Bitidea Agent. Imports Bitidea-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports what could not be migrated and why. | `migration/openclaw-migration` |

## productivity

| Skill | Description | Path |
|-------|-------------|------|
| `telephony` | Give Bitidea phone capabilities — provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi. | `productivity/telephony` |
