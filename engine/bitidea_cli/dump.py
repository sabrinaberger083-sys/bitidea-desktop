"""
Dump command for bitidea CLI.

Outputs a compact, plain-text summary of the user's Bitidea setup
...
from bitidea_cli.config import get_bitidea_home, get_env_path, get_project_root, load_config
...
def _count_skills(bitidea_home: Path) -> int:
...
def _cron_summary(bitidea_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = bitidea_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"bitidea_home:      {display_bitidea_home()}")
    ...
    toolsets = config.get("toolsets", ["bitidea-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(bitidea_home)}")
    lines.append(f"  skills:             {_count_skills(bitidea_home)}")
