...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_bitidea_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:bitidea_pkce",
    ...
    raise SystemExit(f"`bitidea auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    if removed.source.startswith("env:"):
        ...
        if env_var:
            from bitidea_cli.config import remove_env_value
            cleared = remove_env_value(env_var)
            if cleared:
                print(f"Cleared {env_var} from .env")

    # If this was a singleton-seeded credential (OAuth device_code, bitidea_pkce),
    ...
    # re-seeded on the next load_pool() call.
    ...
    elif removed.source == "bitidea_pkce" and provider == "anthropic":
        from bitidea_constants import get_bitidea_home
        oauth_file = get_bitidea_home() / ".anthropic_oauth.json"
        if oauth_file.exists():
            oauth_file.unlink()
            print("Cleared Bitidea Anthropic OAuth credentials")

    elif removed.source == "claude_code" and provider == "anthropic":
        from bitidea_cli.auth import suppress_credential_source
        ...
        print("Run `bitidea auth add anthropic` to re-enable if needed.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `bitidea auth` is called bare."""
