"""``bitidea debug`` — debug tools for Bitidea Agent.

Currently supports:
    bitidea debug share    Upload debug report (system info + logs) to a
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, Bitidea version, provider, which API keys
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `bitidea debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "bitidea-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    script = (
        ...
        f"headers={{'User-Agent': 'bitidea-agent/auto-delete'}}), timeout=15) "
        f"for u in [{url_list}]]"
    )
...
def _delete_hint(url: str) -> str:
    ...
    paste_id = _extract_paste_id(url)
    if paste_id:
        return f"bitidea debug delete {url}"
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "bitidea-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----BitideaDebugBoundary9f3c"
...
def _resolve_log_path(log_name: str) -> Optional[Path]:
    ...
    """
    from bitidea_cli.logs import LOG_FILES
...
def _read_log_tail(log_name: str, num_lines: int) -> str:
    ...
    from bitidea_cli.logs import _read_last_n_lines
...
def _capture_dump() -> str:
    """Run ``bitidea dump`` and return its stdout as a string."""
...
def collect_debug_report(*, log_lines: int = 200, dump_text: str = "") -> str:
    ...
    dump_text
        Pre-captured dump output.  If empty, ``bitidea dump`` is run
        internally.
...
def run_debug_share(args):
    ...
    print(f"To delete now:  bitidea debug delete <url>")

    print(f"\nShare these links with the Bitidea team for support.")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: bitidea debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'bitidea debug share'.")
        return
...
def run_debug(args):
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: bitidea debug <command>")
        print()
        print("Commands:")
