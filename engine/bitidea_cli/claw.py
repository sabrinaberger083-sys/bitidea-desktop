"""bitidea claw — OpenClaw migration commands.
...
Usage:
    ...
    bitidea claw migrate --preset full --overwrite  # Full migration, overwrite conflicts
    bitidea claw cleanup              # Archive leftover OpenClaw directories
    bitidea claw cleanup --dry-run    # Preview what would be archived
"""
...
from bitidea_cli.config import get_bitidea_home, get_config_path, load_config, save_config
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    print_info(
        ...
        "Bitidea may try to use the same token, causing disconnects."
    )
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a Bitidea gateway is running with connected platforms.
    ...
    print()
    print_error(
        "Bitidea gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
    ...
    print_info("Recommendation: stop the gateway first with 'bitidea stop'.")
    print()
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_bitidea.py script in known locations."""
...
def claw_command(args):
    """Route bitidea claw subcommands."""
    ...
    if action == "migrate":
        ...
        print("Usage: bitidea claw <command> [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to Bitidea")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'bitidea claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → Bitidea migration."""
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: bitidea claw migrate --source /path/to/.openclaw")
        return
    ...
    print_info(f"Target:      {bitidea_home}")
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: bitidea claw migrate --yes")
            return
    ...
    # 'bitidea claw cleanup' separately.
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: bitidea claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: bitidea claw cleanup --yes")
        else:
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  bitidea claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  bitidea claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  bitidea config set OPENROUTER_API_KEY sk-or-v1-...")
