...
Add, remove, or reorder entries here — both `bitidea setup` and
`bitidea` provider-selection will pick up the change automatically.
"""
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `bitidea model`
    flow without maintaining a separate static list.
    """
    from bitidea_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
def check_nous_free_tier() -> bool:
    ...
    try:
        from bitidea_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
# this list:  bitidea model, /model, /provider, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `bitidea model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `bitidea model` TUI
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``bitidea model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``bitidea auth add openai-codex`` without
    ``bitidea model``).
    """
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``bitidea model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from bitidea_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Bitidea' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``bitidea_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Bitidea should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    try:
        from bitidea_cli.auth import resolve_api_key_provider_credentials
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from bitidea_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
def _fetch_ai_gateway_models(timeout: float = 5.0) -> Optional[list[str]]:
    ...
    if not base_url:
        from bitidea_constants import AI_GATEWAY_BASE_URL
...
def _ollama_cloud_cache_path() -> Path:
    ...
    return get_bitidea_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom":
        ...
        message = (
            ...
            f"Bitidea will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
