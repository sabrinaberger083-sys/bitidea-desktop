"""
Gateway subcommand for bitidea CLI.

Handles: bitidea gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_bitidea_home is imported lazily at call sites to avoid ImportError
# when bitidea_constants is cached from a pre-update version during `bitidea update`.
from bitidea_cli.setup import (
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "bitidea-gateway*",
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``bitidea update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Bitidea profile are returned.
    """
    _exclude = exclude_pids or set()
    pids = [pid for pid in _get_service_pids() if pid not in _exclude]
    patterns = [
        "bitidea_cli.main gateway",
        "bitidea_cli.main --profile",
        "bitidea_cli.main -p",
        "bitidea_cli/main.py gateway",
        "bitidea_cli/main.py --profile",
        "bitidea_cli/main.py -p",
        "bitidea gateway",
        "gateway/run.py",
    ]
    current_home = str(get_bitidea_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"BITIDEA_HOME={current_home}" in command
            )

        if "--profile " in command or " -p " in command:
            return False
        if "BITIDEA_HOME=" in command and f"BITIDEA_HOME={current_home}" not in command:
            return False
        return True
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (BITIDEA_HOME-scoped).
...
from bitidea_constants import is_container, is_termux, is_wsl
...
_SERVICE_BASE = "bitidea-gateway"
SERVICE_DESCRIPTION = "Bitidea Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current BITIDEA_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (BITIDEA_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from bitidea_constants import get_default_bitidea_root
    home = get_bitidea_home().resolve()
    default = get_default_bitidea_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary BITIDEA_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(bitidea_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when BITIDEA_HOME is a named profile.

    For ``~/.bitidea/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        bitidea_home: Optional explicit BITIDEA_HOME path. Defaults to the current
            ``get_bitidea_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from bitidea_constants import get_default_bitidea_root
    home = Path(bitidea_home or str(get_bitidea_home())).resolve()
    default = get_default_bitidea_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this BITIDEA_HOME.

    Default ``~/.bitidea`` returns ``bitidea-gateway`` (backward compatible).
    Profile ``~/.bitidea/profiles/coder`` returns ``bitidea-gateway-coder``.
    Any other BITIDEA_HOME appends a short hash for uniqueness.
    """
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    bitidea gateway uninstall")
    print_info("    sudo bitidea gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            print_warning("  System service install requires sudo, so Bitidea can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo bitidea gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo bitidea gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo bitidea gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Bitidea often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.bitidea`` → ``ai.bitidea.gateway.plist`` (backward compatible).
    Profile ``~/.bitidea/profiles/coder`` → ``ai.bitidea.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.bitidea.gateway-{suffix}" if suffix else "ai.bitidea.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.bitidea/bitidea-agent  -> /home/alice/.bitidea/bitidea-agent
      /opt/bitidea                 -> /opt/bitidea  (kept as-is)
...
def _bitidea_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current BITIDEA_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_bitidea_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.bitidea                    → /home/alice/.bitidea
      /root/.bitidea/profiles/coder     → /home/alice/.bitidea/profiles/coder
      /opt/custom-bitidea               → /opt/custom-bitidea  (kept as-is)
    """
    current_bitidea = get_bitidea_home().resolve()
    current_default = (Path.home() / ".bitidea").resolve()
    target_default = Path(target_home_dir) / ".bitidea"

    # Default ~/.bitidea → remap to target user's default
    if current_bitidea == current_default:
        return str(target_default)

    # Profile or subdir of ~/.bitidea → preserve the relative structure
    try:
        relative = current_bitidea.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.bitidea) — keep as-is
        return str(current_bitidea)
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        bitidea_home = _bitidea_home_for_target_user(home_dir)
        profile_arg = _profile_arg(bitidea_home)
...
User={username}
Group={group_name}
ExecStart={python_path} -m bitidea_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
Environment="BITIDEA_HOME={bitidea_home}"
Restart=on-failure
RestartSec=30
...
"""
    ...
    bitidea_home = str(get_bitidea_home().resolve())
    profile_arg = _profile_arg(bitidea_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Bitidea install")
    return True
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}bitidea gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}bitidea gateway status{scope_flag}             # Check status")
...
def systemd_restart(system: bool = False):
    ...
    if pid is not None and _request_gateway_self_restart(pid):
        ...
        print(
            ...
            f"  Check status: {'sudo ' if system else ''}bitidea gateway status\n"
...
def systemd_status(deep: bool = False, system: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}bitidea gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}bitidea gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}bitidea gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.bitidea.gateway-{suffix}" if suffix else "ai.bitidea.gateway"
...
def generate_launchd_plist() -> str:
    ...
    prog_args = [
        ...
        "<string>-m</string>",
        "<string>bitidea_cli.main</string>",
    ]
    if profile_arg:
...
<dict>
    ...
    <dict>
        ...
        <key>BITIDEA_HOME</key>
        <string>{bitidea_home}</string>
    </dict>
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Bitidea install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  bitidea gateway status             # Check status")
...
def launchd_stop():
    ...
    # `bitidea gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Bitidea install")
    else:
        print("⚠ Service definition is stale relative to the current Bitidea install")
        print("  Run: bitidea gateway start")
    ...
    if deep:
        log_file = get_bitidea_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ◆ Bitidea Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Bitidea will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @bitidea:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. bitidea) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where Bitidea delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Bitidea agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Bitidea will use (e.g., bitidea@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Bitidea connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: bitidea pairing generate bluebubbles",
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_bitidea_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'bitidea pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: bitidea pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'bitidea gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from bitidea_cli.main import cmd_whatsapp
    import argparse
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Bitidea will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info("  3. Bitidea will store the returned account_id/token in ~/.bitidea/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `bitidea gateway setup`.")
        return
    ...
    import asyncio
    try:
        credentials = asyncio.run(qr_login(str(get_bitidea_home())))
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `bitidea pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `bitidea pairing approve`.")
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"BitideaAgent\"")
        ...
        if service_running:
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: bitidea gateway")
                except subprocess.CalledProcessError as e:
            ...
            if supports_systemd_services() or is_macos():
                ...
                if prompt_yes_no(f"  Install the gateway as a {platform_name} service?{wsl_note} (runs in background, starts on boot)", True):
                    ...
                    print_info("  Or run in foreground:  bitidea gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: bitidea gateway run")
                print_info("  For persistence:   tmux new -s bitidea 'bitidea gateway run'")
                ...
                if is_termux():
                    ...
                    print_info("  Run in foreground: bitidea gateway run")
                    print_info(f"  Or start it manually in the background (best effort): nohup bitidea gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
                else:
                    print_info("  Service install not supported on this platform.")
                    print_info("  Run in foreground: bitidea gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'bitidea gateway setup' when ready.")
...
def gateway_command(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: bitidea gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: bitidea gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s bitidea 'bitidea gateway run'")
                print()
            ...
            print()
            print("  bitidea gateway run                              # direct foreground")
            print("  tmux new -s bitidea 'bitidea gateway run'         # persistent via tmux")
            print("  nohup bitidea gateway run > ~/.bitidea/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: bitidea gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: bitidea gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: bitidea gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: bitidea gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: bitidea gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  bitidea gateway install")
                    print("  sudo bitidea gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  bitidea gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup bitidea gateway run > ~/.bitidea/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s bitidea 'bitidea gateway run'         # persistent via tmux")
                    print("  nohup bitidea gateway run > ~/.bitidea/logs/gateway.log 2>&1 &  # background")
                else:
                    print("  bitidea gateway install  # Install as user service")
                    print("  sudo bitidea gateway install --system  # Install as boot-time system service")
