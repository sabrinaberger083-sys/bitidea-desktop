"""
Unified tool configuration for Bitidea Agent.

`bitidea tools` and `bitidea setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.bitidea/config.yaml under
...
from bitidea_cli.nous_subscription import (
...
from bitidea_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _BITIDEA_CORE_TOOLS (available at runtime if enabled),
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from bitidea_cli.plugins import discover_plugins, get_plugin_toolsets
...
from bitidea_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
    ...
    if post_setup_key in ("agent_browser", "browserbase"):
        ...
        if not node_modules.exists() and shutil.which("npm"):
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_bitidea_home()}/bitidea-agent && npm install")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in bitidea-agent directory)")
...
def _get_platform_tools(
    ...
    *,
    ...
    # "bitidea-cli" (which include all _BITIDEA_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "bitidea-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
    ...
    # A plugin toolset is "known" for a platform once `bitidea tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by bitidea tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (bitidea-cli, bitidea-telegram, etc.)
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `bitidea tools` and `bitidea setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("◆ Bitidea Tool Configuration", Colors.CYAN, Colors.BOLD))
    ...
    print()
    from bitidea_constants import display_bitidea_home
    print(color(f"  Tool configuration saved to {display_bitidea_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'bitidea' or gateway restart.", Colors.DIM))
    print()
