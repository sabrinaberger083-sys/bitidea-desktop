"""
Bitidea Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m bitidea_cli.main web          # Start on http://127.0.0.1:9119
    python -m bitidea_cli.main web --port 8080
"""
...
try:
    ...
    raise SystemExit(
        ...
        "Run 'bitidea web' to auto-install, or: pip install bitidea-agent[web]"
    )

WEB_DIST = Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)

app = FastAPI(title="Bitidea Agent", version=__version__)
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Bitidea supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# the canonical ``bitidea auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    Bitidea resolves Anthropic creds in this order at runtime:
    1. ``~/.bitidea/.anthropic_oauth.json`` — Bitidea-managed PKCE flow
    ...
    try:
        from agent.anthropic_adapter import (
            read_bitidea_oauth_credentials,
            read_claude_code_credentials,
            _BITIDEA_OAUTH_FILE,
        )
    except ImportError:
        read_claude_code_credentials = None  # type: ignore
        read_bitidea_oauth_credentials = None  # type: ignore
        _BITIDEA_OAUTH_FILE = None  # type: ignore

    bitidea_creds = None
    if read_bitidea_oauth_credentials:
        try:
            bitidea_creds = read_bitidea_oauth_credentials()
        except Exception:
            bitidea_creds = None
    if bitidea_creds and bitidea_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "bitidea_pkce",
            "source_label": f"Bitidea PKCE ({_BITIDEA_OAUTH_FILE})",
            "token_preview": _truncate_token(bitidea_creds.get("accessToken")),
            "expires_at": bitidea_creds.get("expiresAt"),
            "has_refresh_token": bool(bitidea_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Bitidea even
    when they also have a separate Bitidea-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "bitidea auth add anthropic",
    ...
    },
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "bitidea auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "bitidea auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "bitidea auth add qwen-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from bitidea_cli import auth as hauth
        if provider_id == "nous":
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("bitidea_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Bitidea-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in ("anthropic", "claude-code"):
        try:
            from agent.anthropic_adapter import _BITIDEA_OAUTH_FILE
            if _BITIDEA_OAUTH_FILE.exists():
                _BITIDEA_OAUTH_FILE.unlink()
        except Exception:
            pass
...
#          → persists to ~/.bitidea/.anthropic_oauth.json AND credential pool
...
# Guarded so bitidea web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Bitidea file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``bitidea auth add anthropic``.
    """
    from agent.anthropic_adapter import _BITIDEA_OAUTH_FILE
    payload = {
    ...
    _BITIDEA_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    _BITIDEA_OAUTH_FILE.write_text(json.dumps(payload, indent=2), encoding="utf-8")
...
def _submit_anthropic_pkce(session_id: str, code_input: str) -> Dict[str, Any]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "bitidea-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    from bitidea_cli import auth as hauth
    if provider_id == "nous":
        from bitidea_cli.auth import _request_device_code, PROVIDER_REGISTRY
        import httpx
        pconfig = PROVIDER_REGISTRY["nous"]
        portal_base_url = (
            os.getenv("BITIDEA_PORTAL_BASE_URL")
...
def _nous_poller(session_id: str) -> None:
    ...
    from bitidea_cli.auth import _poll_for_token, refresh_nous_oauth_from_state
...
async def get_logs(
    ...
    from bitidea_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_bitidea_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from bitidea_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
async def toggle_skill(body: SkillToggle):
    from bitidea_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
def mount_spa(application: FastAPI):
    ...
    def _serve_index():
        ...
        token_script = (
            f'<script>window.__BITIDEA_SESSION_TOKEN__="{_SESSION_TOKEN}";</script>'
        )
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",   "label": "Bitidea Teal",  "description": "Classic dark teal — the canonical Bitidea look"},
...
def _discover_user_themes() -> list:
    """Scan ~/.bitidea/dashboard-themes/*.yaml for user-created themes."""
    themes_dir = get_bitidea_home() / "dashboard-themes"
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as bitidea_cli.plugins):
    1. User plugins:    ~/.bitidea/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.bitidea/plugins/  (only if BITIDEA_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    search_dirs = [
        (get_bitidea_home() / "plugins", "user"),
        (PROJECT_ROOT / "plugins" / "memory", "bundled"),
        (PROJECT_ROOT / "plugins", "bundled"),
    ]
    if os.environ.get("BITIDEA_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".bitidea" / "plugins", "project"))
    ...
    for plugin in _get_dashboard_plugins():
        ...
        try:
            spec = importlib.util.spec_from_file_location(
                f"bitidea_dashboard_plugin_{plugin['name']}", api_path,
            )
...
def start_server(
    ...
    print(f"  Bitidea Web UI → http://{host}:{port}")
