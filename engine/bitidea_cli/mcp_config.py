"""
MCP Server Management CLI — ``bitidea mcp`` subcommand.

Implements ``bitidea mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.bitidea/config.yaml under the ``mcp_servers`` key.
"""
...
from bitidea_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_bitidea_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from bitidea_cli.cli_output import prompt as _shared_prompt
...
def cmd_mcp_add(args):
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  bitidea mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  bitidea mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  bitidea mcp add myserver --preset mypreset')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_bitidea_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: bitidea mcp test " + name)
        return
    ...
    print()
    _success(f"Saved '{name}' to {display_bitidea_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  bitidea mcp add <name> --url <endpoint>')
        _info('  bitidea mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
# ─── bitidea mcp configure ────────────────────────────────────────────────────

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'bitidea mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``bitidea mcp`` subcommands."""
    ...
    if handler:
        ...
        _info("bitidea mcp serve                              Run as MCP server")
        _info("bitidea mcp add <name> --url <endpoint>        Add an MCP server")
        _info("bitidea mcp add <name> --command <cmd>         Add a stdio server")
        _info("bitidea mcp add <name> --preset <preset>       Add from a known preset")
        _info("bitidea mcp remove <name>                      Remove a server")
        _info("bitidea mcp list                               List servers")
        _info("bitidea mcp test <name>                        Test connection")
        _info("bitidea mcp configure <name>                   Toggle tools")
        print()
