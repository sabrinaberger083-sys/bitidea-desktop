...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or bitidea -c.",
    ...
    "Ctrl+Z suspends Bitidea to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "bitidea -c resumes your most recent CLI session. bitidea -c \"project name\" resumes by title.",
    "bitidea -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "bitidea -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "bitidea chat -t web,terminal enables only specific toolsets for a focused session.",
    "bitidea chat -s github-pr-workflow preloads a skill at launch.",
    "bitidea chat -q \"query\" runs a single non-interactive query and exits.",
    "bitidea chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "bitidea chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "bitidea --yolo bypasses all dangerous command approval prompts for the entire session.",
    "bitidea chat --source telegram tags the session for filtering in bitidea sessions list.",
    "bitidea -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "bitidea doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "bitidea dump outputs a compact setup summary — great for bug reports.",
    "bitidea config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "bitidea config edit opens config.yaml in your default editor.",
    "bitidea config check scans for missing or stale configuration options.",
    "bitidea sessions browse opens an interactive session picker with search.",
    "bitidea sessions stats shows session counts by platform and database size.",
    "bitidea sessions prune --older-than 30 cleans up old sessions.",
    "bitidea skills search react --source skills-sh searches the skills.sh public directory.",
    "bitidea skills check scans installed hub skills for upstream updates.",
    "bitidea skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "bitidea skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "bitidea mcp add github --command npx adds MCP servers from the command line.",
    "bitidea mcp serve runs Bitidea itself as an MCP server for other agents.",
    "bitidea auth add lets you add multiple API keys for credential pool rotation.",
    "bitidea completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "bitidea logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "bitidea backup creates a zip backup of your entire Bitidea home directory.",
    "bitidea profile create coder creates an isolated profile that becomes its own command.",
    "bitidea profile create work --clone copies your current config and keys to a new profile.",
    "bitidea update syncs new bundled skills to ALL profiles automatically.",
    "bitidea gateway install sets up Bitidea as a system service (systemd/launchd).",
    "bitidea memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "bitidea webhook subscribe creates event-driven webhook routes with HMAC validation.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as bitidea -w).",
    ...
    "BITIDEA_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Bitidea tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'bitidea profile create coder' creates the 'coder' command.",
    "bitidea profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Bitidea prints a resume command with session ID and stats.",
    "bitidea sessions export backup.jsonl exports all sessions for backup or analysis.",
    "bitidea -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "bitidea skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .bitidea/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: bitidea cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Bitidea runs on 18 platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, email, and more.",
    "bitidea gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Bitidea via a local macOS server.",
    ...
    "Context files (.bitidea.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.bitidea/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Bitidea loads project context from .bitidea.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — bitidea tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.bitidea/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "bitidea chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "bitidea plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "bitidea acp runs Bitidea as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "BITIDEA_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "bitidea login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "bitidea status --deep runs deeper diagnostic checks across all components.",

    # --- Hidden Gems & Power-User Tricks ---
    "BOOT.md at ~/.bitidea/BOOT.md runs automatically on every gateway start — use it for startup checks.",
    "Cron jobs can attach a Python script (--script) whose stdout is injected into the prompt as context.",
    "Cron scripts live in ~/.bitidea/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Bitidea your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.bitidea/SOUL.md to customize.",
    ...
    "Before a gateway session reset, Bitidea auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.bitidea/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at BITIDEA_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "BITIDEA_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in BITIDEA_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.bitidea/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'bitidea profile create ls' would be rejected.",
    "bitidea profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "bitidea claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "BITIDEA_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via bitidea tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.bitidea/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.bitidea/skills/.hub/quarantine/ holds skills pending security review.",
]
