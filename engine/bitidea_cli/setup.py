"""
Interactive setup wizard for Bitidea Agent.
...
Config files are stored in ~/.bitidea/ for easy access.
"""
...
# display_bitidea_home imported lazily at call sites (stale-module safety during bitidea update)

from bitidea_cli.colors import Colors, color
...
from bitidea_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("◆ Bitidea Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Bitidea using environment variables or config commands:")
    print_info("  bitidea config set model.provider custom")
    print_info("  bitidea config set model.base_url http://localhost:8080/v1")
    print_info("  bitidea config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'bitidea setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from bitidea_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'bitidea setup')")


def _print_setup_summary(config: dict, bitidea_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'bitidea setup' to configure"))
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'bitidea setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'bitidea setup terminal'"))
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'bitidea setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {bitidea_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('bitidea setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('bitidea setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('bitidea setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('bitidea setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('bitidea setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('bitidea config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('bitidea config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('bitidea config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('bitidea', Colors.GREEN)}              Start chatting")
    print(f"   {color('bitidea gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('bitidea doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `bitidea tools` and `bitidea setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``bitidea model``)
    ...
    provider added to ``bitidea model`` is automatically available here.
    ...
    # Delegate to the shared bitidea model flow — handles provider picker,
    ...
    try:
        ...
        print_info("You can try again later with: bitidea model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            print_info(
                "Bitidea can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Bitidea can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'bitidea setup' or configure AUXILIARY_VISION_* settings")
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.bitidea/.env."
            )
        ...
        if not existing:
            ...
            if api_key:
                ...
                print_warning(
                    ...
                    f"bitidea setup model or {_dhh()}/.env to use xAI TTS. "
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'bitidea setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Bitidea runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("  When using Bitidea via Telegram/Discord, this is where")
        print_info(
...
def _apply_default_agent_settings(config: dict):
    ...
    save_env_value("BITIDEA_MAX_ITERATIONS", "90")
    ...
    print_info("  Run `bitidea setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    # ── Max Iterations ──
    current_max = get_env_value("BITIDEA_MAX_ITERATIONS") or str(
    ...
    try:
        ...
        if max_iter > 0:
            save_env_value("BITIDEA_MAX_ITERATIONS", str(max_iter))
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Bitidea delivers cron job results,")
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Bitidea delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where Bitidea delivers cron job results and notifications.")
...
def _setup_whatsapp():
    ...
    print_info("Requires Node.js. Run 'bitidea whatsapp' for guided setup.")
    print()
    if prompt_yes_no("Enable WhatsApp now?", True):
        save_env_value("WHATSAPP_ENABLED", "true")
        print_success("WhatsApp enabled")
        print_info("Run 'bitidea whatsapp' to choose your mode (separate bot number")
        print_info("or personal self-chat) and pair via QR code.")


def _setup_weixin():
    """Configure Weixin (personal WeChat) via iLink Bot API QR login."""
    from bitidea_cli.gateway import _setup_weixin as _gateway_setup_weixin
    _gateway_setup_weixin()


def _setup_signal():
    """Configure Signal via gateway setup."""
    from bitidea_cli.gateway import _setup_signal as _gateway_setup_signal
    _gateway_setup_signal()


def _setup_email():
    """Configure Email via gateway setup."""
    from bitidea_cli.gateway import _setup_email as _gateway_setup_email
    _gateway_setup_email()


def _setup_sms():
    """Configure SMS (Twilio) via gateway setup."""
    from bitidea_cli.gateway import _setup_sms as _gateway_setup_sms
    _gateway_setup_sms()


def _setup_dingtalk():
    """Configure DingTalk via gateway setup."""
    from bitidea_cli.gateway import _setup_dingtalk as _gateway_setup_dingtalk
    _gateway_setup_dingtalk()


def _setup_feishu():
    """Configure Feishu / Lark via gateway setup."""
    from bitidea_cli.gateway import _setup_feishu as _gateway_setup_feishu
    _gateway_setup_feishu()


def _setup_wecom():
    """Configure WeCom (Enterprise WeChat) via gateway setup."""
    from bitidea_cli.gateway import _setup_wecom as _gateway_setup_wecom
    _gateway_setup_wecom()


def _setup_wecom_callback():
    """Configure WeCom Callback (self-built app) via gateway setup."""
    from bitidea_cli.gateway import _setup_wecom_callback as _gw_setup
    _gw_setup()
...
def _setup_qqbot():
    ...
    print_info("Connects Bitidea to QQ via the Official QQ Bot API (v2).")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Bitidea to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    if qq_platform:
        from bitidea_cli.gateway import _setup_standard_platform
...
def _setup_webhooks():
    ...
    print()
    print_info("   Open config in your editor:  bitidea config edit")
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Bitidea from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'bitidea setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     bitidea config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                try:
                    ...
                    print_info("  You can try manually: bitidea gateway install")
            else:
                print_info("  You can install later: bitidea gateway install")
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo bitidea gateway install --system")
                print_info("  Or run in foreground:  bitidea gateway")
        else:
            from bitidea_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   bitidea gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   bitidea gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `bitidea setup tools` and `bitidea tools` use the same flow:
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from bitidea_cli.config
...
def _load_openclaw_migration_module():
    """Load the openclaw_to_bitidea migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_bitidea", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Bitidea.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Bitidea to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Bitidea at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Bitidea at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Bitidea at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Bitidea at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Bitidea equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Bitidea config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Bitidea.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Bitidea's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(bitidea_home: Path) -> bool:
    ...
    print_info("Bitidea can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: bitidea claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=bitidea_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: bitidea claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Bitidea configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Bitidea config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Bitidea (use bitidea claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      bitidea setup           — full or quick (auto-detected)
      bitidea setup model     — just model/provider
      bitidea setup tts       — just text-to-speech
      bitidea setup terminal  — just terminal backend
      bitidea setup gateway   — just messaging platforms
      bitidea setup tools     — just tool configuration
      bitidea setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ◆ Bitidea Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ◆ Bitidea Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Bitidea Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Bitidea configured.")
        print()
        ...
        if choice == 0:
            # Quick setup
            _run_quick_setup(config, bitidea_home)
            return
        elif choice == 1:
            # Full setup — fall through to run all sections
            pass
        elif choice == 7:
            print_info("Exiting. Run 'bitidea setup' again when ready.")
            return
        elif 2 <= choice <= 6:
            ...
            if section:
                ...
                func(config)
                save_config(config)
                _print_setup_summary(config, bitidea_home)
            return
    else:
        # ── First-Time Setup ──
        print()
        ...
        migration_ran = _offer_openclaw_migration(bitidea_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice("How would you like to set up Bitidea?", [
            "Quick setup — provider, model & messaging (recommended)",
            "Full setup — configure everything",
        ], 0)

        if setup_mode == 0:
            _run_first_time_quick_setup(config, bitidea_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {bitidea_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'bitidea config edit'")
...
def _resolve_bitidea_chat_argv() -> Optional[list[str]]:
    """Resolve argv for launching ``bitidea chat`` in a fresh process."""
    bitidea_bin = shutil.which("bitidea")
    if bitidea_bin:
        return [bitidea_bin, "chat"]

    try:
        if importlib.util.find_spec("bitidea_cli") is not None:
            return [sys.executable, "-m", "bitidea_cli.main", "chat"]
    except Exception:
        pass
...
def _offer_launch_chat():
    ...
    print()
    if not prompt_yes_no("Launch bitidea chat now?", True):
        return

    chat_argv = _resolve_bitidea_chat_argv()
    if not chat_argv:
        print_info("Could not relaunch Bitidea automatically. Run 'bitidea chat' manually.")
        return

    os.execvp(chat_argv[0], chat_argv)


def _run_first_time_quick_setup(config: dict, bitidea_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'bitidea setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    bitidea setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  bitidea setup gateway")
    print()
...
def _run_quick_setup(config: dict, bitidea_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'bitidea setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Bitidea to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'bitidea setup gateway'.")
