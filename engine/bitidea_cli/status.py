"""
Status command for bitidea CLI.

Shows the status of all Bitidea Agent components.
"""
...
from bitidea_cli.auth import AuthError, resolve_provider
from bitidea_cli.colors import Colors, color
from bitidea_cli.config import get_env_path, get_env_value, get_bitidea_home, load_config
from bitidea_cli.models import provider_label
from bitidea_cli.nous_subscription import get_nous_subscription_features
from bitidea_cli.runtime_provider import resolve_requested_provider
...
def show_status(args):
    """Show status of all Bitidea Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)
    
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ◆ Bitidea Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        from bitidea_cli.auth import get_nous_auth_status, get_codex_auth_status, get_qwen_auth_status
    ...
    print(
        ...
        f"{'logged in' if nous_logged_in else 'not logged in (run: bitidea model)'}"
    )
    if nous_logged_in:
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: bitidea model)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: bitidea model)"
    ...
    if not terminal_env:
        ...
        # (bitidea status doesn't go through cli.py's config loading)
        try:
            _cfg = load_config()
    ...
    if _is_termux():
        try:
            from bitidea_cli.gateway import find_gateway_pids
        ...
        if gateway_pids:
            ...
            print("  Start with:   bitidea gateway")
        ...
        from bitidea_cli.gateway import get_launchd_label
        try:
            result = subprocess.run(
    ...
    jobs_file = get_bitidea_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_bitidea_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'bitidea doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'bitidea setup' to configure", Colors.DIM))
    print()
