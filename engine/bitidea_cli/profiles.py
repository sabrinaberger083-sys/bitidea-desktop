"""
Profile management for multiple isolated Bitidea instances.

Each profile is a fully independent BITIDEA_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.bitidea/profiles/<name>/`` by default.

The "default" profile is ``~/.bitidea`` itself — backward compatible,
zero migration needed.

Usage::

    bitidea profile create coder          # fresh profile + bundled skills
    bitidea profile create coder --clone  # also copy config, .env, SOUL.md
    bitidea profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    bitidea -p coder chat                 # or via flag
    bitidea profile use coder             # set as sticky default
    bitidea profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See bitidea_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# Directories/files to exclude when exporting the default (~/.bitidea) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "bitidea-agent",         # repo checkout (multi-GB)
...
_RESERVED_NAMES = frozenset({
    "bitidea", "default", "test", "tmp", "root", "sudo",
})

# Bitidea subcommands that cannot be used as profile names/aliases
_BITIDEA_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the bitidea root, NOT to the current BITIDEA_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where BITIDEA_HOME points outside
    ``~/.bitidea``, profiles live under ``BITIDEA_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_bitidea_home() / "profiles"


def _get_default_bitidea_home() -> Path:
    """Return the default (pre-profile) BITIDEA_HOME path.

    In standard deployments this is ``~/.bitidea``.
    In Docker/custom deployments where BITIDEA_HOME is outside ``~/.bitidea``
    (e.g. ``/opt/data``), returns BITIDEA_HOME directly.
    """
    from bitidea_constants import get_default_bitidea_root
    return get_default_bitidea_root()


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_bitidea_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name == "default":
        return  # special alias for ~/.bitidea
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its BITIDEA_HOME directory."""
    if name == "default":
        return _get_default_bitidea_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, bitidea subcommands, existing binaries in PATH.
    """
    if name in _RESERVED_NAMES:
        return f"'{name}' is a reserved name"
    if name in _BITIDEA_SUBCOMMANDS:
        return f"'{name}' conflicts with a bitidea subcommand"
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_bitidea_home()
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.bitidea)."
        )
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches BITIDEA_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "BITIDEA_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.bitidea).\n"
            "To remove everything, use: bitidea uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set BITIDEA_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("BITIDEA_HOME")
    try:
        os.environ["BITIDEA_HOME"] = str(profile_dir)
        from bitidea_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["BITIDEA_HOME"] = old_home
        elif "BITIDEA_HOME" in os.environ:
            del os.environ["BITIDEA_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.bitidea/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. "
            f"Create it with: bitidea profile create {name}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from BITIDEA_HOME.

    Returns ``"default"`` if BITIDEA_HOME is not set or points to ``~/.bitidea``.
    Returns the profile name if BITIDEA_HOME points into ``~/.bitidea/profiles/<name>``.
    Returns ``"custom"`` if BITIDEA_HOME is set to an unrecognized path.
    """
    from bitidea_constants import get_bitidea_home
    bitidea_home = get_bitidea_home()
    resolved = bitidea_home.resolve()

    default_resolved = _get_default_bitidea_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.bitidea itself — its parent is ~/ and its
        # directory name is ".bitidea", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: bitidea profile import <archive> --name <name>"
        )

    # Archives exported from the default profile have "default/" as top-level
    # dir.  Importing as "default" would target ~/.bitidea itself — disallow
    # that and guide the user toward a named profile.
    if inferred_name == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.bitidea). "
            "Specify a different name: bitidea profile import <archive> --name <name>"
        )
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for bitidea profile names."""
    return '''# Bitidea Agent profile completion
# Add to ~/.bashrc: eval "$(bitidea completion bash)"

_bitidea_profiles() {
    local profiles_dir="$HOME/.bitidea/profiles"
...
_bitidea_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_bitidea_profiles)" -- "$cur"))
        return
    fi
...
complete -F _bitidea_completion bitidea
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for bitidea profile names."""
    return '''#compdef bitidea
# Bitidea Agent profile completion
# Add to ~/.zshrc: eval "$(bitidea completion zsh)"

_hermes() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.bitidea/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.bitidea/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a BITIDEA_HOME path string.

    Called early in the CLI entry point, before any bitidea modules
    are imported, to set the BITIDEA_HOME environment variable.
    """
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: bitidea profile create {profile_name}"
        )
