#!/usr/bin/env python3
"""
Bitidea CLI - Main entry point.

Usage:
    bitidea                     # Interactive chat (default)
    bitidea chat                # Interactive chat
    bitidea gateway             # Run gateway in foreground
    bitidea gateway start       # Start gateway as service
    bitidea gateway stop        # Stop gateway service
    bitidea gateway status      # Show gateway status
    bitidea gateway install     # Install gateway service
    bitidea gateway uninstall   # Uninstall gateway service
    bitidea setup               # Interactive setup wizard
    bitidea logout              # Clear stored authentication
    bitidea status              # Show status of all components
    bitidea cron                # Manage cron jobs
    bitidea cron list           # List cron jobs
    bitidea cron status         # Check if cron scheduler is running
    bitidea doctor              # Check configuration and dependencies
    bitidea honcho setup                    # Configure Honcho AI memory integration
    bitidea honcho status                   # Show Honcho config and connection status
    bitidea honcho sessions                 # List directory → session name mappings
    bitidea honcho map <name>               # Map current directory to a session name
    bitidea honcho peer                     # Show peer names and dialectic settings
    bitidea honcho peer --user NAME         # Set user peer name
    bitidea honcho peer --ai NAME           # Set AI peer name
    bitidea honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    bitidea honcho mode                     # Show current memory mode
    bitidea honcho mode [hybrid|honcho|local]  # Set memory mode
    bitidea honcho tokens                   # Show token budget settings
    bitidea honcho tokens --context N       # Set session.context() token cap
    bitidea honcho tokens --dialectic N     # Set dialectic result char cap
    bitidea honcho identity                 # Show AI peer identity representation
    bitidea honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    bitidea honcho migrate                  # Step-by-step migration guide: OpenClaw native → Bitidea + Honcho
    bitidea version             Show version
    bitidea update              Update to latest version
    bitidea uninstall           Uninstall Bitidea Agent
    bitidea acp                 Run as an ACP server for editor integration
    bitidea sessions browse     Interactive session picker with search

    bitidea claw migrate --dry-run  # Preview migration without changes
"""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (bitidea tools, bitidea setup, bitidea model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'bitidea {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any bitidea module import.
#
# Many modules cache BITIDEA_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("BITIDEA_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.bitidea/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set BITIDEA_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # 2. If no flag, check active_profile in the bitidea root
    if profile_name is None:
        try:
            from bitidea_constants import get_default_bitidea_root
            active_path = get_default_bitidea_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set BITIDEA_HOME
    if profile_name is not None:
        try:
            from bitidea_cli.profiles import resolve_profile_env
            bitidea_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent bitidea from starting
...
from bitidea_cli.config import get_bitidea_home
from bitidea_cli.env_loader import load_bitidea_dotenv
load_bitidea_dotenv(project_env=PROJECT_ROOT / '.env')

# Initialize centralized file logging early — all `bitidea` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from bitidea_logging import setup_logging as _setup_logging
...
try:
    from bitidea_cli.config import load_config as _load_config_early
    from bitidea_constants import apply_ipv4_preference as _apply_ipv4
...
from bitidea_cli import __version__, __release_date__
...
def _has_any_provider_configured() -> bool:
    ...
    from bitidea_cli.config import get_env_path, get_bitidea_home, load_config
    from bitidea_cli.auth import get_auth_status

    # Determine whether Bitidea itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_bitidea_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    auth_file = get_bitidea_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Bitidea has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Bitidea to use their tokens.
    if _has_bitidea_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, bitidea_bin
        cli_args: the original CLI arguments (everything after 'bitidea')
    """
    import shutil
    ...
    bitidea_bin = container_info["bitidea_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f'  }}];\n'
                    f"\n"
                    f"Or run: sudo bitidea {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo bitidea {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'bitidea sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print("It looks like Bitidea isn't configured yet -- no API keys or providers found.")
        print()
        print("  Run:  bitidea setup")
        print()

        from bitidea_cli.setup import is_interactive_stdin, print_noninteractive_setup_guidance
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'bitidea setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from bitidea_cli.banner import prefetch_update_check
    ...
    if getattr(args, "source", None):
        os.environ["BITIDEA_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from bitidea_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    import subprocess
    from pathlib import Path
    from bitidea_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Bitidea?")
        print()
    ...
    session_dir = get_bitidea_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: bitidea gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  bitidea gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '◆ Bitidea Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  bitidea gateway")
        ...
        print()
        print("  Or install as a service: bitidea gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'bitidea whatsapp' to try again.")
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``bitidea model``) and the setup wizard
    ...
    from bitidea_cli.config import get_compatible_custom_providers, load_config, get_env_value
    ...
    effective_provider = (
        config_provider
        or os.getenv("BITIDEA_INFERENCE_PROVIDER")
        or "auto"
    )
    try:
    ...
    from bitidea_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    # a leftover OPENAI_BASE_URL in ~/.bitidea/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.bitidea/.env if the active provider is not 'custom'.
    ...
    """
    from bitidea_cli.config import get_env_value, save_env_value, load_config
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from bitidea_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from bitidea_cli.auth import _prompt_model_selection, _save_model_choice, deactivate_provider
    from bitidea_cli.config import get_env_value, save_env_value
    ...
    from bitidea_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from bitidea_cli.config import get_env_value, load_config, save_config, save_env_value
    from bitidea_cli.nous_subscription import prompt_enable_tool_gateway
    import argparse
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from bitidea_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from bitidea_cli.codex_models import get_codex_model_ids
    import argparse
    ...
    _codex_token = None
    # Prefer credential pool (where `bitidea auth` stores device_code tokens),
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from bitidea_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from bitidea_cli.auth import _save_model_choice, deactivate_provider
    from bitidea_cli.config import get_env_value, load_config, save_config
    ...
    from bitidea_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `bitidea model` to set a model.")
...
def _model_flow_copilot(config, current_model=""):
    ...
    from bitidea_cli.config import save_env_value, load_config, save_config
    from bitidea_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from bitidea_cli.copilot_auth import copilot_device_code_login
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Bitidea turns to `copilot --acp`.")
    print("  Bitidea currently starts its own ACP subprocess for each request.")
    print("  Bitidea uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print("  Set BITIDEA_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere.")
        return
...
def _model_flow_kimi(config, current_model=""):
    ...
    from bitidea_cli.config import get_env_value, save_env_value, load_config, save_config
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    """
    from bitidea_cli.auth import _prompt_model_selection, _save_model_choice, deactivate_provider
    from bitidea_cli.config import load_config, save_config, get_env_value, save_env_value
    from bitidea_cli.models import _PROVIDER_MODELS
    ...
    if selected:
        ...
        # Save the API key env var name so bitidea knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    from bitidea_cli.config import get_env_value, save_env_value, load_config, save_config
    from bitidea_cli.models import fetch_api_models, opencode_model_api_mode, normalize_opencode_model_id
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print(f"    Bitidea will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env.")
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from bitidea_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'bitidea model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Bitidea CLI with a provider."""
    from bitidea_cli.auth import login_command
    login_command(args)


def cmd_logout(args):
    """Clear provider authentication."""
    from bitidea_cli.auth import logout_command
    logout_command(args)


def cmd_auth(args):
    """Manage pooled credentials."""
    from bitidea_cli.auth_commands import auth_command
    auth_command(args)
...
def cmd_webhook(args):
    ...
    from bitidea_cli.webhook import webhook_command
    webhook_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from bitidea_cli.doctor import run_doctor
    run_doctor(args)


def cmd_dump(args):
    """Dump setup summary for support/debugging."""
    from bitidea_cli.dump import run_dump
    run_dump(args)
...
def cmd_config(args):
    ...
    from bitidea_cli.config import config_command
    config_command(args)


def cmd_backup(args):
    """Back up Bitidea home directory to a zip file."""
    if getattr(args, "quick", False):
        from bitidea_cli.backup import run_quick_backup
...
def cmd_import(args):
    """Restore a Bitidea backup from a zip file."""
...
def cmd_version(args):
    """Show version."""
    print(f"Bitidea Agent v{__version__} ({__release_date__})")
    ...
    try:
        ...
        from bitidea_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Bitidea Agent."""
    _require_tty("uninstall")
    from bitidea_cli.uninstall import run_uninstall
    run_uninstall(args)
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``bitidea update --gateway`` so interactive prompts (stash restore,
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``bitidea web``).
    ...
    if r1.returncode != 0:
        print(f"  {'✗' if fatal else '⚠'} Web UI npm install failed"
              + ("" if fatal else " (bitidea web will not be available)"))
        if fatal:
...
def _update_via_zip(args):
    """Update Bitidea Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = f"https://github.com/sabrinaberger083-sys/bitidea-agent/archive/refs/heads/{branch}.zip"
    
    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="bitidea-update-")
        zip_path = os.path.join(tmp_dir, f"bitidea-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to bitidea-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"bitidea-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime("bitidea-update-autostash-%Y%m%d-%H%M%S")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Bitidea behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes bitidea completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print("⚠ Local changes were restored, but Bitidea couldn't find the stash entry to drop.")
        ...
        if drop.returncode != 0:
            print("⚠ Local changes were restored, but Bitidea couldn't drop the saved stash entry.")
    ...
    print("  Review `git diff` / `git status` if Bitidea behaves unexpectedly.")
    return True

# =========================================================================
# Fork detection and upstream management for `bitidea update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/sabrinaberger083-sys/bitidea-agent.git",
    "git@github.com:sabrinaberger083-sys/bitidea-agent.git",
    "https://github.com/sabrinaberger083-sys/bitidea-agent",
    "git@github.com:sabrinaberger083-sys/bitidea-agent",
}
OFFICIAL_REPO_URL = "https://github.com/sabrinaberger083-sys/bitidea-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    from bitidea_constants import get_bitidea_home
    return (get_bitidea_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        from bitidea_constants import get_bitidea_home
        (get_bitidea_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Bitidea repository.")
        print("  This means you may miss updates from sabrinaberger083-sys/bitidea-agent.")
        print()
        try:
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print("  ✓ Added upstream: https://github.com/sabrinaberger083-sys/bitidea-agent.git")
                has_upstream = True
            else:
                print("  ✗ Failed to add upstream remote. Skipping upstream sync.")
                return
        else:
            print("  Skipped. Run 'git remote add upstream https://github.com/sabrinaberger083-sys/bitidea-agent.git' to add later.")
...
def _invalidate_update_cache():
    ...
    ``bitidea update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from bitidea_constants import get_default_bitidea_root
    default_home = get_default_bitidea_root()
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "bitidea-agent[matrix]" or "package-name[extra]".
...
def cmd_update(args):
    """Update Bitidea Agent to the latest version."""
    import shutil
    from bitidea_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Bitidea Agent")
        return
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print("  curl -fsSL https://raw.githubusercontent.com/sabrinaberger083-sys/bitidea-agent/main/scripts/install.sh | bash")
            sys.exit(1)
    ...
    try:
        ...
        # the old bytecode (e.g. get_bitidea_home added to bitidea_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}")
        ...
        # modules in this process.  Reload bitidea_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_bitidea_home() added since the last release.
        try:
            import importlib
            import bitidea_constants as _hc
        ...
        try:
            from bitidea_cli.profiles import list_profiles, get_active_profile_name, seed_profile_skills
        ...
        if needs_migration:
            ...
            if gateway_mode:
                ...
                print("    Run 'bitidea config migrate' later to apply any new config/env options.")
                response = "n"
            else:
                try:
            ...
            if response in ('', 'y', 'yes'):
                ...
                if gateway_mode and missing_env:
                    print("  ℹ API keys require manual entry: bitidea config migrate")
            else:
                print()
                print("Skipped. Run 'bitidea config migrate' later to configure.")
        else:
        ...
        # When running as ``bitidea update --gateway`` (spawned by the gateway's
        # /update command), this process lives inside the gateway's systemd
        # cgroup.  ``systemctl restart bitidea-gateway`` kills everything in the
        ...
        if gateway_mode:
            _exit_code_path = get_bitidea_home() / ".update_exit_code"
            try:
        ...
        try:
            from bitidea_cli.gateway import (
            ...
            # Discover all bitidea-gateway* units (default + profiles)
            if supports_systemd_services():
                ...
                for scope, scope_cmd in [("user", ["systemctl", "--user"]), ("system", ["systemctl"])]:
                    try:
                        result = subprocess.run(
                            scope_cmd + ["list-units", "bitidea-gateway*", "--plain", "--no-legend", "--no-pager"],
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            if not parts:
                                continue
                            unit = parts[0]  # e.g. bitidea-gateway.service or bitidea-gateway-coder.service
            ...
            if is_macos():
                try:
                    from bitidea_cli.gateway import launchd_restart, get_launchd_label, get_launchd_plist_path
            ...
            if restarted_services or killed_pids:
                ...
                if killed_pids:
                    ...
                    print("    Restart manually: bitidea gateway run")
                    # Also restart for each profile if needed
                    if len(killed_pids) > 1:
                        print("    (or: bitidea -p <profile> gateway run  for each profile)")
        ...
        print("  bitidea model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``bitidea -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from bitidea_constants import display_bitidea_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `bitidea profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_bitidea_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → bitidea -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.bitidea)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(f"  Choose a custom alias:  bitidea profile alias {name} --name <custom>")
                    print(f"  Or access via flag:     bitidea -p {name} chat")
                else:
        ...
        name = args.profile_name
        from bitidea_cli.profiles import get_profile_dir, profile_exists, _read_config_model, _check_gateway_running, _count_skills
        ...
        from bitidea_cli.profiles import profile_exists
        if not profile_exists(name):
            print(f"Error: Profile '{name}' does not exist.")
            sys.exit(1)
        ...
        from bitidea_cli.profiles import rename_profile
        try:
        ...
        from bitidea_cli.profiles import export_profile
        name = args.profile_name
        ...
        from bitidea_cli.profiles import import_profile
        try:
...
def cmd_dashboard(args):
    ...
    try:
        ...
        print("Install them with:  pip install bitidea-agent[web]")
        sys.exit(1)

    if not _build_web_ui(PROJECT_ROOT / "web", fatal=True):
        sys.exit(1)

    from bitidea_cli.web_server import start_server
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args, parser=None):
    ...
    from bitidea_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter Bitidea log files."""
    from bitidea_cli.logs import tail_log, list_logs
...
def main():
    """Main entry point for bitidea CLI."""
    parser = argparse.ArgumentParser(
        prog="bitidea",
        description="Bitidea Agent - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    bitidea                        Start interactive chat
    bitidea chat -q "Hello"        Single query mode
    bitidea -c                     Resume the most recent session
    bitidea -c "my project"        Resume a session by name (latest in lineage)
    bitidea --resume <session_id>  Resume a specific session by ID
    bitidea setup                  Run setup wizard
    bitidea logout                 Clear stored authentication
    bitidea auth add <provider>    Add a pooled credential
    bitidea auth list              List pooled credentials
    bitidea auth remove <p> <t>    Remove pooled credential by index, id, or label
    bitidea auth reset <provider>  Clear exhaustion status for a provider
    bitidea model                  Select default model
    bitidea config                 View configuration
    bitidea config edit            Edit config in $EDITOR
    bitidea config set model gpt-4 Set a config value
    bitidea gateway                Run messaging gateway
    bitidea -s bitidea-agent-dev,github-auth
    bitidea -w                     Start in isolated git worktree
    bitidea gateway install        Install gateway background service
    bitidea sessions list          List past sessions
    bitidea sessions browse        Interactive session picker
    bitidea sessions rename ID T   Rename/title a session
    bitidea logs                   View agent.log (last 50 lines)
    bitidea logs -f                Follow agent.log in real time
    bitidea logs errors            View errors.log
    bitidea logs --since 1h        Lines from the last hour
    bitidea debug share             Upload debug report for support
    bitidea update                 Update to latest version

For more help on a command:
    bitidea <command> --help
"""
    )
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Bitidea Agent"
    )
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: bitidea-cli)"
    )
    model_parser.add_argument(
        "--scope",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Bitidea Agent with an interactive wizard. "
                    "Run a specific section: bitidea setup model|tts|terminal|gateway|tools|agent"
    )
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Bitidea CLI"
    )
    ...
    login_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use (default: bitidea-cli)"
    )
    login_parser.add_argument(
        "--scope",
        default=None,
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Bitidea Agent components"
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Bitidea Agent setup"
    )
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Bitidea setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Bitidea Agent. Use 'bitidea debug share' to "
                    "upload a debug report (system info + recent logs) to a paste "
                    "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    bitidea debug share              Upload debug report and print URL
    bitidea debug share --lines 500  Include more log lines
    bitidea debug share --expire 30  Keep paste for 30 days
    bitidea debug share --local      Print report locally (no upload)
    bitidea debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'bitidea debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Bitidea home directory to a zip file",
        description="Create a zip archive of your entire Bitidea configuration, "
                    "skills, sessions, and data (excludes the bitidea-agent codebase). "
                    "Use --quick for a fast snapshot of just critical state files."
    )
    backup_parser.add_argument(
        "-o", "--output",
        help="Output path for the zip file (default: ~/bitidea-backup-<timestamp>.zip)"
    )
    backup_parser.add_argument(
        "-q", "--quick",
        action="store_true",
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Bitidea backup from a zip file",
        description="Extract a previously created Bitidea backup into your "
                    "Bitidea home directory, restoring configuration, skills, "
                    "sessions, and data"
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Bitidea Agent configuration"
    )
    ...
    def cmd_pairing(args):
        from bitidea_cli.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, 'skills_action', None) == 'config':
            ...
            from bitidea_cli.skills_config import skills_command as skills_config_command
            skills_config_command(args)
        else:
            from bitidea_cli.skills_hub import skills_command
            skills_command(args)
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/bitidea-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
    ...
    def cmd_plugins(args):
        from bitidea_cli.plugins_cmd import plugins_command
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            if not existing:
                print(f"\n  Nothing to reset — no memory files found in {display_bitidea_home()}/memories/\n")
                return
            ...
            print(f"  Files were in: {display_bitidea_home()}/memories/\n")
        else:
            from bitidea_cli.memory_setup import memory_command
            memory_command(args)
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'bitidea tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # bitidea tools list [--platform cli]
    ...
    # bitidea tools disable <name...> [--platform cli]
    ...
    # bitidea tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            ...
            _require_tty("tools")
            from bitidea_cli.tools_config import tools_command
            tools_command(args)
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Bitidea as an MCP server",
        description=(
            "Manage MCP server connections and run Bitidea as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'bitidea mcp add' to connect to a new server, or\n"
            "'bitidea mcp serve' to expose Bitidea conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Bitidea as an MCP server (expose conversations to other agents)",
    )
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch bitidea --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            import shutil
            bitidea_bin = shutil.which("bitidea")
            if bitidea_bin:
                os.execvp(bitidea_bin, ["bitidea", "--resume", selected_id])
            else:
                # Fallback: re-invoke via python -m
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "bitidea_cli.main", "--resume", selected_id],
                )
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Bitidea"
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Bitidea",
    ...
    def cmd_claw(args):
        from bitidea_cli.claw import claw_command
        claw_command(args)
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Bitidea Agent to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Bitidea Agent",
        description="Remove Bitidea Agent from your system. Can keep configs/data for reinstall."
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Bitidea Agent as an ACP (Agent Client Protocol) server",
        description="Start Bitidea Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )

    def cmd_acp(args):
        """Launch Bitidea Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Bitidea instances",
    )
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Bitidea Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Bitidea log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    bitidea logs                    Show last 50 lines of agent.log
    bitidea logs -f                 Follow agent.log in real time
    bitidea logs errors             Show last 50 lines of errors.log
    bitidea logs gateway -n 100     Show last 100 lines of gateway.log
    bitidea logs --level WARNING    Only show WARNING and above
    bitidea logs --session abc123   Filter by session ID
    bitidea logs --component tools  Only show tool-related lines
    bitidea logs --since 1h         Lines from the last hour
    bitidea logs --since 30m -f     Follow, starting from 30 min ago
    bitidea logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``bitidea -c Pokemon Agent Dev`` → ``bitidea -c 'Pokemon Agent Dev'``
    ...
    from bitidea_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'bitidea -c model' where 'model' is consumed as the
