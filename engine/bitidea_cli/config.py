"""
Configuration management for Bitidea Agent.

Config files are stored in ~/.bitidea/ for easy access:
- ~/.bitidea/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.bitidea/.env         - API keys and secrets

This module provides:
- bitidea config          - Show current configuration
- bitidea config edit     - Open config in editor
- bitidea config set      - Set a specific value
- bitidea config wizard   - Re-run setup wizard
"""
...
from bitidea_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("BITIDEA_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_bitidea_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Bitidea is running in package-manager-managed mode.

    Two signals: the BITIDEA_MANAGED env var (set by the systemd service),
    or a .managed marker file in BITIDEA_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade bitidea-agent"
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "bitidea update"


def format_managed_message(action: str = "modify this Bitidea installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("BITIDEA_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Bitidea installation is managed by NixOS "
            f"(BITIDEA_MANAGED={env_hint}).\n"
            "Edit services.bitidea-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Bitidea installation is managed by Homebrew "
            f"(BITIDEA_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade bitidea-agent"
        )

    return (
        f"Cannot {action}: this Bitidea installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Bitidea."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from BITIDEA_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, bitidea_bin
    or None if container mode is not active, we're already inside the
    container, or BITIDEA_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("BITIDEA_DEV") == "1":
        return None
    ...
    container_mode_file = get_bitidea_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "bitidea-agent")
    exec_user = info.get("exec_user", "bitidea")
    bitidea_bin = info.get("bitidea_bin", "/data/current-package/bin/bitidea")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "bitidea_bin": bitidea_bin,
    }
...
# Re-export from bitidea_constants — canonical definition lives there.
from bitidea_constants import get_bitidea_home  # noqa: F811,E402

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_bitidea_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_bitidea_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the bitidea group can
    share state with the gateway service.

    The mode can be overridden via the BITIDEA_HOME_MODE environment variable
    (e.g. BITIDEA_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse BITIDEA_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("BITIDEA_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Bitidea runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("BITIDEA_CONTAINER") or os.environ.get("BITIDEA_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set BITIDEA_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into BITIDEA_HOME if the user doesn't have one yet."""
...
def ensure_bitidea_home():
    """Ensure ~/.bitidea directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_bitidea_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_bitidea_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"BITIDEA_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "terminal": {
        ...
        # docker_env lets you specify exact key-value pairs — useful when Bitidea
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Bitidea sends a stable profile-scoped userId to Camofox
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.bitidea/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # always goes to ~/.bitidea/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
    },

    # Honcho AI-native memory -- reads ~/.honcho/config.json as single source of truth.
    # This section is only needed for bitidea-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "◆ *Bitidea Agent*" header.
    ...
    # Logging — controls file logging to ~/.bitidea/logs/.
...
OPTIONAL_ENV_VARS = {
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Bitidea auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @bitidea:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. BITIDEA_BOT)",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'bitidea-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Bitidea API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Bitidea API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Bitidea API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # BITIDEA_TOOL_PROGRESS and BITIDEA_TOOL_PROGRESS_MODE are deprecated —
    # now configured via display.tool_progress in config.yaml (off|new|all|verbose).
    # Gateway falls back to these env vars for backward compatibility.
    "BITIDEA_TOOL_PROGRESS": {
    ...
    "BITIDEA_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.bitidea.config`` entries, then checks
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'bitidea setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Bitidea won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'bitidea doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("BITIDEA_HOME", "~/.bitidea")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("BITIDEA_TOOL_PROGRESS")
            old_mode = get_env_value("BITIDEA_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from BITIDEA_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from BITIDEA_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("BITIDEA_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from BITIDEA_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: bitidea config set <key> <value>")
    ...
    # metadata.bitidea.config in their SKILL.md frontmatter.
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.bitidea/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.bitidea/config.yaml."""
    import copy
    ensure_bitidea_home()
    config_path = get_config_path()
...
#   openai-codex (OAuth — bitidea auth) — OpenAI Codex
#   nous         (OAuth — bitidea auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.bitidea/config.yaml."""
    if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.bitidea/.env.
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Bitidea env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.bitidea/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.bitidea/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.bitidea/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.bitidea/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Bitidea — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Bitidea vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.bitidea/.env or environment."""
...
def show_config():
    ...
    print(color("│              ◆ Bitidea Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  bitidea config edit     # Edit config file", Colors.DIM))
    print(color("  bitidea config set <key> <value>", Colors.DIM))
    print(color("  bitidea setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: bitidea config set <key> <value>")
            print()
            print("Examples:")
            print("  bitidea config set model anthropic/claude-sonnet-4")
            print("  bitidea config set terminal.backend docker")
            print("  bitidea config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'bitidea config migrate' to add them")
        ...
        print("  bitidea config           Show current configuration")
        print("  bitidea config edit      Open config in editor")
        print("  bitidea config set <key> <value>   Set a config value")
        print("  bitidea config check     Check for missing/outdated config")
        print("  bitidea config migrate   Update config with new options")
        print("  bitidea config path      Show config file path")
        print("  bitidea config env-path  Show .env file path")
        sys.exit(1)
