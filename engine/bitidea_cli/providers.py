"""
Single source of truth for provider identity in Bitidea Agent.
...
2. **Bitidea overlays** — transport type, auth patterns, aggregator flags,
...
# -- Bitidea overlay ----------------------------------------------------------
# Bitidea-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class BitideaOverlay:
    """Bitidea-specific provider metadata layered on top of models.dev."""
...
BITIDEA_OVERLAYS: Dict[str, BitideaOverlay] = {
    "openrouter": BitideaOverlay(
    ...
    "openai-codex": BitideaOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "qwen-oauth": BitideaOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="BITIDEA_QWEN_BASE_URL",
    ),
    "copilot-acp": BitideaOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": BitideaOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": BitideaOverlay(
    ...
    "kimi-for-coding": BitideaOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "minimax": BitideaOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-cn": BitideaOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": BitideaOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": BitideaOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "vercel": BitideaOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ),
    "opencode": BitideaOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": BitideaOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "xiaomi": BitideaOverlay(
        transport="openai_chat",
        base_url_env_var="XIAOMI_BASE_URL",
    ),
    "arcee": BitideaOverlay(
        transport="openai_chat",
        base_url_override="https://api.arcee.ai/api/v1",
        base_url_env_var="ARCEE_BASE_URL",
    ),
    "ollama-cloud": BitideaOverlay(
        transport="openai_chat",
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "bitidea", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Bitidea overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Bitidea overlay
    ...
    overlay = BITIDEA_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + bitidea extra
    ...
    if overlay is not None:
        # Bitidea-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in BITIDEA_OVERLAYS
    if provider == "bedrock":
