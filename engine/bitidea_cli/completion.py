"""Shell completion script generation for bitidea CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_bitidea_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Bitidea Agent bash completion
# Add to ~/.bashrc:
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef bitidea
# Bitidea Agent zsh completion
# Add to ~/.zshrc:
...
_hermes() {{
    ...
    _arguments -C \\
        ...
        '(-p --profile){{-p,--profile}}[Profile name]:profile:_bitidea_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'bitidea command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Bitidea Agent fish completion",
        "# Add to your config:",
        "#   bitidea completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __bitidea_profiles",
        "    echo default",
        "    if test -d $HOME/.bitidea/profiles",
        "        ls $HOME/.bitidea/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        "# Disable file completion by default",
        "complete -c bitidea -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c bitidea -f -s p -l profile"
        " -d 'Profile name' -xa '(__bitidea_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__bitidea_profiles)' -d 'Profile name'"
                )
