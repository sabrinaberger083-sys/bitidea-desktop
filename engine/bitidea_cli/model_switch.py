...
- ``bitidea_cli.providers``        -- canonical provider identity + overlays
- ``bitidea_cli.model_normalize``  -- per-provider name formatting
...
from bitidea_cli.providers import (
...
_HERMES_MODEL_WARNING = (
    ...
    "for use with Bitidea Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research Hermes 3 / Hermes 4 chat families.
# The previous substring check (`"bitidea" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``hermes-brain:qwen3-14b-ctx16k`` that just
# happen to carry "bitidea" in their tag but are fully tool-capable.
#
...
_NOUS_BITIDEA_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])hermes[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)
...
def is_nous_hermes_non_agentic(model_name: str) -> bool:
    ...
    if not model_name:
        return False
    return bool(_NOUS_BITIDEA_NON_AGENTIC_RE.search(model_name))
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'bitidea model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'bitidea doctor' — config issues detected:"
    ...
    bitidea_warn = _check_hermes_model_warning(new_model)
    if bitidea_warn:
        warnings.append(bitidea_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from bitidea_cli/models.py (OPENROUTER_MODELS,
    ...
    from bitidea_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS
    ...
    # Build curated model lists keyed by bitidea provider ID
    ...
    # --- 1. Check Bitidea-mapped providers ---
    for bitidea_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(bitidea_id)
        ...
        model_ids = curated.get(bitidea_id, [])
        total = len(model_ids)
    ...
    # --- 2. Check Bitidea-only providers (nous, openai-codex, copilot, opencode-go) ---
    from bitidea_cli.providers import BITIDEA_OVERLAYS
    from bitidea_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Bitidea provider ID.
    # BITIDEA_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Bitidea IDs ("copilot").
    _mdev_to_bitidea = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in BITIDEA_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Bitidea slug — e.g. "github-copilot" → "copilot"
        bitidea_slug = _mdev_to_bitidea.get(pid, pid)
        if bitidea_slug.lower() in seen_slugs:
            continue
        ...
        if not has_creds:
            try:
                from bitidea_cli.auth import _load_auth_store
                store = _load_auth_store()
                providers_store = store.get("providers", {})
                pool_store = store.get("credential_pool", {})
                if store and (
                    pid in providers_store or bitidea_slug in providers_store
                    or pid in pool_store or bitidea_slug in pool_store
                ):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(bitidea_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", bitidea_slug, exc)
        ...
        # configured.
        if not has_creds and bitidea_slug == "anthropic":
            try:
                ...
                if (bitidea_creds and bitidea_creds.get("accessToken")) or \
        ...
        # Use curated list — look up by Bitidea slug, fall back to overlay key
        model_ids = curated.get(bitidea_slug, []) or curated.get(pid, [])
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": bitidea_slug,
            "name": get_label(bitidea_slug),
            "is_current": bitidea_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "bitidea",
        ...
        seen_slugs.add(bitidea_slug.lower())

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or BITIDEA_OVERLAYS (keeps /model in sync
    # with `bitidea model`).
    try:
        from bitidea_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
