"""``bitidea logs`` — view and filter Bitidea log files.
...
Usage examples::
    ...
    bitidea logs                    # last 50 lines of agent.log
    bitidea logs -f                 # follow agent.log in real time
    bitidea logs errors             # last 50 lines of errors.log
    bitidea logs gateway -n 100    # last 100 lines of gateway.log
    bitidea logs --level WARNING    # only WARNING+ lines
    bitidea logs --session abc123   # filter by session ID substring
    bitidea logs --component tools  # only tool-related lines
    bitidea logs --since 1h         # lines from the last hour
    bitidea logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_bitidea_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Bitidea runs — try 'bitidea chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_bitidea_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_bitidea_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_bitidea_home()}/logs/")
        return

    print(f"Log files in {display_bitidea_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'bitidea chat' to generate logs)")
