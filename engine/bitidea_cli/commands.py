"""Slash command definitions and autocomplete for the Bitidea CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Bitidea config/state", "Session",
    ...
    CommandDef("update", "Update Bitidea Agent to the latest version", "Info",
               gateway_only=True),
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``bitidea skills
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /bitidea handler.

    Maps both canonical names and aliases so /bitidea bg do stuff works
    the same as /bitidea background do stuff.
    """
    overrides = _resolve_config_gates()
...
class SlashCommandCompleter(Completer):
    ...
    def get_completions(self, document, complete_event):
        ...
        try:
            from bitidea_cli.plugins import get_plugin_commands
