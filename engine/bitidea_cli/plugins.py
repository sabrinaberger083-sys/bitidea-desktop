"""
Bitidea Plugin System
====================

Discovers, loads, and manages plugins from three sources:

1. **User plugins**   – ``~/.bitidea/plugins/<name>/``
2. **Project plugins** – ``./.bitidea/plugins/<name>/`` (opt-in via
   ``BITIDEA_ENABLE_PROJECT_PLUGINS``)
3. **Pip plugins**     – packages that expose the ``bitidea_agent.plugins``
   entry-point group.
...
ENTRY_POINTS_GROUP = "bitidea_agent.plugins"

_NS_PARENT = "bitidea_plugins"
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``bitidea honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``bitidea <subcommand>``
    ...
    def register_skill(
        ...
        ``~/.bitidea/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self) -> None:
        ...
        # 1. User plugins (~/.bitidea/plugins/)
        user_dir = get_bitidea_home() / "plugins"
        manifests.extend(self._scan_directory(user_dir, source="user"))

        # 2. Project plugins (./.bitidea/plugins/)
        if _env_enabled("BITIDEA_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".bitidea" / "plugins"
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``bitidea_plugins.<name>``."""
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``bitidea tools`` TUI so plugin-provided toolsets appear
