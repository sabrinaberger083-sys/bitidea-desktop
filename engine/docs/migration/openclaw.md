# Migrating from OpenClaw to Bitidea Agent

This guide covers how to import your OpenClaw settings, memories, skills, and API keys into Bitidea Agent.

## Three Ways to Migrate

### 1. Automatic (during first-time setup)

When you run `bitidea setup` for the first time and Bitidea detects `~/.openclaw`, it automatically offers to import your OpenClaw data before configuration begins. Just accept the prompt and everything is handled for you.

### 2. CLI Command (quick, scriptable)

```bash
bitidea claw migrate                      # Preview then migrate (always shows preview first)
bitidea claw migrate --dry-run            # Preview only, no changes
bitidea claw migrate --preset user-data   # Migrate without API keys/secrets
bitidea claw migrate --yes                # Skip confirmation prompt
```
...
```
> Migrate my OpenClaw setup to Bitidea
```
...
|------|--------|-------------|
| SOUL.md | `~/.openclaw/workspace/SOUL.md` | `~/.bitidea/SOUL.md` |
| Memory entries | `~/.openclaw/workspace/MEMORY.md` | `~/.bitidea/memories/MEMORY.md` |
| User profile | `~/.openclaw/workspace/USER.md` | `~/.bitidea/memories/USER.md` |
| Skills | `~/.openclaw/workspace/skills/` | `~/.bitidea/skills/openclaw-imports/` |
| Command allowlist | `~/.openclaw/workspace/exec_approval_patterns.yaml` | Merged into `~/.bitidea/config.yaml` |
| Messaging settings | `~/.openclaw/config.yaml` (TELEGRAM_ALLOWED_USERS, MESSAGING_CWD) | `~/.bitidea/.env` |
| TTS assets | `~/.openclaw/workspace/tts/` | `~/.bitidea/tts/` |
...
| Item | Source | Destination |
|------|--------|-------------|
| Telegram bot token | `openclaw.json` channels config | `~/.bitidea/.env` |
| OpenRouter API key | `.env`, `openclaw.json`, or `openclaw.json["env"]` | `~/.bitidea/.env` |
| OpenAI API key | `.env`, `openclaw.json`, or `openclaw.json["env"]` | `~/.bitidea/.env` |
| Anthropic API key | `.env`, `openclaw.json`, or `openclaw.json["env"]` | `~/.bitidea/.env` |
| ElevenLabs API key | `.env`, `openclaw.json`, or `openclaw.json["env"]` | `~/.bitidea/.env` |
...
- **TTS provider**: OpenClaw renamed "edge" to "microsoft" — both are recognized and mapped to Bitidea' "edge"
...
By default, the migration **will not overwrite** existing Bitidea data:

- **SOUL.md** — skipped if one already exists in `~/.bitidea/`
...
- **API keys** — skipped if the key is already set in `~/.bitidea/.env`
...
For executed migrations, the full report is saved to `~/.bitidea/migration/openclaw/<timestamp>/`.

## Post-Migration Notes

- **Skills require a new session** — imported skills take effect after restarting your agent or starting a new chat.
- **WhatsApp requires re-pairing** — WhatsApp uses QR-code pairing, not token-based auth. Run `bitidea whatsapp` to pair.
- **Archive cleanup** — after migration, you'll be offered to rename `~/.openclaw/` to `.openclaw.pre-migration/` to prevent state confusion. You can also run `bitidea claw cleanup` later.
...
```bash
bitidea claw migrate --source /path/to/.openclaw
```

### "Migration script not found"
The migration script ships with Bitidea Agent. If you installed via pip (not git clone), the `optional-skills/` directory may not be present. Install the skill from the Skills Hub:
```bash
bitidea skills install openclaw-migration
```

### Memory overflow
If your OpenClaw MEMORY.md or USER.md exceeds Bitidea' character limits, excess entries are exported to an overflow file in the migration report directory. You can manually review and add the most important ones.
...
The migration checks all four. If keys use `source: "file"` or `source: "exec"` SecretRefs, they can't be resolved automatically — add them via `bitidea config set`.
