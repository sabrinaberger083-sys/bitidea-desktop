...
Comparison of Bitidea Agent vs. openclaw-honcho — and a porting spec for bringing Bitidea patterns into other Honcho integrations.

---

## Overview

Two independent Honcho integrations have been built for two different agent runtimes: **Bitidea Agent** (Python, baked into the runner) and **openclaw-honcho** (TypeScript plugin via hook/tool API). Both use the same Honcho peer paradigm — dual peer model, `session.context()`, `peer.chat()` — but they made different tradeoffs at every layer.

This document maps those tradeoffs and defines a porting spec: a set of Bitidea-originated patterns, each stated as an integration-agnostic interface, that any Honcho integration can adopt regardless of runtime or language.

> **Scope** Both integrations work correctly today. This spec is about the delta — patterns in Bitidea that are worth propagating and patterns in openclaw-honcho that Bitidea should eventually adopt. The spec is additive, not prescriptive.

---

## Architecture comparison

### Bitidea: baked-in runner
...
| Dimension | Bitidea Agent | openclaw-honcho |
|---|---|---|
...
| **AI peer name in identity** | Replaces "Bitidea Agent" in DEFAULT_AGENT_IDENTITY when configured. | Not implemented. |
...
Six patterns from Bitidea are worth adopting in any Honcho integration. Each is described as an integration-agnostic interface.
...
**openclaw-honcho contributes back (Bitidea should adopt):**
...
- [ ] **honcho mode / sessions / map** — CLI parity with Bitidea
...
Greenfield integration. Start from openclaw-honcho's architecture and apply all Bitidea patterns from day one.
