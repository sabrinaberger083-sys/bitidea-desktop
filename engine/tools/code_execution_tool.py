...
Lets the LLM write a Python script that calls Bitidea tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `bitidea_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `bitidea_tools.py` with file-based RPC stubs
...
# ---------------------------------------------------------------------------
# bitidea_tools.py code generator
# ---------------------------------------------------------------------------
...
def generate_bitidea_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the bitidea_tools.py stub module.
...
"""Auto-generated Bitidea tools RPC stubs."""
...
def _connect():
    ...
    if _sock is None:
        ...
        _sock.connect(os.environ["BITIDEA_RPC_SOCKET"])
...
"""Auto-generated Bitidea tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, time

_RPC_DIR = os.environ.get("BITIDEA_RPC_DIR") or os.path.join(tempfile.gettempdir(), "bitidea_rpc")
_seq = 0
''' + _COMMON_HELPERS + '''\
...
def _execute_remote(
    ...
    The script and the generated bitidea_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/bitidea_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_bitidea_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/bitidea_tools.py", tools_src)
        ...
        env_prefix = (
            f"BITIDEA_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("BITIDEA_TIMEZONE", "").strip()
        if tz:
            env_prefix += f" TZ={tz}"
...
def execute_code(
    ...
    # --- Set up temp directory with bitidea_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="bitidea_sandbox_")
    # Use /tmp on macOS to avoid the long /var/folders/... path that pushes
    # Unix domain socket paths past the 104-byte macOS AF_UNIX limit.
    # On Linux, tempfile.gettempdir() already returns /tmp.
    _sock_tmpdir = "/tmp" if sys.platform == "darwin" else tempfile.gettempdir()
    sock_path = os.path.join(_sock_tmpdir, f"bitidea_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated bitidea_tools module
        # sandbox_tools is already the correct set (intersection with session
        # tools, or SANDBOX_ALLOWED_TOOLS as fallback — see lines above).
        tools_src = generate_bitidea_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "bitidea_tools.py"), "w") as f:
            f.write(tools_src)
        ...
        for k, v in os.environ.items():
            ...
            if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
                child_env[k] = v
        child_env["BITIDEA_RPC_SOCKET"] = sock_path
        child_env["PYTHONDONTWRITEBYTECODE"] = "1"
        # Ensure the bitidea-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.
        _bitidea_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        child_env["PYTHONPATH"] = _bitidea_root + (os.pathsep + _existing_pp if _existing_pp else "")
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # BITIDEA_TIMEZONE is an internal Bitidea setting and must not leak
        # into child processes.
        _tz_name = os.getenv("BITIDEA_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("BITIDEA_TIMEZONE", None)

        # Per-profile HOME isolation: redirect system tool configs into
        # {BITIDEA_HOME}/home/ when that directory exists.
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.bitidea/.env')).
...
def build_execute_code_schema(enabled_sandbox_tools: set = None) -> dict:
    ...
    When tools are disabled via ``bitidea tools`` (e.g. web is turned off),
    ...
    description = (
        "Run a Python script that can call Bitidea tools programmatically. "
        ...
        f"Available via `from bitidea_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        "Limits: 5-minute timeout, 50KB stdout cap, max 50 tool calls per script. "
        "terminal() is foreground-only (no background or pty).\n\n"
        "Print your final result to stdout. Use Python stdlib (json, re, math, csv, "
        "datetime, collections, etc.) for processing between tool calls.\n\n"
        "Also available (no import needed — built into bitidea_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from bitidea_tools import {import_str}` "
