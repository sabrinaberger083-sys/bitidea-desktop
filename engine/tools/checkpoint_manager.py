...
Architecture:
    ~/.bitidea/checkpoints/{sha256(abs_dir)[:16]}/   — shadow git repo
        HEAD, refs/, objects/                        — standard git internals
        BITIDEA_WORKDIR                               — original dir path
...
CHECKPOINT_BASE = get_bitidea_home() / "checkpoints"
...
_GIT_TIMEOUT: int = max(10, min(60, int(os.getenv("BITIDEA_CHECKPOINT_TIMEOUT", "30"))))
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    _run_git(["config", "user.email", "bitidea@local"], shadow_repo, working_dir)
    _run_git(["config", "user.name", "Bitidea Checkpoint"], shadow_repo, working_dir)
    ...
    (shadow_repo / "BITIDEA_WORKDIR").write_text(
