...
Then set ``CAMOFOX_URL=http://localhost:9377`` in ``~/.bitidea/.env``.
"""
...
def _managed_persistence_enabled() -> bool:
    """Return whether Bitidea-managed persistence is enabled for Camofox.
...
def _get_session(task_id: Optional[str]) -> Dict[str, Any]:
    ...
    derived from the Bitidea profile so the Camofox server can map it
...
def camofox_vision(question: str, annotate: bool = False,
    ...
    try:
        ...
        screenshots_dir = get_bitidea_home() / "browser_screenshots"
