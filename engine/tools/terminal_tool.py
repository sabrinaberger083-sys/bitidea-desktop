...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or Bitidea exit
...
# display_bitidea_home imported lazily at call site (stale-module safety during bitidea update)
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of bitidea directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "bitidea-*")):
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (BITIDEA_INTERACTIVE=1).
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    If SUDO_PASSWORD is not set and in interactive mode (BITIDEA_INTERACTIVE=1):
    ...
    if not has_configured_password and not sudo_password and os.getenv("BITIDEA_INTERACTIVE"):
...
def _parse_env_var(name: str, default: str, converter=int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.bitidea/.env or environment variables."
        )
        ...
        if modal_state["selected_backend"] != "direct":
            if modal_state["managed_mode_blocked"]:
                raise ValueError(
                    ...
                    "Modal credentials/config were found. Log in with `bitidea model` or "
        ...
        if background:
            ...
            try:
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    _gw_platform = _gse("BITIDEA_SESSION_PLATFORM", "")
                    if _gw_platform:
                        _gw_chat_id = _gse("BITIDEA_SESSION_CHAT_ID", "")
                        _gw_thread_id = _gse("BITIDEA_SESSION_THREAD_ID", "")
                        _gw_user_id = _gse("BITIDEA_SESSION_USER_ID", "")
                        _gw_user_name = _gse("BITIDEA_SESSION_USER_NAME", "")
            ...
            if modal_state["selected_backend"] != "direct":
                if modal_state["managed_mode_blocked"]:
                    logger.error(
                        ...
                        "a paid Nous subscription is required for the Tool Gateway and no direct "
                        "Modal credentials/config were found. Log in with `bitidea model` "
