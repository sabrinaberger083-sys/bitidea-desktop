...
This module provides the glue:
    - ``BitideaTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Bitidea Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses BITIDEA_HOME so each profile gets its own OAuth tokens.
    Layout: ``BITIDEA_HOME/mcp-tokens/``
    """
    try:
        from bitidea_constants import get_bitidea_home
        base = Path(get_bitidea_home())
    except ImportError:
        base = Path(os.environ.get("BITIDEA_HOME", str(Path.home() / ".bitidea")))
...
# ---------------------------------------------------------------------------
# BitideaTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class BitideaTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        BITIDEA_HOME/mcp-tokens/<server_name>.json         -- tokens
        BITIDEA_HOME/mcp-tokens/<server_name>.client.json   -- client info
    """
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Bitidea.</p></body></html>"
            ) if code else (
...
def remove_oauth_tokens(server_name: str) -> None:
    ...
    storage = BitideaTokenStorage(server_name)
    storage.remove()
...
def build_oauth_auth(
    ...
    # --- Client metadata ---
    client_name = cfg.get("client_name", "Bitidea Agent")
    scope = cfg.get("scope")
