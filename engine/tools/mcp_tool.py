...
discovers their tools, and registers them into the bitidea-agent tool registry
so the agent can call them like any built-in tool.

Configuration is read from ~/.bitidea/config.yaml under the ``mcp_servers`` key.
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            bitidea_home = os.path.expanduser(
                os.getenv(
                    "BITIDEA_HOME", os.path.join(os.path.expanduser("~"), ".bitidea")
                )
            )
            candidates = [
                os.path.join(bitidea_home, "node", "bin", resolved_command),
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Bitidea config file.
    ...
    ``os.environ`` (which includes ``~/.bitidea/.env`` loaded at startup).
    """
    try:
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Bitidea's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Bitidea registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``bitidea tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Bitidea registry.
