...
Configuration is loaded from ~/.bitidea/config.yaml under the 'tts:' key.
...
from tools.xai_http import bitidea_xai_user_agent
...
def _get_default_output_dir() -> str:
    from bitidea_constants import get_bitidea_dir
    return str(get_bitidea_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.bitidea/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.bitidea/config.yaml.
    ...
    try:
        ...
        logger.debug("bitidea_cli.config not available, using default TTS config")
        return {}
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    # output_format when Bitidea actually needs a non-default format/override.
    ...
    response = requests.post(
        ...
        headers={
            ...
            "User-Agent": bitidea_xai_user_agent(),
        },
        json=payload,
        timeout=60,
    )
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.bitidea/config.yaml (tts: section).
    ...
    platform = get_session_env("BITIDEA_SESSION_PLATFORM", "").lower()
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run: pip install 'bitidea-agent[mistral]'"
                }, ensure_ascii=False)
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run bitidea setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_bitidea_home()}/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
