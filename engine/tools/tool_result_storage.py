...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   SANDBOX temp dir (for example /tmp/bitidea-results/{tool_use_id}.txt on
   standard Linux, or $TMPDIR/bitidea-results/{tool_use_id}.txt on Termux)
...
STORAGE_DIR = "/tmp/bitidea-results"
HEREDOC_MARKER = "BITIDEA_PERSIST_EOF"
        ...
        if callable(get_temp_dir):
            try:
                ...
                if temp_dir:
                    ...
                    return f"{temp_dir}/bitidea-results"
    return STORAGE_DIR
...
def _heredoc_marker(content: str) -> str:
    ...
    return f"BITIDEA_PERSIST_{uuid.uuid4().hex[:8]}"
