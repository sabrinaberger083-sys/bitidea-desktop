...
# All skills live in ~/.bitidea/skills/ (seeded from bundled skills/ on install).
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from BITIDEA_HOME/.env."""
    env_path = get_bitidea_home() / ".env"
...
def _is_gateway_surface() -> bool:
    if os.getenv("BITIDEA_GATEWAY_SESSION"):
        return True
    from gateway.session_context import get_session_env
    return bool(get_session_env("BITIDEA_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_bitidea_home()}/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.bitidea/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    try:
        ...
        resolved_platform = platform or os.getenv("BITIDEA_PLATFORM")
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.bitidea/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``bitidea skills`` config UI). Default False
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": f"No skills found. Skills directory created at {display_bitidea_home()}/skills/",
                },
                ensure_ascii=False,
            )
...
def _serve_plugin_skill(
    ...
    from bitidea_cli.plugins import _get_disabled_plugins, get_plugin_manager
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: bitidea plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
...
def skill_view(name: str, file_path: str = None, task_id: str = None) -> str:
    ...
    try:
        ...
        if ":" in name:
            ...
            from bitidea_cli.plugins import discover_plugins, get_plugin_manager
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.bitidea/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `bitidea skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        # Check metadata.bitidea.* first (agentskills.io convention), fall back to top-level
        bitidea_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            bitidea_meta = metadata.get("bitidea", {}) or {}

        tags = _parse_tags(bitidea_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            bitidea_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
