...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("BITIDEA_SESSION_KEY", default)

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $BITIDEA_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_BITIDEA_ENV_PATH = (
    r'(?:~\/\.bitidea/|'
    r'(?:\$home|\$\{home\})/\.bitidea/|'
    r'(?:\$bitidea_home|\$\{bitidea_home\})/)'
    r'\.env\b'
)
_SENSITIVE_WRITE_TARGET = (
    r'(?:/etc/|/dev/sd|'
    rf'{_SSH_SENSITIVE_PATH}|'
    rf'{_BITIDEA_ENV_PATH})'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bhermes\s+gateway\s+(stop|restart)\b', "stop/restart bitidea gateway (kills running agents)"),
    (r'\bhermes\s+update\b', "bitidea update (restarts gateway, kills running agents)"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart bitidea-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart bitidea-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(bitidea|gateway|cli\.py)\b', "kill bitidea/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill bitidea` but not
    # `kill -9 $(pgrep -f bitidea)` because the substitution is opaque
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    os.environ["BITIDEA_SPINNER_PAUSE"] = "1"
    try:
        while True:
            print()
        ...
        if "BITIDEA_SPINNER_PAUSE" in os.environ:
            del os.environ["BITIDEA_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if os.getenv("BITIDEA_YOLO_MODE") or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    is_cli = os.getenv("BITIDEA_INTERACTIVE")
    is_gateway = os.getenv("BITIDEA_GATEWAY_SESSION")

    if not is_cli and not is_gateway:
        return {"approved": True, "message": None}

    if is_gateway or os.getenv("BITIDEA_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if os.getenv("BITIDEA_YOLO_MODE") or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = os.getenv("BITIDEA_INTERACTIVE")
    is_gateway = os.getenv("BITIDEA_GATEWAY_SESSION")
    is_ask = os.getenv("BITIDEA_EXEC_ASK")
