...
it is automatically downloaded from GitHub releases to $BITIDEA_HOME/bin/tirith.
...
def _get_bitidea_home() -> str:
    """Return the Bitidea home directory, respecting BITIDEA_HOME env var."""
    return str(get_bitidea_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_bitidea_home(), ".tirith-install-failed")
...
def _bitidea_bin_dir() -> str:
    """Return $BITIDEA_HOME/bin, creating it if needed."""
    d = os.path.join(_get_bitidea_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $BITIDEA_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_bitidea_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $BITIDEA_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $BITIDEA_HOME/bin/tirith
    ...
    bitidea_bin = os.path.join(_bitidea_bin_dir(), "tirith")
    if os.path.isfile(bitidea_bin) and os.access(bitidea_bin, os.X_OK):
        _resolved_path = bitidea_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return bitidea_bin
