...
from tools.environments.local import _BITIDEA_PROVIDER_ENV_BLOCKLIST
...
def _load_bitidea_env_vars() -> dict[str, str]:
    """Load ~/.bitidea/.env values without failing Docker command execution."""
    try:
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``BITIDEA_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("BITIDEA_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using BITIDEA_DOCKER_BINARY override: %s", override)
        return override
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.bitidea/sandboxes/). Non-persistent
        ...
        container_name = f"bitidea-{uuid.uuid4().hex[:8]}"
        run_cmd = [
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Bitidea secret blocklist. Only implicit passthrough
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _BITIDEA_PROVIDER_ENV_BLOCKLIST)
        bitidea_env = _load_bitidea_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if value is None:
                value = bitidea_env.get(key)
