...
The PID file lives at ``{BITIDEA_HOME}/gateway.pid``.  BITIDEA_HOME defaults to
``~/.bitidea`` but can be overridden via the environment variable.  This means
separate BITIDEA_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting BITIDEA_HOME."""
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("BITIDEA_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "bitidea" / _LOCKS_DIRNAME
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Bitidea gateway."""
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different BITIDEA_HOME dirs).
    """
