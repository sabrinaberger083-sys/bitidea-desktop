...
Checklist for integrating a new messaging platform into the Bitidea gateway.
...
And add it to the `bitidea-gateway` composite:

```python
"bitidea-gateway": {
    "includes": [..., "bitidea-your-platform"]
}
```
...
## 12. Status Display (`bitidea_cli/status.py`)
...
## 13. Gateway Setup Wizard (`bitidea_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ bitidea_cli/ toolsets.py \
  --include="*.py" -l | sort -u
