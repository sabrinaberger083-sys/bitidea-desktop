# nix/nixosModules.nix — NixOS module for bitidea-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: bitidea runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $BITIDEA_HOME/.env
# and read by bitidea at startup — no container recreation needed for env changes.
#
# Tool resolution: the bitidea wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.bitidea-agent;
    bitidea-agent = inputs.self.packages.${pkgs.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-bitidea-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "bitidea-config-attrs";
      description = "Bitidea YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "bitidea-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "bitidea-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'BITIDEA_DOC_EOF'\n${value}\nHERMES_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "bitidea-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/bitidea";
    ...
    # bitidea user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "bitidea-container-entrypoint" ''
      set -eu

      BITIDEA_UID="''${BITIDEA_UID:?BITIDEA_UID must be set}"
      BITIDEA_GID="''${BITIDEA_GID:?BITIDEA_GID must be set}"

      # ── Group: ensure a group with GID=$BITIDEA_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$BITIDEA_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="bitidea"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$BITIDEA_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$BITIDEA_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$BITIDEA_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$BITIDEA_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="bitidea"
        TARGET_HOME="/home/bitidea"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$BITIDEA_UID" -g "$BITIDEA_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$BITIDEA_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$BITIDEA_UID:$BITIDEA_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure BITIDEA_HOME is owned by the target user
      if [ -n "''${BITIDEA_HOME:-}" ] && [ -d "$BITIDEA_HOME" ]; then
        chown -R "$BITIDEA_UID:$BITIDEA_GID" "$BITIDEA_HOME"
      fi
      ...
      if [ ! -f /var/lib/bitidea-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        echo "First boot: provisioning agent tools..."
        apt-get update -qq
        apt-get install -y -qq sudo nodejs npm curl
        touch /var/lib/bitidea-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/bitidea ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/bitidea
        chmod 0440 /etc/sudoers.d/bitidea
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$BITIDEA_UID" --regid="$BITIDEA_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $BITIDEA_HOME/.env.
    ...
    # Default: /var/lib/bitidea/workspace → /data/workspace.
  ...
  in {
    options.services.bitidea-agent = with lib; {
      enable = mkEnableOption "Bitidea Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = bitidea-agent;
        description = "The bitidea-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/bitidea";
        description = "State directory. Contains .bitidea/ subdir (BITIDEA_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Bitidea config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $BITIDEA_HOME/.env at activation time.
          Bitidea reads this file on every startup via load_bitidea_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $BITIDEA_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $BITIDEA_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `bitidea gateway`.";
      };
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the bitidea CLI to environment.systemPackages and export
          BITIDEA_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.bitidea symlink to the service
            stateDir. These users are automatically added to the bitidea group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.bitidea-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the bitidea CLI to system PATH and export BITIDEA_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.bitidea/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ cfg.package ];
        environment.variables.BITIDEA_HOME = "${cfg.stateDir}/.bitidea";
      })
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.bitidea-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed bitidea
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure bitidea is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.bitidea        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.bitidea/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.bitidea/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.bitidea/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.bitidea/memories 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."bitidea-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.bitidea
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.bitidea ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.bitidea ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.bitidea -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories; do
            mkdir -p "${cfg.stateDir}/.bitidea/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.bitidea/$_subdir"
            chmod 2770 "${cfg.stateDir}/.bitidea/$_subdir"
            find "${cfg.stateDir}/.bitidea/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.bitidea/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.bitidea/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.bitidea/config.yaml
            chmod 0640 ${cfg.stateDir}/.bitidea/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.bitidea/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.bitidea/.managed
          chmod 0644 ${cfg.stateDir}/.bitidea/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.bitidea/.container-mode <<'BITIDEA_CONTAINER_MODE_EOF'
# Written by NixOS activation script. Do not edit manually.
backend=${cfg.container.backend}
container_name=${containerName}
exec_user=${cfg.user}
bitidea_bin=${containerDataDir}/current-package/bin/bitidea
BITIDEA_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.bitidea/.container-mode
            chmod 0644 ${cfg.stateDir}/.bitidea/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.bitidea/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.bitidea";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.bitidea" ]; then
                  rm -f "${symlinkPath}"
                  echo "bitidea-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.bitidea -> stateDir/.bitidea for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.bitidea";
                target = "${cfg.stateDir}/.bitidea";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "bitidea-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.bitidea/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.bitidea/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.bitidea/auth.json
              fi
            ''}
          ''}
          ...
          # Bitidea reads $BITIDEA_HOME/.env at startup via load_bitidea_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.bitidea/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'BITIDEA_NIX_ENV_EOF'
${envFileContent}
BITIDEA_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.bitidea-agent = {
          description = "Bitidea Agent Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            BITIDEA_HOME = "${cfg.stateDir}/.bitidea";
            BITIDEA_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $BITIDEA_HOME/.env by the activation script. load_bitidea_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${cfg.package}/bin/bitidea"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the bitidea group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.bitidea-agent = {
          description = "Bitidea Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              BITIDEA_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              BITIDEA_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env BITIDEA_UID="$BITIDEA_UID" \
                --env BITIDEA_GID="$BITIDEA_GID" \
                --env BITIDEA_HOME=${containerDataDir}/.bitidea \
                --env BITIDEA_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/bitidea gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
