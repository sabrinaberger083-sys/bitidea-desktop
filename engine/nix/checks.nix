...
{ inputs, ... }: {
  perSystem = { pkgs, system, lib, ... }:
    let
      bitidea-agent = inputs.self.packages.${system}.default;
      ...
      configKeys = pkgs.runCommand "bitidea-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${bitideaVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = lib.optionalAttrs pkgs.stdenv.hostPlatform.isLinux {
        ...
        package-contents = pkgs.runCommand "bitidea-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${bitidea-agent}/bin/bitidea || (echo "FAIL: bitidea binary missing"; exit 1)
          test -x ${bitidea-agent}/bin/bitidea-agent || (echo "FAIL: bitidea-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${bitidea-agent}/bin/bitidea version 2>&1 | grep -qi "bitidea" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "bitidea-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in bitidea bitidea-agent bitidea-acp; do
            test -x ${bitidea-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "bitidea-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking bitidea --help ==="
          ${bitidea-agent}/bin/bitidea --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${bitidea-agent}/bin/bitidea --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "bitidea-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${bitidea-agent}/share/bitidea-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${bitidea-agent}/share/bitidea-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "BITIDEA_BUNDLED_SKILLS" ${bitidea-agent}/bin/bitidea || \
            (echo "FAIL: BITIDEA_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: BITIDEA_BUNDLED_SKILLS set in wrapper"
        ...
        # Verify BITIDEA_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "bitidea-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(BITIDEA_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking BITIDEA_MANAGED guards ==="
          check_blocked "config set" ${bitidea-agent}/bin/bitidea config set model foo
          check_blocked "config edit" ${bitidea-agent}/bin/bitidea config edit
        ...
        in pkgs.runCommand "bitidea-config-roundtrip" {
          ...
          merge_and_load() {
            local bitidea_home="$1"
            export BITIDEA_HOME="$bitidea_home"
            ${configMergeScript} ${nixSettings} "$bitidea_home/config.yaml"
            ${bitideaVenv}/bin/python3 -c '
import json, sys
