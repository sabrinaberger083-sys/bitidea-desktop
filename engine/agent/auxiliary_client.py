...
Resolution order for text tasks (auto mode):
  ...
  2. Nous Portal (~/.bitidea/auth.json active provider)
...
NOUS_EXTRA_BODY = {"tags": ["product=bitidea-agent"]}
...
_AUTH_JSON_PATH = get_bitidea_home() / "auth.json"
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.bitidea/auth.json for an active Nous provider.
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Bitidea auth store.
    ...
    try:
        from bitidea_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from bitidea_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    try:
        from bitidea_cli.models import check_nous_free_tier
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `bitidea setup` or `bitidea model` and enter a valid http(s) base URL."
        ) from exc
...
def _resolve_auto(main_runtime: Optional[Dict[str, Any]] = None) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    #    scenario where a user switches providers via `bitidea model` but the
    #    old OPENAI_BASE_URL lingers in ~/.bitidea/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: bitidea model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.bitidea/.env",
...
def resolve_provider_client(
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Nous Portal not configured (run: bitidea auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model) if async_mode
                else (client, final_model))
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: bitidea model)")
                return None, None
    ...
    try:
        from bitidea_cli.runtime_provider import _get_named_custom_provider
    ...
    try:
        ...
        logger.debug("bitidea_cli.auth not available for provider %s", provider)
        return None, None
...
def _build_call_kwargs(
    ...
    if provider == "nous" or auxiliary_is_nous:
        merged_extra.setdefault("tags", []).extend(["product=bitidea-agent"])
    if merged_extra:
