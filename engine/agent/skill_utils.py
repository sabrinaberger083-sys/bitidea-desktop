...
from bitidea_constants import get_config_path, get_skills_dir
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``BITIDEA_PLATFORM`` or
            ``BITIDEA_SESSION_PLATFORM`` env vars.  Falls back to the
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.bitidea/skills/`` are silently skipped.
    """
    config_path = get_config_path()
    if not config_path.exists():
        return []
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.bitidea/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    bitidea = metadata.get("bitidea") or {}
    if not isinstance(bitidea, dict):
        bitidea = {}
    return {
        "fallback_for_toolsets": bitidea.get("fallback_for_toolsets", []),
        "requires_toolsets": bitidea.get("requires_toolsets", []),
        "fallback_for_tools": bitidea.get("fallback_for_tools", []),
        "requires_tools": bitidea.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    bitidea = metadata.get("bitidea")
    if not isinstance(bitidea, dict):
        return []
    raw = bitidea.get("config")
    if not raw:
        return []
