...
class CredentialPool:
    ...
    def _sync_codex_entry_from_cli(self, entry: PooledCredential) -> PooledCredential:
        ...
        When the Codex CLI (or another Bitidea profile) refreshes its token,
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("bitidea_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # The Codex CLI (or another Bitidea profile) may have already
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("bitidea_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Bitidea profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # the Codex CLI or another Bitidea profile.
            if (self.provider == "openai-codex"
                    and entry.last_status == STATUS_EXHAUSTED
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Bitidea PKCE)
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_bitidea_oauth_credentials

        for source_name, creds in (
            ("bitidea_pkce", read_bitidea_oauth_credentials()),
            ("claude_code", read_claude_code_credentials()),
        ):
            if creds and creds.get("accessToken"):
                # Check if user explicitly removed this source
                try:
                    from bitidea_cli.auth import is_source_suppressed
        ...
        # Bitidea auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from bitidea_cli.auth import resolve_qwen_runtime_credentials
        ...
        # Fallback: import from Codex CLI (~/.codex/auth.json) if Bitidea auth
        ...
        if not (isinstance(tokens, dict) and tokens.get("access_token")):
            try:
                from bitidea_cli.auth import _import_codex_cli_tokens, _save_codex_tokens
                cli_tokens = _import_codex_cli_tokens()
                if cli_tokens:
                    logger.info("Importing Codex CLI tokens into Bitidea auth store.")
...
def _prune_stale_seeded_entries(entries: List[PooledCredential], active_sources: Set[str]) -> bool:
    retained = [
        ...
        or not (
            ...
            or entry.source in {"claude_code", "bitidea_pkce"}
        )
    ]
