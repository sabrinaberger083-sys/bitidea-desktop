"""Anthropic Messages API adapter for Bitidea Agent.

Translates between Bitidea's internal OpenAI-style message format and
...
# Bitidea effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Bitidea historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Bitidea)
    ...
    # 1. Bitidea-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration Bitidea configs.
...
# ── Bitidea-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.bitidea/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_BITIDEA_OAUTH_FILE = get_bitidea_home() / ".anthropic_oauth.json"
...
def run_bitidea_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Bitidea-native OAuth PKCE flow and return credential state."""
    import time
    import webbrowser
    ...
    print()
    print("Authorize Bitidea with your Claude Pro/Max subscription.")
    print()
...
def read_bitidea_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Bitidea-managed OAuth credentials from ~/.bitidea/.anthropic_oauth.json."""
    if _BITIDEA_OAUTH_FILE.exists():
        try:
            data = json.loads(_BITIDEA_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read Bitidea OAuth credentials: %s", e)
    return None
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Bitidea Agent", "Claude Code")
                text = text.replace("Bitidea agent", "Claude Code")
                text = text.replace("bitidea-agent", "claude-code")
    ...
    # silently hides reasoning text that Bitidea surfaces in its CLI. We
