"""Centralized logging setup for Bitidea Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.bitidea/logs/`` (profile-aware via ``get_bitidea_home()``).
...
from bitidea_constants import get_config_path, get_bitidea_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_bitidea_session_injector", False):
    ...
    _session_record_factory._bitidea_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``bitidea logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway",),
...
def setup_logging(
    *,
    ...
    """Configure the Bitidea logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    bitidea_home
        Override for the Bitidea home directory.  Falls back to
        ``get_bitidea_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_bitidea_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._bitidea_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the bitidea group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from bitidea_cli.config import is_managed
