...
version: 1.0.0
author: Siddharth Balyan + Bitidea Agent
license: MIT
...
This skill intentionally does not vendor a separate CLI implementation into Bitidea. Install and use upstream `x-cli` instead.
...
If you already keep your X credentials in `~/.bitidea/.env`, the cleanest setup is:

```bash
mkdir -p ~/.config/x-cli
ln -sf ~/.bitidea/.env ~/.config/x-cli/.env
```
...
- **Credential drift**: if you rotate tokens in `~/.bitidea/.env`, make sure `~/.config/x-cli/.env` still points at the current file.
