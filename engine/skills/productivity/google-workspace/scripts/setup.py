#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Bitidea Agent.
...
try:
    from bitidea_constants import display_bitidea_home, get_bitidea_home
except ModuleNotFoundError:
    BITIDEA_AGENT_ROOT = Path(__file__).resolve().parents[4]
    if BITIDEA_AGENT_ROOT.exists():
        sys.path.insert(0, str(BITIDEA_AGENT_ROOT))
    from bitidea_constants import display_bitidea_home, get_bitidea_home

BITIDEA_HOME = get_bitidea_home()
TOKEN_PATH = BITIDEA_HOME / "google_token.json"
CLIENT_SECRET_PATH = BITIDEA_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = BITIDEA_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Bitidea profile to refresh consent."
    )
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Bitidea home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_bitidea_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Bitidea")
