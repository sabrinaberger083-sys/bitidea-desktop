...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("BITIDEA_HOME", os.path.expanduser("~/.bitidea")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("BITIDEA_HOME", Path.home() / ".bitidea")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Bitidea config paths
# ═══════════════════════════════════════════════════════════════════

BITIDEA_HOME = Path(os.getenv("BITIDEA_HOME", Path.home() / ".bitidea"))
CONFIG_PATH = BITIDEA_HOME / "config.yaml"
PREFILL_PATH = BITIDEA_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Bitidea models — already uncensored, just needs clean prompt
    "bitidea": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "bitidea" in model_lower or "nous" in model_lower:
        return "bitidea"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Bitidea config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.bitidea/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Bitidea for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Bitidea for changes to take effect.")
