# Contributing to Bitidea Agent

Thank you for contributing to Bitidea Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — Windows, macOS, different Linux distros, different terminal emulators. We want Bitidea to work everywhere.
...
Bundled skills (in `skills/`) ship with every Bitidea install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `bitidea skills browse` (labeled "official") and install it with `bitidea skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `bitidea skills install`.
...
```bash
git clone --recurse-submodules https://github.com/sabrinaberger083-sys/bitidea-agent.git
cd bitidea-agent
...
```bash
mkdir -p ~/.bitidea/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.bitidea/config.yaml
touch ~/.bitidea/.env

# Add at minimum an LLM provider key:
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.bitidea/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/bitidea" ~/.local/bin/bitidea

# Verify
bitidea doctor
bitidea chat -q "Hello"
```
...
├── cli.py                    # BitideaCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (bitidea-cli, bitidea-telegram, etc.)
├── bitidea_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── bitidea_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.bitidea/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── environments/             # RL training environments (Atropos integration)
├── tests/                    # Test suite
├── website/                  # Documentation site (hermes-agent.nousresearch.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.bitidea/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.bitidea/`)

| Path | Purpose |
|------|---------|
| `~/.bitidea/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.bitidea/.env` | API keys and secrets |
| `~/.bitidea/auth.json` | OAuth credentials (Nous Portal) |
| `~/.bitidea/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.bitidea/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.bitidea/state.db` | SQLite session database |
| `~/.bitidea/sessions/` | JSON session logs |
| `~/.bitidea/cron/` | Scheduled job data |
| `~/.bitidea/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`bitidea_state.py`) with full-text search and unique session titles. JSON logs go to `~/.bitidea/sessions/`.
...
Four fields are supported under `metadata.bitidea`:
...
The user may skip setup and keep loading the skill. Bitidea only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `bitidea setup` or update `~/.bitidea/.env` locally.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Bitidea tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `bitidea --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Bitidea uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.bitidea/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `bitidea_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `bitidea_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Bitidea runs on Linux, macOS, and Windows. When writing code that touches the OS:
...
Bitidea has terminal access. Security matters.
...
2. **Test manually**: Run `bitidea` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/sabrinaberger083-sys/bitidea-agent/issues)
- Include: OS, Python version, Bitidea version (`bitidea version`), full error traceback
